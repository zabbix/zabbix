...
/******************************************************************************
 *                                                                            *
 * Function: zbx_udp_clean                                                    *
 *                                                                            *
 * Purpose: initialize a UDP socket                                           *
 *                                                                            *
 ******************************************************************************/
static void	zbx_udp_clean(zbx_udp_sock_t *s)
{
	memset(s, 0, sizeof(*s));
	s->socket = ZBX_SOCK_ERROR;
}

/******************************************************************************
 *                                                                            *
 * Function: zbx_udp_timeout_set                                              *
 *                                                                            *
 * Purpose: set timeout for socket operations                                 *
 *                                                                            *
 * Parameters: s       - [IN] socket descriptor                               *
 ...
 *                                                                            *
 ******************************************************************************/
static void	zbx_udp_timeout_set(zbx_udp_sock_t *s, int timeout)
{
	s->timeout = timeout;
#if defined(_WINDOWS)
	timeout *= 1000;
...
/******************************************************************************
 *                                                                            *
 * Function: zbx_udp_timeout_cleanup                                          *
 *                                                                            *
 * Purpose: clean up timeout for socket operations                            *
 *                                                                            *
 * Parameters: s - [IN] socket descriptor                                     *
 *                                                                            *
 ******************************************************************************/
static void	zbx_udp_timeout_cleanup(zbx_udp_sock_t *s)
{
#if !defined(_WINDOWS)
	if (0 != s->timeout)
	{
...
/******************************************************************************
 *                                                                            *
 * Function: zbx_udp_create                                                   *
 *                                                                            *
 * Purpose: create a UDP socket for communicating with the given host         *
 *                                                                            *
 * Parameters: s - [OUT] socket descriptor                                    *
 *                                                                            *
 * Return value: SUCCEED - created successfully                               *
 *               FAIL - an error occurred                                     *
 *                                                                            *
 ******************************************************************************/
#if defined(HAVE_IPV6)
...
int	zbx_udp_recv(zbx_udp_sock_t *s, int timeout)
{
	...
	if (ZBX_TCP_ERROR == (s->read_bytes = recvfrom(s->socket, s->buf_stat, sizeof(s->buf_stat) - 1, 0, NULL, NULL)))
	{
		zbx_set_tcp_strerror("recvfrom() failed: %s", strerror_from_system(zbx_sock_last_error()));
		s->read_bytes = FAIL;
	}
	else
		s->buf_stat[s->read_bytes] = '\0';

	if (0 != timeout)
		zbx_udp_timeout_cleanup(s);

	return (FAIL != s->read_bytes ? SUCCEED : FAIL);
}

/******************************************************************************
 *                                                                            *
 * Function: zbx_udp_close                                                    *
 *                                                                            *
