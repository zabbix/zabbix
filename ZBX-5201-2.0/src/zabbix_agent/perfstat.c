...
{
	zabbix_log(LOG_LEVEL_DEBUG, "deactivate_perf_counter() \"%s\"", counter->counterpath);
...
PERF_COUNTER_DATA	*add_perf_counter(const char *name, const char *counterpath, int interval)
{
	...
	{
		zabbix_log(LOG_LEVEL_WARNING, "cannot add performance counter \"%s\": collector is not started",
				counterpath);
		return NULL;
	}
	...
	{
		zabbix_log(LOG_LEVEL_WARNING, "cannot add performance counter \"%s\": interval value out of range",
				counterpath);
		return NULL;
	}
...
void	collect_perfstat()
{
		...
		for (cptr = ppsd.pPerfCounterList; NULL != cptr; cptr = cptr->next)
 		{
			...
			zabbix_log(LOG_LEVEL_DEBUG, "%s() refreshing unsupported counter \"%s\"", __function_name,
					cptr->counterpath);
	...
	for (cptr = ppsd.pPerfCounterList; NULL != cptr; cptr = cptr->next)
	{
		...
		{
			zabbix_log(LOG_LEVEL_DEBUG, "%s() \"%s\" old first:%I64d second:%I64d", __function_name,
					cptr->counterpath,
					cptr->rawValues[(cptr->olderRawValue + 1) % 2].FirstValue,
					cptr->rawValues[(cptr->olderRawValue + 1) % 2].SecondValue);

			zabbix_log(LOG_LEVEL_DEBUG, "%s() \"%s\" new first:%I64d second:%I64d", __function_name,
					cptr->counterpath,
		...
		if (PDH_CALC_NEGATIVE_DENOMINATOR == pdh_status)
		{
			...
			zabbix_log(LOG_LEVEL_DEBUG, "%s() \"%s\" negative denominator error is treated as 0.0",
