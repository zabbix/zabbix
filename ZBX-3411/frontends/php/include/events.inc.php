...
/**
 * function getTriggerIdByExpression
 * get trigger by expression
 *
 * @param INT $triggerid
 * @param INT $hostid
 *
 * @return INT new trigger id
 *
 * @author Vlad
 */
function getTriggerIdByExpression($triggerid, $hostid){
	// get old function
	$options = array(
	...
	$result = reset($result);
	$oldFunc = reset($result['functions']);

	// get old item
	$options = array(
		'itemids' => $oldFunc['itemid'],
		'output' => API_OUTPUT_EXTEND
	);
	$result = CItem::get($options);
	$oldItem = reset($result);

	// compare items and get new item
	$options = array(
		'hostids' => $hostid,
		'filter' => array('description'=> $oldItem['description'], 'key_' => $oldItemId['key_'], 'type' => $oldItem['type'], ),
		'output' => API_OUTPUT_EXTEND
	);
	$result = CItem::get($options);
	$newItem = reset($result);
