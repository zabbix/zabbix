...
function prepareMapExport(&$exportMaps) {
	...
	try {
		foreach ($exportMaps as &$sysmap) {
			unset($sysmap['sysmapid'], $sysmap['iconmapid']);
			...
			foreach ($sysmap['selements'] as &$selement) {
				...
				foreach ($selement['urls'] as $unum => $url) {
					unset($selement['urls'][$unum]['sysmapelementurlid'], $selement['urls'][$unum]['selementid']);
				}
				switch ($selement['elementtype']) {
					case SYSMAP_ELEMENT_TYPE_MAP:
			...
			foreach ($sysmap['links'] as &$link) {
				unset($link['sysmapid'], $link['linkid']);

				foreach ($link['linktriggers'] as &$linktrigger) {
					unset($linktrigger['linktriggerid'], $linktrigger['linkid']);

					$linktrigger['triggerid'] = $triggers[$linktrigger['triggerid']];
				}
			}
			unset($linktrigger, $link);
		}
		unset($sysmap);
	}
	catch (Exception $e) {
