...
jQuery(function($) {
	 ...
	 * @returns bool
	 */
	jQuery.fn.changeDropdownStyle = function(cssClass) {
		var objId = $(this).attr('id');
		if (!objId || !cssClass) {
			return false;
		}
		if ($('#'+objId+' option:selected.'+cssClass).length) {
			$('#'+objId).addClass(cssClass);
		}
		else {
			$('#'+objId).removeClass(cssClass);
		}
		return true;
	}

	$('#hostid').changeDropdownStyle('not-monitored');
