...
func init() {
	...
	plugin.RegisterMetric(&impl, "zabbixsync", "system.swap.in", "Swap in (from device into memory) statistics")
	plugin.RegisterMetric(&impl, "zabbixsync", "system.swap.out", "Swap out (from memory onto device) statistics")
	plugin.RegisterMetric(&impl, "zabbixsync", "system.swap.size", "Swap space size in bytes or in percentage from total")
	...
	plugin.RegisterMetric(&impl, "zabbixsync", "vm.memory.size", "Memory size in bytes or in percentage from total")
	impl.SetCapacity(1)
}
