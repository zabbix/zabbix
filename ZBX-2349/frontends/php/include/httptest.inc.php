...
//      if(!eregi('^([0-9a-zA-Z\_\.[.-.]\$ ]+)$', $name)){
		...
		/**
		 * Commented check below to make possible creating steps in non-ascii
		 * chars.
		 * @see ZBX-2349
		 * @since 08.09.2010
		...
		/**
		 * Adding double qutes and escapind " charecter before saving item name
		 * @author Konstantin Buravcov
		 */
        $name = zbx_str_replace('"', '\"', $name);
        $testname = zbx_str_replace('"', '\"', $testname);
