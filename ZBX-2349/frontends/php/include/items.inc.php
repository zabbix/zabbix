...
/*      $result = DBexecute('DELETE FROM items WHERE '.DBcondition('itemid',$itemids));*/
    ...
    function expand_item_key_by_data($item){
        ...
        if(!empty($macStack)){
            ...
            foreach($macStack as $macro){
                switch($macro){
                    case '{HOSTNAME}':
                        $key = zbx_str_replace('{HOSTNAME}', $host['host'], $key);
                    break;
                    case '{IPADDRESS}':
                        $key = zbx_str_replace('{IPADDRESS}', $host['ip'], $key);
                    break;
                    case '{HOST.DNS}':
                        $key = zbx_str_replace('{HOST.DNS}', $host['dns'], $key);
                    break;
                    case '{HOST.CONN}':
                        $key = zbx_str_replace('{HOST.CONN}', $host['useip'] ? $host['ip'] : $host['dns'], $key);
                    break;
                }
            }
        }
	...
	function item_description($item){
		 ...
		 */
        for($i=9;$i>0;$i--){
            $descr = zbx_str_replace("$$i",get_n_param($key,$i),$descr);
        }
