...
//		VAR			TYPE	OPTIONAL FLAGS	VALIDATION	EXCEPTION
	$fields = array(
		...
		'graphtype'=>	array(T_ZBX_INT, O_OPT, NULL,		IN('0,1,2,3'),
			'isset({save}) || isset({preview})'),
		'yaxismin'=>	array(T_ZBX_DBL, O_OPT, NULL,		null,
			'(isset({save}) || isset({preview}))&&(({graphtype} == 0) || ({graphtype} == 1))'),
		'yaxismax'=>	array(T_ZBX_DBL, O_OPT, NULL,		null,
			'(isset({save}) || isset({preview}))&&(({graphtype} == 0) || ({graphtype} == 1))'),
		'graph3d'=>	array(T_ZBX_INT, O_OPT,	P_NZERO,	IN('0,1'),	null),
		'legend'=>	array(T_ZBX_INT, O_OPT,	P_NZERO,	IN('0,1'),	null),
		'ymin_itemid'=>	array(T_ZBX_INT, O_OPT, NULL,		DB_ID,
			'(isset({save}) || isset({preview}))&&isset({ymin_type})&&({ymin_type}==3)'),
		'ymax_itemid'=>	array(T_ZBX_INT, O_OPT, NULL,		DB_ID,
			'(isset({save}) || isset({preview}))&&isset({ymax_type})&&({ymax_type}==3)'),
		'percent_left'=>	array(T_ZBX_DBL, O_OPT, NULL,	BETWEEN(0, 100),	null,
			_("Percentile line (Left)"), _s('must be between %1$s and %2$s', 0, 100)),
		'percent_right'=>	array(T_ZBX_DBL, O_OPT, NULL,	BETWEEN(0,100),	null,
			_("Percentile line (Right)"), _s('must be between %1$s and %2$s', 0, 100)),
