...
#	define zbx_rwlock_wrlock(rwlock)	do {zbx_prof_start(__func__, ZBX_PROF_RWLOCK); __zbx_rwlock_wrlock(__FILE__, __LINE__, rwlock); zbx_prof_end_wait(); }while(0);
#	define zbx_rwlock_rdlock(rwlock)	do {zbx_prof_start(__func__, ZBX_PROF_RWLOCK); __zbx_rwlock_rdlock(__FILE__, __LINE__, rwlock); zbx_prof_end_wait(); }while(0);
#	define zbx_rwlock_unlock(rwlock)	do {__zbx_rwlock_unlock(__FILE__, __LINE__, rwlock); zbx_prof_end();}while(0);
...
#endif	/* _WINDOWS */
#	define zbx_mutex_lock(mutex)		do {zbx_prof_start(__func__, ZBX_PROF_MUTEX); __zbx_mutex_lock(__FILE__, __LINE__, mutex); zbx_prof_end_wait(); }while(0);
#	define zbx_mutex_unlock(mutex)		do {__zbx_mutex_unlock(__FILE__, __LINE__, mutex); zbx_prof_end();}while(0);
