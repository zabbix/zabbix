...
DC_ITEM
{
	DC_HOST		host;
	zbx_uint64_t	itemid;
	char		*string_list;	/* pointer to a null-separated strings, for own needs */
	const char	*key_orig;
	const char	*logtimefmt;
	const char	*snmp_community_orig;
	const char	*snmp_oid_orig;
	const char	*snmpv3_securityname_orig;
	const char	*snmpv3_authpassphrase_orig;
	const char	*snmpv3_privpassphrase_orig;
	const char	*trapper_hosts;
	const char	*ipmi_sensor;
	char		*params_orig;
	const char	*username_orig;
	const char	*publickey_orig;
	const char	*privatekey_orig;
