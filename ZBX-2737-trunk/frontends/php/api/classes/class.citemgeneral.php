	...
	protected function errorInheritFlags($flag, $key, $host){
		switch($flag){
			case ZBX_FLAG_DISCOVERY_NORMAL:
				self::exception(ZBX_API_ERROR_PARAMETERS, _s('Item with key "%2$s" already exists on host "%1$s" as an item.', $host, $key));
				break;
			case ZBX_FLAG_DISCOVERY:
				self::exception(ZBX_API_ERROR_PARAMETERS, _s('Item with key "%2$s" already exists on host "%1$s" as a discovery rule.', $host, $key));
				break;
			case ZBX_FLAG_DISCOVERY_CHILD:
				self::exception(ZBX_API_ERROR_PARAMETERS, _s('Item with key "%2$s" already exists on host "%1$s" as an item prototype.', $host, $key));
				break;
			case ZBX_FLAG_DISCOVERY_CREATED:
				self::exception(ZBX_API_ERROR_PARAMETERS, _s('Item with key "%2$s" already exists on host "%1$s" as an item created from item prototype.', $host, $key));
				break;
			default:
				self::exception(ZBX_API_ERROR_PARAMETERS, _s('Item with key "%2$s" already exists on host "%1$s" as unknown item element.', $host, $key));
		}
	}
