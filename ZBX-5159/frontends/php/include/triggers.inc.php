...
function get_trigger_overview_cells($triggerHosts, $hostName, $params = array()) {
	...
	if (isset($triggerHosts[$hostName])) {
		...
		while ($dep_row = DBfetch($dep_res)) {
			$dep_table->addRow(SPACE.'-'.SPACE.CDescription::expandTriggerById($dep_row['triggerid_up']));
			$dependency = true;
		}
		...
		while ($dep_row = DBfetch($dep_res)) {
			$dep_table->addRow(SPACE.'-'.SPACE.CDescription::expandTriggerById($dep_row['triggerid_down']));
			$dependency = true;
		}
