...
static void	read_tags(const char *path, zbx_vector_db_tag_ptr_t *tags)
{
	...
	while (ZBX_MOCK_END_OF_VECTOR != (mock_err = (zbx_mock_vector_element(htag_vector, &htag))))
	{
		...
		/* tag_orig */
		mock_err = zbx_mock_object_member(htag, "tag_orig", &hname_orig);

		if (ZBX_MOCK_SUCCESS == mock_err)
		{
			if (ZBX_MOCK_SUCCESS != (mock_err = zbx_mock_string(hname_orig, &tag_orig_str)))
				fail_msg("Cannot read '%s[%d].tag_orig': %s", path, i, zbx_mock_error_string(mock_err));
			/* "~" is the canonical representation of NULL in YAML */
			else if (0 != strcmp("~", tag_orig_str))
				db_tag->tag_orig = zbx_strdup(NULL, tag_orig_str);
		}
		else if (ZBX_MOCK_NO_SUCH_MEMBER != mock_err)
			fail_msg("Cannot read '%s[%d].tag_orig': %s", path, i, zbx_mock_error_string(mock_err));

		/* value_orig */
		mock_err = zbx_mock_object_member(htag, "value_orig", &hvalue_orig);

		if (ZBX_MOCK_SUCCESS == mock_err)
		{
			if (ZBX_MOCK_SUCCESS != (mock_err = zbx_mock_string(hvalue_orig, &value_orig_str)))
			{
				fail_msg("Cannot read '%s[%d].value_orig': %s", path, i,
						zbx_mock_error_string(mock_err));
			}
			/* "~" is the canonical representation of NULL in YAML */
			else if (0 != strcmp("~", value_orig_str))
				db_tag->value_orig = zbx_strdup(NULL, value_orig_str);
		}
		else if (ZBX_MOCK_NO_SUCH_MEMBER != mock_err)
			fail_msg("Cannot read '%s[%d].value_orig': %s", path, i, zbx_mock_error_string(mock_err));
