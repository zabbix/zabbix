...
{
	const zbx_config_option_t	*opt1 = (zbx_config_option_t *)d1;
	const zbx_config_option_t	*opt2 = (zbx_config_option_t *)d2;

	return strcmp(opt1->name, opt2->name);
}

static void	mock_get_options(const char *path, zbx_vector_config_option_t *options)
{
	...
	zbx_vector_config_option_sort(options, history_option_compare);
}

static void	mock_compare_options(const zbx_vector_config_option_t *opt_exp,
		const zbx_vector_config_option_t *opt_ret)
{
...
void	zbx_mock_test_entry(void **state)
{
	...
	zbx_vector_config_option_t	options_ret, options_exp;
	const char			*conf, *name_exp;
	char				*name_ret, *error = NULL;

	ZBX_UNUSED(state);

	zbx_vector_config_option_create(&options_ret);
	zbx_vector_config_option_create(&options_exp);
	...
	if (FAIL != result_ret)
	{
		...
		zbx_vector_config_option_sort(&options_ret, history_option_compare);
		mock_get_options("out.options", &options_exp);
		mock_compare_options(&options_exp, &options_ret);

		zbx_config_option_clear_options(options_ret.values, options_ret.values_num);
		zbx_config_option_clear_options(options_exp.values, options_exp.values_num);
	}

	zbx_vector_config_option_destroy(&options_ret);
	zbx_vector_config_option_destroy(&options_exp);
}
