 ...
 *                                                                            *
 ******************************************************************************/
static void	get_request_params(zbx_mock_handle_t handle, zbx_uint64_t *itemid, unsigned char *value_type,
		int *seconds, int *count, int *end)
{
	...
	*value_type = zbx_mock_get_value_type(zbx_mock_get_object_member_string(handle, "value type"));
...
/******************************************************************************
 *                                                                            *
 * Function: set_vc_mode                                                      *
 *                                                                            *
 * Purpose: sets value cache mode if the specified key is present in input    *
 *          data                                                              *
 *                                                                            *
 ******************************************************************************/
static void	set_vc_mode(zbx_mock_handle_t hitem, const char *key)
{
	const char		*data;
	...
	if (ZBX_MOCK_SUCCESS == zbx_mock_object_member(hitem, key, &hmode))
	{
		...
		zbx_vc_set_mode(zbx_vcmock_get_cache_mode(data));
	}
}

/******************************************************************************
 *                                                                            *
 * Function: set_vc_size                                                      *
 *                                                                            *
 * Purpose: sets the available size in value cache if the specified key is    *
 *          present in input data                                             *
 *                                                                            *
 ******************************************************************************/
static void	set_vc_size(zbx_mock_handle_t hitem, const char *key)
{
	const char		*data;
...
/******************************************************************************
 *                                                                            *
 * Function: set_vc_time                                                      *
 *                                                                            *
 * Purpose: sets the current time. The key must be present in input data or   *
 *          the test case will fail                                           *
 *                                                                            *
 ******************************************************************************/
static void	set_vc_time(zbx_mock_handle_t hitem, const char *key)
{
	zbx_mock_error_t	err;
	const char		*data;
	zbx_timespec_t		ts;
...
void	zbx_mock_test_entry(void **state)
{
	...
	if (ZBX_MOCK_SUCCESS == zbx_mock_parameter("in.precache", &handle))
	{
		...
		{
			set_vc_time(hitem, "time");
			set_vc_mode(hitem, "cache mode");
			set_vc_size(hitem, "cache size");

			get_request_params(hitem, &itemid, &value_type, &seconds, &count, &end);
	...
	set_vc_time(handle, "time");
	set_vc_mode(handle, "cache mode");

	get_request_params(handle, &itemid, &value_type, &seconds, &count, &end);
	...
	while (ZBX_MOCK_END_OF_VECTOR != (mock_err = (zbx_mock_vector_element(hitems, &hitem))))
	{
		...
		if (ZBX_MOCK_SUCCESS == mock_err)
		{
			...
			zbx_mock_assert_int_eq("item.status", zbx_vcmock_get_item_status(data), item_status);
			...
			value_type = zbx_mock_get_value_type(zbx_mock_get_object_member_string(hitem, "value type"));
	...
	zbx_mock_assert_int_eq("cache.mode", zbx_vcmock_get_cache_mode(zbx_mock_get_parameter_string("out.cache.mode")),
			cache_mode);
