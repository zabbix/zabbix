...
/* Drives pb_discovery_check_age()'s offline-buffer eviction loop through the public write API
 ...
 * through zbx_pb_discovery_close() a FAIL unconditionally falls through to a real database
 * write for the rows being closed (see zbx_pb_mock_stubs.c), which these tests do not set up. */
void	zbx_mock_test_entry(void **state)
{
	...
	/* pb->discovery is empty for this first close, so its own age check trivially succeeds
	 * regardless of "row_age" - the second close below is what exercises eviction */
	handle = zbx_pb_discovery_open();
	zbx_pb_discovery_write_service(handle, 1, 1, "127.0.0.1", "", 0, 0, "", (int)time(NULL) - row_age);
	zbx_pb_discovery_close(handle);

	handle = zbx_pb_discovery_open();
	zbx_pb_discovery_write_service(handle, 1, 1, "127.0.0.2", "", 0, 0, "", (int)time(NULL));
	zbx_pb_discovery_close(handle);
