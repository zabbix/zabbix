...
void	zbx_mock_test_entry(void **state)
{
	...
	ret = zbx_substitute_snmp_oid_params(&result, error, MAX_BUFFER_LEN, &subst_func_resolved, macro);
	...
	if (SUCCEED == ret)
	{
		...
		zbx_mock_assert_str_eq("zbx_substitute_snmp_oid_params() result", expected_result, result);
	}
	else
	{
		const char	*expected_error = zbx_mock_get_parameter_string("out.error");

		zbx_mock_assert_str_eq("zbx_substitute_snmp_oid_params() error", expected_error, error);
	}
