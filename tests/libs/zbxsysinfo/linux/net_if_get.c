...
void	zbx_mock_test_entry(void **state)
{
	...
	const char		*item_key = "net.if.get";
	...
	if (ZBX_MOCK_SUCCESS == zbx_mock_in_parameter("key", &handle))
	{
		if (ZBX_MOCK_SUCCESS != zbx_mock_string(handle, &item_key))
		{
			fail_msg("Cannot read \"key\" parameter");
		}
	}
	...
	actual_ret = net_if_get(&request, &result);

	zbx_free_agent_request(&request);

	if (actual_ret != expected_ret)
		fail_msg("Unexpected return code from net_if_get(): expected %s, got %s",
