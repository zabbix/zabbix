...
class CTriggerFunctionValidator extends CValidator {
	...
	public function validate($value) {
		...
		if (!isset($this->allowed[$value['functionName']])) {
			$this->setError('Incorrect trigger function "'.$value['functionName'].'" provided in expression. Unknown function.');
			return false;
		}

		if (!isset($this->allowed[$value['functionName']]['value_types'][$value['valueType']])) {
			$this->setError('Incorrect item value type provided for trigger function "'.$value['functionName'].'".');
			return false;
		}

		if (count($this->allowed[$value['functionName']]['args']) < count($value['functionParamList'])) {
			$this->setError('Incorrect trigger function "'.$value['functionName'].'" provided in expression. '.
					'Function supports '.count($this->allowed[$value['functionName']]['args']).' parameters.');
			return false;
		}
					...
					switch ($arg['type']) {
						case 'str':
							if (!is_string($value['functionParamList'][$aNum])) {
								$this->setError('Incorrect trigger function "'.$value['functionName'].'" provided in expression. '.
										'Parameter of type string or user macro expected, "'.$value['functionParamList'][$aNum].'" given.');
								return false;
							}
							break;
						case 'sec':
							if (!$this->validateSec($value['functionParamList'][$aNum])) {
								$this->setError('Incorrect trigger function "'.$value['functionName'].'" provided in expression. '.
										'Parameter sec or user macro expected, "'.$value['functionParamList'][$aNum].'" given.');
								return false;
							}
							break;
						case 'sec_num':
							if (!$this->validateSecNum($value['functionParamList'][$aNum])) {
								$this->setError('Incorrect trigger function "'.$value['functionName'].'" provided in expression. '.
										'Parameter sec or #num or user macro expected, "'.$value['functionParamList'][$aNum].'" given.');
								return false;
							}
							break;
						case 'num':
							if (!is_numeric($value['functionParamList'][$aNum])) {
								$this->setError('Incorrect trigger function "'.$value['functionName'].'" provided in expression. '.
										'Parameter num or user macro expected, "'.$value['functionParamList'][$aNum].'" given.');
								return false;
							}
							break;
					}
