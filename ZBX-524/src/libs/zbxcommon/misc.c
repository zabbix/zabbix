...
void    *zbx_malloc2(const char *filename, int line, void *old, size_t size)
{
	...
	zabbix_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] zbx_malloc: out of memory. Requested " ZBX_FS_SIZE " bytes.",
			filename, line, (zbx_size_t)size);
	exit(FAIL);
...
void    *zbx_realloc2(const char *filename, int line, void *src, size_t size)
{
	...
	zabbix_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] zbx_realloc: out of memory. Requested " ZBX_FS_SIZE " bytes.",
			filename, line, (zbx_size_t)size);
	exit(FAIL);
...
char    *zbx_strdup2(const char *filename, int line, char *old, const char *str)
{
	...
	zabbix_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] zbx_strdup: out of memory. Requested " ZBX_FS_SIZE " bytes.",
			filename, line, (zbx_size_t)(strlen(str) + 1));
	exit(FAIL);
