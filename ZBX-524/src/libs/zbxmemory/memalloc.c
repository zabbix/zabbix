...
void	zbx_mem_create(zbx_mem_info_t **info, key_t shm_key, int lock_name, size_t size, const char *descr, const char *param)
{
	...
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() descr:'%s' param:'%s' size:" ZBX_FS_SIZE,
			__function_name, descr, param, (zbx_size_t)size);

	/* allocate shared memory */

	if (4 != ZBX_PTR_SIZE && 8 != ZBX_PTR_SIZE)
	{
		zabbix_log(LOG_LEVEL_CRIT, "failed assumption about pointer size (" ZBX_FS_SIZE " not in {4, 8})",
				(zbx_size_t)ZBX_PTR_SIZE);
		exit(FAIL);
	}

	if (!(MEM_MIN_SIZE <= size && size <= MEM_MAX_SIZE))
	{
		zabbix_log(LOG_LEVEL_CRIT, "requested size " ZBX_FS_SIZE " not within bounds [%d <= size <= %d]",
				(zbx_size_t)size, MEM_MIN_SIZE, MEM_MAX_SIZE);
		exit(FAIL);
	}
	...
	*info = ALIGN8(base);
	...
	zabbix_log(LOG_LEVEL_DEBUG, "valid user addresses: [%p, %p] total size: " ZBX_FS_SIZE,
			(*info)->lo_bound + MEM_SIZE_FIELD,
			(*info)->hi_bound - MEM_SIZE_FIELD,
			(zbx_size_t)(*info)->total_size);
...
void	*__zbx_mem_malloc(const char *file, int line, zbx_mem_info_t *info, const void *old, size_t size)
{
	...
	{
		zabbix_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): asking for a bad number of bytes (" ZBX_FS_SIZE ")",
				file, line, __function_name, (zbx_size_t)size);
		exit(FAIL);
	}
	...
	if (NULL == chunk)
	{
		zabbix_log(LOG_LEVEL_CRIT, "[file:%s,line:%d] %s(): out of memory (requested " ZBX_FS_SIZE " bytes)",
				file, line, __function_name, (zbx_size_t)size);
...
size_t	zbx_mem_required_size(size_t size, int chunks_num, const char *descr, const char *param)
{
	...
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() size:" ZBX_FS_SIZE " chunks_num:%d descr:'%s' param:'%s'",
			__function_name, (zbx_size_t)size, chunks_num, descr, param);
	...
	zabbix_log(LOG_LEVEL_DEBUG, "End of %s() size:" ZBX_FS_SIZE, __function_name, (zbx_size_t)size);
