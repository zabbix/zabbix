		...
		int *interval, int *size, int *timeout, icmppingsec_type_t *type, char *error, int max_error_len)
{
	...
	else if (FAIL == is_uint32(buffer, (uint32_t*)count) || MIN_COUNT > *count || *count > MAX_COUNT)
	{
	...
	else if (FAIL == is_uint32(buffer, (uint32_t*)interval) || MIN_INTERVAL > *interval)
	{
	...
	else if (FAIL == is_uint32(buffer, (uint32_t*)size) || MIN_SIZE > *size || *size > MAX_SIZE)
	{
	...
	if (0 != get_param(params, 5, buffer, sizeof(buffer)) || '\0' == *buffer)
		*timeout = 0;
	else if (FAIL == is_uint32(buffer, (uint32_t*)timeout) || MIN_TIMEOUT > *timeout)
	{
