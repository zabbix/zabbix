...
class CTweenBox {
	...
	public function __construct(&$form, $name, $value = null, $size = 10) {
		...
		$this->lbox->attr('style', 'width: 280px;');
		$this->rbox->attr('style', 'width: 280px;');
	}
	...
	public function get($caption_l = null, $caption_r = null) {
		...
		$add_btn->attr('onclick', 'moveListBoxSelectedItem("'.$this->form->getName().'", "'.$this->varname.'", "'.$this->id_r.'", "'.$this->id_l.'", "add");');
		$rmv_btn = new CButton('remove', '  &raquo;  ', null, 'formlist');
		$rmv_btn->attr('onclick', 'moveListBoxSelectedItem("'.$this->form->getName().'", "'.$this->varname.'", "'.$this->id_l.'", "'.$this->id_r.'", "rmv");');
