...
// PRIVATE --------------------------------------------------------------------------------------------
	...
	private function checkSymbolPrevious($symbol){
		...
		if(isset($this->symbols['linkage'][$symbol]) && ($this->symbols['previous'] == $symbol)){
	...
	private function checkSymbolSequence($symbol){
		...
		if($this->currExpr['part']['expression']){
			switch($symbol){
				case '"':
					if(($this->symbols['expr']['previous'] == '\\') && ($this->symbols['expr']['sequence'] % 2 == 1)){
			...
			if(!isset($this->symbols['linkage'][$this->symbols['previous']])){
				if(!zbx_empty($this->symbols['previous']) && !zbx_ctype_digit($this->symbols['previous']))
	...
	private function checkSimpleExpression(&$expression){
		...
		if(!preg_match('/^([\(\)\=\#\<\&\|\>\d\s]+)$/i', $simpleExpr))
			throw new Exception('Incorrect trigger expression format " '.$expression.' "');
//SDI($this->expressions);
//SDI($simpleExpr);
		$exprLinks = $this->symbols['linkage']['&'] + 
			$this->symbols['linkage']['|'] +
			$this->symbols['linkage']['<'] +
			$this->symbols['linkage']['>'] +
			$this->symbols['linkage']['='] +
			$this->symbols['linkage']['#'];
	...
	private function setPreviousSymbol($symbol){
		if($this->symbols['previous'] == $symbol){
			...
			if($symbol != ' ')
				$this->symbols['previous'] = $symbol;
		}

		if(isset($this->symbols['open'][$symbol])){
			$this->symbols['open']['previous'] = $symbol;
		}

		if(isset($this->symbols['close'][$symbol])){
			$this->symbols['close']['previous'] = $symbol;
		}

		if(isset($this->symbols['linkage'][$symbol])){
			$this->symbols['linkage']['previous'] = $symbol;
		}

		if(isset($this->symbols['expr'][$symbol])){
			$this->symbols['expr']['previous'] = $symbol;
		}
	}
	...
	private function setExpressionParts($symbol){
		if(!$this->inQuotes($symbol)){
			if(!$this->currExpr['part']['key']){
				if($symbol == '{'){
					$this->currExpr = array(
						'part' => array( 'expression' => false, 'usermacro' => false, 'host' => false, 'key' => false,'function' => false, 'functionName' => false),
						'object' => array('expression' => '', 'usermacro' => '', 'host' => '', 'key' => '', 'function' => '', 'functionName' => '')
					);
				}
