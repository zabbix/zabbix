...
class CTriggerExpressionTest extends PHPUnit_Framework_TestCase {
	...
	public static function provider() {
		return [
			...
			[
				'1 - (1/({host:key.last(0)}+1))+(({host:key.last(0)}+1))/({host:key.last(0)}+1)/({host:key.last(0)}+1)',
				null,
				true
			],
			['(({host:key.last(0)}+0.1)) + + (({host:key.last(0)}+0.1))', null, false],
			['(({host:key.last(0)}+0.1)) + - (({host:key.last(0)}+0.1))', null, true],
			['(({host:key.last(0)}+0.1)) + (({host:key.last(0)}+0.1))', null, true],
			['(({host:key.last(0)}+0.1))', null, true],
			...
			['({host:key.last(0)}+0.1)-({host:key.last(0)}+0.1)', null, true],
			['({host:key.last(0)}+0.1)+({host:key.last(0)}+0.1)', null, true],
			['({host:key.last(0)}+0.1) or ({host:key.last(0)}+0.1)', null, true],
			['({host:key.last(0)}+0.1) and ({host:key.last(0)}+0.1)', null, true],
			['({host:key.last(0)}+0.1) and ({host:key.last(0)}+0.1)/({host:key.last(0)}+0.1)', null, true],
			['({host:key.last(0)}+0.1)+({host:key.last(0)}+0.1)/({host:key.last(0)}+0.1)', null, true],
			[
				'1 - (1/({host:key.last(0)}+0.1))+(({host:key.last(0)}+0.1))/({host:key.last(0)}+0.1)/({host:key.last('.
					'0)}+0.1)',
				null,
				true
			],
