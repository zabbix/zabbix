...
// db storage field is 256
		...
		if(count($delay_flex_el)==0)
			array_push($delay_flex_el, _('No flexible intervals'));
		else
		...
		if(count($dbHosts) == 1){
			...
			$frmItem->addRow(array( new CVisibilityBox('interface_visible', get_request('interface_visible'), 'interfaceid', _('Original')),
				_('Host interface')), $sbIntereaces);
		}
		...
		$frmItem->addRow(array( new CVisibilityBox('type_visible', get_request('type_visible'), 'type', _('Original')),
			S_TYPE), $cmbType);

		$frmItem->addRow(array( new CVisibilityBox('community_visible', get_request('community_visible'), 'snmp_community', _('Original')),
			_('SNMP community')), new CTextBox('snmp_community',$snmp_community,16));

		$frmItem->addRow(array( new CVisibilityBox('securityname_visible', get_request('securityname_visible'), 'snmpv3_securityname',
			_('Original')), _('SNMPv3 security name')), new CTextBox('snmpv3_securityname',$snmpv3_securityname,64));
		...
		$frmItem->addRow(array( new CVisibilityBox('securitylevel_visible',  get_request('securitylevel_visible'), 'snmpv3_securitylevel',
			_('Original')), _('SNMPv3 security level')), $cmbSecLevel);
		$frmItem->addRow(array( new CVisibilityBox('authpassphrase_visible', get_request('authpassphrase_visible'),
			'snmpv3_authpassphrase', _('Original')), _('SNMPv3 auth passphrase')),
		...
		$frmItem->addRow(array( new CVisibilityBox('privpassphras_visible', get_request('privpassphras_visible'), 'snmpv3_privpassphrase',
			_('Original')), _('SNMPv3 priv passphrase')), new CTextBox('snmpv3_privpassphrase',$snmpv3_privpassphrase,64));

		$frmItem->addRow(array( new CVisibilityBox('port_visible', get_request('port_visible'), 'port', _('Original')), S_PORT),
		...
		$frmItem->addRow(array( new CVisibilityBox('value_type_visible', get_request('value_type_visible'), 'value_type', _('Original')),
			_('Type of information')), $cmbValType);

		$cmbDataType = new CComboBox('data_type',$data_type);
		$cmbDataType->addItems(item_data_type2str());
		$frmItem->addRow(array( new CVisibilityBox('data_type_visible', get_request('data_type_visible'), 'data_type', _('Original')),
			_('Data type')), $cmbDataType);

		$frmItem->addRow(array( new CVisibilityBox('units_visible', get_request('units_visible'), 'units', _('Original')), _('Units')),
		...
		$frmItem->addRow(
			array(new CVisibilityBox('authtype_visible', get_request('authtype_visible'), 'authtype', _('Original')), _('Authentication method')),
			$cmbAuthType
		);
		$frmItem->addRow(
			array(new CVisibilityBox('username_visible', get_request('username_visible'), 'username', _('Original')), _('User name')),
			new CTextBox('username', $username, 40)
		);
		$frmItem->addRow(
			array(new CVisibilityBox('publickey_visible', get_request('publickey_visible'), 'publickey', _('Original')), _('Public key file')),
			new CTextBox('publickey', $publickey, 40)
		);
		$frmItem->addRow(
			array(new CVisibilityBox('privatekey_visible', get_request('privatekey_visible'), 'privatekey', _('Original')), _('Private key file')),
			new CTextBox('privatekey', $privatekey, 40)
		);
		$frmItem->addRow(
			array(new CVisibilityBox('password_visible', get_request('password_visible'), 'password', _('Original')), _('Password')),
			new CTextBox('password', $password, 40)
		);

		$frmItem->addRow(array( new CVisibilityBox('formula_visible', get_request('formula_visible'), 'formula', _('Original')),
			_('Custom multiplier').' (0 - '.S_DISABLED.')'), new CTextBox('formula', $formula,40));

		$frmItem->addRow(array( new CVisibilityBox('delay_visible', get_request('delay_visible'), 'delay', _('Original')),
		...
		$frmItem->addRow(array(
			new CVisibilityBox('delay_flex_visible',
					...
					_('Original')),
			_('Flexible intervals (sec)')), $delay_flex_el);
		...
		$frmItem->addRow(_('New flexible interval'), $new_delay_flex_el, 'new');

		$frmItem->addRow(array( new CVisibilityBox('history_visible', get_request('history_visible'), 'history', _('Original')),
			_('Keep history (in days)')), new CNumericBox('history',$history,8));
		$frmItem->addRow(array( new CVisibilityBox('trends_visible', get_request('trends_visible'), 'trends', _('Original')),
			_('Keep trends (in days)')), new CNumericBox('trends',$trends,8));

		$cmbStatus = new CComboBox('status',$status);
		foreach(array(ITEM_STATUS_ACTIVE,ITEM_STATUS_DISABLED,ITEM_STATUS_NOTSUPPORTED) as $st)
			$cmbStatus->addItem($st,item_status2str($st));
		$frmItem->addRow(array( new CVisibilityBox('status_visible', get_request('status_visible'), 'status', _('Original')), S_STATUS),
			$cmbStatus);

		$frmItem->addRow(array( new CVisibilityBox('logtimefmt_visible', get_request('logtimefmt_visible'), 'logtimefmt', _('Original')),
			_('Log time format')), new CTextBox("logtimefmt",$logtimefmt,16));

		$cmbDelta= new CComboBox('delta',$delta);
		$cmbDelta->addItem(0, _('As is'));
		$cmbDelta->addItem(1, _('Delta (speed per second)'));
		$cmbDelta->addItem(2, _('Delta (simple change)'));
		$frmItem->addRow(array( new CVisibilityBox('delta_visible', get_request('delta_visible'), 'delta', _('Original')),
			_('Store value')), $cmbDelta);

		$cmbMap = new CComboBox('valuemapid',$valuemapid);
		$cmbMap->addItem(0, _('As is'));
		...
		$frmItem->addRow(array( new CVisibilityBox('valuemapid_visible', get_request('valuemapid_visible'), 'valuemapid', _('Original')),
			_('Show value')), array($cmbMap, SPACE, $link));

		$frmItem->addRow(array( new CVisibilityBox('trapper_hosts_visible', get_request('trapper_hosts_visible'), 'trapper_hosts',
			_('Original')), _('Allowed hosts')), new CTextBox('trapper_hosts', $trapper_hosts, 40));
		...
		$frmItem->addRow(array( new CVisibilityBox('applications_visible', get_request('applications_visible'), 'applications_',
			_('Original')), S_APPLICATIONS),$cmbApps);

		$tarea = new CTextArea('description', $description);
		$tarea->addStyle('margin-top: 5px;');
		$frmItem->addRow(array( new CVisibilityBox('description_visible', get_request('description_visible'), 'description', _('Original')),
			_('Description')), $tarea);
