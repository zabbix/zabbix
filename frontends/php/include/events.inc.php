 ...
 * @param array $correlation						An array of event correlation data.
 * @param string $correlation['correlationid']		Event correlation ID. If no correlation is given, ID does not exist.
 * @param string $correlation['name']				Event correlation name. If no correlation is given, name does not exist.
 * @param array $trigger							An array of trigger data.
 * @param string $backurl							A link back after acknowledgement has been clicked.
 *
 * @return CTableInfo
 */
function make_event_details($event, array $correlation, $trigger, $backurl) {
	$config = select_config();
