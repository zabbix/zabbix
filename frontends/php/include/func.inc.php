...
function access_deny($mode = ACCESS_DENY_OBJECT) {
	...
	// deny access to a page
		...
		if (CWebUser::isLoggedIn()) {
			...
			if (CWebUser::isGuest()) {
				$buttons[] = new CButton('login', _('Login'),
					'javascript: document.location = "index.php?request='.$url.'";', 'button-form'
				);
			}
			$buttons[] = new CButton('back', _('Go to dashboard'),
				'javascript: document.location = "dashboard.php"', 'button-form'
			);
		}
		// if the user is not logged in - offer to login
			...
			$buttons = array(
				new CButton('login', _('Login'), 'javascript: document.location = "index.php?request='.$url.'";', 'button-form')
			);
		}
