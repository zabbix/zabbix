...
/**
 * Collects all tags of all parent templates and combines them with host tags.
 *
 * @param array  $hosts
 * @param array  $hosts[<hostid>]['parentTemplates']
 * @param int    $hosts[<hostid>]['parentTemplates'][]['templateid']
 * @param array  $hosts[<hostid>]['tags']
 * @param string $hosts[<hostid>]['tags'][]['tag']
 * @param string $hosts[<hostid>]['tags'][]['value']
 */
function addInheritedTags(array &$hosts) {
	$host_templates = [];
	$templateids = [];
	...
	foreach ($hosts as $hostid => &$host) {
		...
		foreach ($host['parentTemplates'] as $template) {
			$host_templates[$hostid][] = $template['templateid'];
	...
	while ($templateids) {
		...
		foreach ($db_templates as $db_template) {
			$template_tags[$db_template['templateid']] = $db_template['tags'];

			foreach ($db_template['parentTemplates'] as $template) {
				$parent_templates[$db_template['templateid']][] = $template['templateid'];

				if (!array_key_exists($template['templateid'], $all_templateids)) {
					$templateids[$template['templateid']] = true;
				}
			}
		}
	...
	$all_parent_templates = [];

	foreach ($parent_templates as $parent_templateid => $templateids) {
		$all_parent_templates[$parent_templateid] = getTemplateParentTemplates($templateids, $parent_templates);
	}

	foreach ($host_templates as $hostid => $templateids) {
		foreach ($templateids as $templateid) {
			if (array_key_exists($templateid, $all_parent_templates)) {
				$host_templates[$hostid] = array_merge($host_templates[$hostid], $all_parent_templates[$templateid]);
			}
		}
	}
	...
	foreach ($host_templates as $hostid => $templateids) {
		foreach ($templateids as $templateid) {
			if (array_key_exists($templateid, $template_tags)) {
				foreach ($template_tags[$templateid] as $t_tag) {
					foreach ($hosts[$hostid]['tags'] as $h_tag) {
						if ($h_tag['tag'] === $t_tag['tag'] && $h_tag['value'] === $t_tag['value']) {
							continue 2;
						}
					}

					$hosts[$hostid]['tags'][] = $t_tag + ['inherited' => true];
				}
			}
		}
	}
...
/**
 * Auxiliary function for addInheritedTags().
 *
 ...
 * @return array
 */
function getTemplateParentTemplates(array $templateids, array $parent_templates) {
	$templates = [];

	foreach ($templateids as $templateid) {
		$templates[] = $templateid;

		if (array_key_exists($templateid, $parent_templates)) {
			$templates = array_merge($templates,
				getTemplateParentTemplates($parent_templates[$templateid], $parent_templates)
			);
		}
	}
