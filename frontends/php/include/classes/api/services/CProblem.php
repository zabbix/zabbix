			...
			foreach ($options['tags'] as $tag) {
				...
				switch ($tag['operator']) {
					case TAG_OPERATOR_EQUAL:
						if ($tag_count[$tag['tag']] > 1) {
							if ($tag_num == 0) {
								...
								$value = ' AND (pt.value='.zbx_dbstr($tag['value']);
							}
							elseif ($prev_tag === $tag['tag']) {
								// Not the first value. Add next OR statement.
								$value .= ' OR pt.value='.zbx_dbstr($tag['value']);
							}
					...
					default:
						...
						if ($tag_count[$tag['tag']] > 1) {
							if ($tag_num == 0) {
								// First value of many. Open Brace for OR statement.
								$value = ' AND (UPPER(pt.value) LIKE'.zbx_dbstr($tag['value'])." ESCAPE '!'";
							}
							elseif ($prev_tag === $tag['tag']) {
								// Not the first value. Add next OR statement.
								$value .= ' OR UPPER(pt.value) LIKE'.zbx_dbstr($tag['value'])." ESCAPE '!'";
							}
				...
				$where .= 'EXISTS ('.
					...
					' WHERE p.eventid=pt.eventid'.
						' AND pt.tag='.zbx_dbstr($tag['tag']).$value.
				')';
			}
