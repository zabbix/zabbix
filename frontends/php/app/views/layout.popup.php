...
if (array_key_exists('only_hostid', $options)) {
	...
	if ($data['popup_type'] === 'help_items') {
		$cmb_types = new CComboBox('itemtype', $page_options['itemtype'], 'javascript: reloadPopup(this.form);');
	...
	if (str_in_array($data['popup_type'], $show_host_cmb_box)
			&& ($data['popup_type'] !== 'item_prototypes' || !$options['parent_discoveryid'])) {
		$controls[] = [
			new CLabel(_('Host'), 'hostid'),
			(new CDiv())->addClass(ZBX_STYLE_FORM_INPUT_MARGIN),
...
if (in_array($data['popup_type'], ['applications', 'triggers'])) {
	if (!array_key_exists('noempty', $options)) {
		...
		$epmty_script = get_window_opener($options['dstfrm'], $options['dstfld1'], $value1);
		$epmty_script .= get_window_opener($options['dstfrm'], $options['dstfld2'], $value2);
		$epmty_script .= get_window_opener($options['dstfrm'], $options['dstfld3'], $value3);
		$epmty_script .= ' overlayDialogueDestroy(jQuery(this).closest("[data-dialogueid]").attr("data-dialogueid"));';
		$epmty_script .= ' return false;';

		$controls[] = [(new CButton('empty', _('Empty')))->onClick($epmty_script)];
	}
}
