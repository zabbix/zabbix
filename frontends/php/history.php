...
COpt::profiling_stop('history');
	...
	if(!isset($_REQUEST['plaintext'])){
		...
		if(str_in_array($_REQUEST['action'],array('showvalues','showgraph'))){
			...
			$scroll_div = new CDiv();			$scroll_div->addOption('id','scroll_cntnr');			$scroll_div->addOption('style','border: 0px #CC0000 solid; height: 25px; width: 800px;');			$scroll_div->show();//		navigation_bar("history.php",$to_save_request);
		}
	}
?>
<?php
