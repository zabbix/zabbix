...
jQuery(function($) {
	...
	$.fn.serializeFieldsOnSubmit = function(options) {
		...
		this.each(function() {
			...
			form.submit(function() {
				...
				options.fields.each(function(fieldName) {
					var result = {};

					jQuery('[name^=' + fieldName + ']', form).each(function() {
						var input = jQuery(this);

						var value = input.val();

						var name = input.attr('name');
						var nameSegments = name.match(/\[.+?\]/g);

						// This loop builds object with structure defined in input name - value "333" of
						// input with name "asdf[bbb][55]" will go into {"bbb": {"55": "333"}}.
						var currentResultSegment = result;
						for (var i = 0; i != nameSegments.length; i++) {
							// Strip square brackets - make "aaa" from "[aaa]"
							var segmentName = decodeURIComponent(nameSegments[i].substr(1, nameSegments[i].length - 2));

							if (currentResultSegment[segmentName] == null) {
								// If this is last segment, set value to last segment,
								// otherwise add new object and make that a base for next iteration.
								if (i == nameSegments.length - 1) {
									currentResultSegment[segmentName] = value;
								}
								else {
									currentResultSegment[segmentName] = {};
									currentResultSegment = currentResultSegment[segmentName];
								}
							}
							else {
								// If this is last segment, set value to last segment,
								// otherwise take the existing object as base for next iteration
								if (i == nameSegments.length - 1) {
									currentResultSegment[segmentName] = value;
								}
								else {
						...
						// Set name of element to empty so that it does not go to POST.
						input.attr('name', null);
					});

					// Append input element with JSON to form
					jQuery('<input/>', {
						type: 'hidden',
						name: fieldName,
