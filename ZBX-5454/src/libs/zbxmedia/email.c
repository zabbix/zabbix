...
void str_base64_encode_rfc2047(const char *src, char **p_base64, int src_len)
{
	...
	int 	subj_alloc = 0;		/* allocated memory size for subject */
	int	subj_offset = 0;	/* offset for writing into subject */
	...
	zbx_snprintf_alloc(&subj, &subj_alloc, &subj_offset, 1, "");
	...
	for (p0 = src; p0 < src + src_len; p0 = p1)
	{
		...
		if ( 0 < p1 - p0)
		{
			...
			{
				zbx_snprintf_alloc(&subj, &subj_alloc, &subj_offset,
						ZBX_EMAIL_B64_MAXWORD_RFC2047 + word_sep_len + 1, "%s=?UTF-8?B?%s?=",
						word_sep, b64_buf);
			}
