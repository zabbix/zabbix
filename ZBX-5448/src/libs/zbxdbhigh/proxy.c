 ...
 *                                                                            *
 ******************************************************************************/
static int	proxy_get_history_data_simple(struct zbx_json *j, const zbx_history_table_t *ht, zbx_uint64_t *lastid, unsigned char * is_more_data_available)
{
...
try_again:
	...
	if(ZBX_MAX_HRECORDS == records)	/* we got full buffer, there might be more data */
	{
		*is_more_data_available = 1;
	}
	else
	{
		*is_more_data_available = 0;
	}
	DBfree_result(result);
 ...
 *                                                                            *
 ******************************************************************************/
static int	proxy_get_history_data(struct zbx_json *j, zbx_uint64_t *lastid, unsigned char * is_more_data_available)
{
...
try_again:
	...
	if(ZBX_MAX_HRECORDS == records ||	/* we got full buffer, there might be more data */
			i != records )	/* we could not fill whole buffer due to items skipped, there might be more data */
	{
		*is_more_data_available = 1;
	}
	else
...
int	proxy_get_hist_data(struct zbx_json *j, zbx_uint64_t *lastid, unsigned char * is_more_data_available)
{
	return proxy_get_history_data(j, lastid, is_more_data_available);
}

int	proxy_get_dhis_data(struct zbx_json *j, zbx_uint64_t *lastid, unsigned char * is_more_data_available)
{
	return proxy_get_history_data_simple(j, &dht, lastid, is_more_data_available);
}

int	proxy_get_areg_data(struct zbx_json *j, zbx_uint64_t *lastid, unsigned char * is_more_data_available)
{
	return proxy_get_history_data_simple(j, &areg, lastid, is_more_data_available);
}
