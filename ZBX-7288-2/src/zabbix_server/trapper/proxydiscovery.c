...
void	send_discovery_data(zbx_sock_t *sock)
{
	...
	{
		zabbix_log(LOG_LEVEL_WARNING, "error while sending discovery data: %s", zbx_tcp_strerror());
		goto out;
	}

	if (SUCCEED != zbx_recv_response_dyn(sock, &info, CONFIG_TIMEOUT))
	{
		zabbix_log(LOG_LEVEL_WARNING, "sending discovery data: negative response from server: %s",
				ZBX_NULL2STR(info));
		goto out;
	}
