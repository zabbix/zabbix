...
<?php
	...
	'S_SCRIPT_ERROR'=>			'Не удается подключиться к порту сервера Zabbix, который должен быть доступен для запуска скрипта.',
		'S_SCRIPT_ERROR_BIG'=>			'Возможные причины:
		1. Неверный IP-сервер / DNS в "zabbix.conf.php".
		2. Программа, отвечающая за безопасность, например SELinux, блокирует соединение.
		3. сервис Zabbix не запущен.',
	'S_NOT_RUN'=>' Сервер  @@server@@ или сервис Zabbix не запущен',
	'S_IP_NOT_AVAILABLE'=>' IP-адрес в той же локальной сети и не доступны',
	'S_TIME_OUT'=>' Правильное имя DNS сервера @@server@@, но недоступен за пределами локальной сети',
	'S_WRONG_DNS'=>' Неправильное имя DNS',
