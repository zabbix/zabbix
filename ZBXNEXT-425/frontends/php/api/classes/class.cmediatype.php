		...
		try{
			...
			foreach($mediatypes as $mnum => $mediatype){
				...
				if(!empty($mediatype_exist)){
					self::exception(ZBX_API_ERROR_PARAMETERS, sprintf(S_CMEDIATYPE_ERROR_MEDIATYPE_EXISTS, $mediatype_exist[0]['description']));
				}
	...
	public static function delete($mediatypeids){
		...
		try{
			...
			if(!empty($actions)){
				...
				self::exception(ZBX_API_ERROR_PARAMETERS, sprintf(S_CMEDIATYPE_ERROR_MEDIATYPE_USED_BY_ACTIONS, $action['name']));
			}
