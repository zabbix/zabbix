...
if ($this->data['config'] == ZBX_AUTH_LDAP) {
	...
	$authenticationFormList->addRow(
		_('LDAP host'),
		new CTextBox('ldap_host', $this->data['config_data']['ldap_host'], ZBX_TEXTBOX_STANDARD_SIZE)
	);
	$authenticationFormList->addRow(
		_('Port'),
		new CNumericBox('ldap_port', $this->data['config_data']['ldap_port'], 5)
	);
	$authenticationFormList->addRow(
		_('Base DN'),
		new CTextBox('ldap_base_dn', $this->data['config_data']['ldap_base_dn'], ZBX_TEXTBOX_STANDARD_SIZE)
	);
	$authenticationFormList->addRow(
		_('Search attribute'),
		new CTextBox(
			'ldap_search_attribute',
			(isset($this->data['config_data']['ldap_search_attribute']) && $this->data['config_data']['ldap_search_attribute'])
				? $this->data['config_data']['ldap_search_attribute']
				: 'uid',
	...
	$authenticationFormList->addRow(
		_('Bind DN'),
		new CTextBox('ldap_bind_dn', $this->data['config_data']['ldap_bind_dn'], ZBX_TEXTBOX_STANDARD_SIZE)
	);

	// bind password
	if (isset($this->data['change_bind_password'])) {
		$authenticationForm->addVar('change_bind_password', $this->data['change_bind_password']);
		$authenticationFormList->addRow(
			_('Bind password'),
...
elseif ($this->data['config'] != ZBX_AUTH_LDAP) {
