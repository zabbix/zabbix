...
class testToken extends CAPITest {
	...
	private function countAuditActions(int $action): int {
		return count(DB::select('auditlog', ['output' => [], 'filter' => [
				'resourcetype' => CAudit::RESOURCE_AUTH_TOKEN,
				'action' => $action
		]]));
	}

	public function testToken_auditlogs(): void {
		$add_records = $this->countAuditActions(CAudit::ACTION_ADD);
		$update_records = $this->countAuditActions(CAudit::ACTION_UPDATE);
		$delete_records = $this->countAuditActions(CAudit::ACTION_DELETE);

		['result' => ['tokenids' => [$new_id]]] = $this->call('token.create', ['name' => 'audit 1']);
		$this->assertEquals($add_records + 1, $this->countAuditActions(CAudit::ACTION_ADD));

		$this->call('token.update', ['tokenid' => $new_id, 'name' => 'audit 2']);
		$this->assertEquals($update_records + 1, $this->countAuditActions(CAudit::ACTION_UPDATE));

		$this->call('token.generate', [$new_id]);
		$this->assertEquals($update_records + 2, $this->countAuditActions(CAudit::ACTION_UPDATE));

		$this->call('token.delete', [$new_id]);
		$this->assertEquals($delete_records + 1, $this->countAuditActions(CAudit::ACTION_DELETE));
	}
