		...
		return [
			...
			[
				[
					'expected' => TEST_BAD,
					'proxy_fields' => [
						'Proxy name' => 'Empty proxy address',
						'Proxy mode' => 'Passive',
						'id:ip' => ''
					],
	...
	public function testFormAdministrationGeneralProxies_Create($data) {
		...
		if (CTestArrayHelper::get($data, 'expected', TEST_GOOD) === TEST_BAD) {
			...
			if (CTestArrayHelper::get($data, 'trim', false)) {
				$data['proxy_fields']['Proxy name'] = trim($data['proxy_fields']['Proxy name']);
			}
			...
			if (CTestArrayHelper::get($data, 'encryption_fields.PSK identity')) {
				$form->selectTab('Encryption');
				$this->assertTrue($form->query('button:Change PSK')->one()->isClickable());
			}
