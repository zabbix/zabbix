...
class testFormTrigger extends CLegacyWebTest {
	...
	public function testFormTrigger_CheckLayout($data) {
		...
		// Check hintbox.
		$this->query('class:zi-circle-question-filled')->one()->click();
		...
		if (isset($data['constructor'])) {
			...
			if (isset($constructor['errors']) && !array_key_exists('elementError', $constructor)) {
				...
				if (isset($constructor['elementError'])) {
					...
					$this->assertEquals($count, $this->query('xpath://a[@class="zi-i status-red"]')->all()->count());
					...
					$this->zbxTestAssertElementNotPresentXpath('//a[@class="zi-i status-red"]');
				}
			}
		}
