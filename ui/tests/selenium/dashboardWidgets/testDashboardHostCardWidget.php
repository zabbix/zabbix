	...
	*/
	...
	public function testDashboardHostCardWidget_Display($data) {
		...
		if (array_key_exists('Maintenance', $data)) {
			...
			$dialog_text =  $this->query('xpath://div[@class="overlay-dialogue hintbox wordbreak hintbox-static"]')->one()->getText();
			$this->assertEquals($data['Maintenance']['Name']."\n".$data['Maintenance']['Description'], $dialog_text);
			$this->query('xpath://div[@class="overlay-dialogue hintbox wordbreak hintbox-static"]/button[@title="Close"]')->one()->click();
		}
		...
		if (array_key_exists('Availability', $data)) {
			...
			foreach ($availabilities as $availability) {
				$availability->click();
				$dialog = $this->query('xpath://div[@class="overlay-dialogue hintbox wordbreak hintbox-static"]')->asOverlayDialog()
