...
class testDashboardTriggerOverviewWidget extends CWebTest {
	...
	private static $trigger_icons = [
		'2_trigger_Information' => 'icon-ackn', // todo zi
		'3_trigger_Average' => 'icon-ackn', // todo zi
		'4_trigger_Average' => 'icon-ackn', // todo zi
		'Dependent trigger ONE' => 'icon-depend-down', // todo zi
		'Inheritance trigger with tags' => 'icon-depend-down', // todo zi
		'Trigger disabled with tags' => 'icon-depend-up' // todo zi
	];
	...
	private function checkTriggerCell ($cell, $trigger) {
		...
		if (in_array($trigger, self::$trigger_icons)) {
			$element = (self::$trigger_icons[$trigger] === 'icon-ackn') ? 'span' : 'a'; // todo zi
