...
class testPageDashboard extends CLegacyWebTest {
	...
	public function testPageDashboard_CheckLayoutForDifferentUsers() {
		...
		foreach ($users as $user) {
			...
			if ($user != 'super-admin') {
				$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[7]//tr[@class='nothing-to-show']/td", 'No graphs added.');
				$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[6]//tr[@class='nothing-to-show']/td", 'No maps added.');
				$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[5]//tr[@class='nothing-to-show']/td", 'No data found.');
			}
			$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[7]//h4", 'Favourite graphs');
			$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[6]//h4", 'Favourite maps');
			$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[5]//h4", 'Problems');
			$this->zbxTestAssertElementPresentXpath("//div[@class='dashboard-grid-container']/div[4]//h4[text()='Problems by severity']");
			$this->zbxTestAssertElementPresentXpath("//div[@class='dashboard-grid-container']/div[3]//h4[text()='Local']");
			$this->zbxTestAssertElementPresentXpath("//div[@class='dashboard-grid-container']/div[2]//h4[text()='Host availability']");
			$this->zbxTestAssertElementText("//div[@class='dashboard-grid-container']/div[1]//h4", 'System information');
	...
	public function testPageDashboard_AddFavouriteGraphs() {
		...
		$this->zbxTestOpen('zabbix.php?action=dashboard.view');
		$this->zbxTestAssertElementText('//div[@class="dashboard-grid-container"]/div[7]//a[@href="zabbix.php?action=charts.view&view_as=showgraph&filter_search_type=0&filter_graphids%5B0%5D='.$this->graphCpuId.'&filter_set=1"]', 'ЗАББИКС Сервер: '.$this->graphCpu);
		$this->zbxTestAssertElementText('//div[@class="dashboard-grid-container"]/div[7]//a[@href="zabbix.php?action=charts.view&view_as=showgraph&filter_search_type=0&filter_graphids%5B0%5D='.$this->graphMemoryId.'&filter_set=1"]', 'ЗАББИКС Сервер: '.$this->graphMemory);
	...
	public function testPageDashboard_RemoveFavouriteMaps() {
		...
		foreach ($FavouriteScreens as $FavouriteScreen) {
			...
			$this->zbxTestWaitUntilElementNotVisible(WebDriverBy::xpath("//div[@class='dashboard-grid-container']/div[2]//button[@onclick=\"rm4favorites('sysmapid','".$FavouriteScreen['value_id']."')\"]"));
		}
