...
if (!defined('PHPUNIT_BASEDIR')) {
	...
	set_error_handler(function ($errno, $errstr, $errfile, $errline) {
		...
		if (error_reporting() & $errno) {
			var_dump('error 1 added at - '.date("H:i:s"));
			file_put_contents(PHPUNIT_ERROR_LOG, $errstr.' ['.formatCallStack()."]\n".'error 1 added at - '.date("H:i:s"), FILE_APPEND);
		}
	...
	set_exception_handler(function ($exception) {
		var_dump('error 2 added at - '.date("H:i:s"));
		file_put_contents(PHPUNIT_ERROR_LOG, $exception."\n".'error 2 added at - '.date("H:i:s"), FILE_APPEND);
	});
}
