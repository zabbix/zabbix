...
class testDataCollection extends CIntegrationTest {
	...
	/**
	 * Remove the certificate directories generated for the TLS tests.
	 */
	public static function clearCertificates(): void {
		if (!empty(self::$certBaseDirProxy) && is_dir(self::$certBaseDirProxy)) {
			shell_exec('rm -rf ' . escapeshellarg(self::$certBaseDirProxy));
		}

		if (!empty(self::$certBaseDirAgent) && is_dir(self::$certBaseDirAgent)) {
