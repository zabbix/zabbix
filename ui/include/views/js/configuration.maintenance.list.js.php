	...
	const view = new class {
		...
		init({csrf_tokens}) {
			...
			document.addEventListener('click', (e) => {
				if (e.target.classList.contains('js-massdelete-maintenance')) {
					if (!this.massDeleteMaintenance(e.target, Object.keys(chkbxRange.getSelectedIds()))) {
						e.preventDefault();
						e.stopPropagation();
						return false;
					};
		...
		massDeleteMaintenance(target, maintenanceids) {
			...
			create_var(target.closest('form'), '<?= CController::CSRF_TOKEN_NAME ?>',
				this.csrf_tokens['maintenance.massdelete'], false
			);
