...
class CHostPrototype extends CHostBase {
	...
	protected function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		...
		$sqlParts['where'][] = 'hd.lldrule_itemid=i.itemid';
		...
		if ($options['discoveryids'] !== null) {
			$sqlParts['where'][] = dbConditionInt('hd.lldrule_itemid', (array) $options['discoveryids']);

			if ($options['groupCount']) {
				$sqlParts['group']['hd'] = 'hd.lldrule_itemid';
			}
		}
	...
	private static function addInternalFields(array &$db_hosts): void {
		$result = DBselect(
			'SELECT h.hostid,h.templateid,hd.lldrule_itemid AS ruleid,hh.status AS host_status'.
	...
	public function unlinkTemplateObjects(array $ruleids): void {
		$result = DBselect(
			'SELECT hd.hostid,h.host,h.uuid,h.templateid,hd.lldrule_itemid AS ruleid,hh.status AS host_status'.
			' FROM host_discovery hd,hosts h,items i,hosts hh'.
			' WHERE hd.hostid=h.hostid'.
				' AND hd.lldrule_itemid=i.itemid'.
				' AND i.hostid=hh.hostid'.
				' AND '.dbConditionId('hd.lldrule_itemid', $ruleids)
		);
