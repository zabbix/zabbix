...
class CMediatype extends CApiService {
	...
	private static function validateSmtpAuthenticationOauthFields(array &$mediatype, ?array $db_mediatype,
		...
		foreach (['redirection_url', 'authorization_url', 'token_url'] as $url_field) {
			if (!array_key_exists($url_field, $mediatype)) {
				continue;
			}

			$url = parse_url($mediatype[$url_field]);

			if (is_array($url) && array_key_exists('scheme', $url)
					&& array_key_exists('host', $url) && $url['host'] !== '') {
				continue;
			}

			$error = _s('Invalid parameter "%1$s": %2$s.', $path.'/'.$url_field, _('unacceptable URL'));
			self::exception(ZBX_API_ERROR_PARAMETERS, $error);
		}
