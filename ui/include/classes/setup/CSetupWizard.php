...
class CSetupWizard extends CForm {
	...
	private function stageRequirements(): array {
		...
		if ($finalResult == CFrontendSetup::CHECK_FATAL) {
			$message_box = makeMessageBox(ZBX_STYLE_MSG_BAD.' zi-circle-info-filled', $messages, null, false, true);
		}
		else {
			$message_box = null;
		}
	...
	private function stageDbConnection(): array {
		...
		if ($this->step_failed) {
			$message_box = makeMessageBox(ZBX_STYLE_MSG_BAD.' zi-circle-info-filled', CMessageHelper::getMessages(),
	...
	private function stageInstall(): array {
		...
		if ($error) {
			...
			$message_box = makeMessageBox(ZBX_STYLE_MSG_BAD.' zi-circle-info-filled', $messages, _('Cannot create the configuration file.'),
				false, true
			);
			$message = [
