...
class C74XmlValidator extends CXmlValidatorGeneral {
	...
	public function getSchema() {
		return ['type' => XML_ARRAY, 'rules' => [
			...
			'host_groups' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'host_group', 'rules' => [
				'host_group' => ['type' => XML_ARRAY, 'rules' => $this->getHostGroupValidationRules()]
			]],
			'hosts' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'host', 'rules' => [
				'host' => ['type' => XML_ARRAY, 'rules' => $this->getHostValidationRules()]
			]],
			'template_groups' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'template_group', 'rules' => [
				'template_group' => ['type' => XML_ARRAY, 'rules' => $this->getHostGroupValidationRules()]
			]],
			'templates' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'template', 'rules' => [
				'template' => ['type' => XML_ARRAY, 'rules' => $this->getTemplateValidationRules()]
			]],
			'triggers' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'trigger', 'rules' => [
				'trigger' => ['type' => XML_ARRAY, 'rules' => $this->getTriggerValidationRules()]
			]],
			'graphs' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'graph', 'rules' => [
				'graph' => ['type' => XML_ARRAY, 'rules' => $this->getGraphValidationRules()]
			]],
			'images' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'image', 'rules' => [
				'image' => ['type' => XML_ARRAY, 'rules' => $this->getImageValidationRules()]
			]],
			'maps' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'map', 'rules' => [
				'map' => ['type' => XML_ARRAY, 'rules' => $this->getMapValidationRules()]
			]],
			'media_types' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'media_type', 'rules' => [
				'media_type' => ['type' => XML_ARRAY, 'rules' => $this->getMediaTypeValidationRules()]
			]]
		]];
	}
	...
	public function getWidgetFieldValueExtendedRules(array $data): array
	{
		switch ($data['type']) {
			...
			case CXmlConstantValue::DASHBOARD_WIDGET_FIELD_TYPE_ACTION:
			case CXmlConstantValue::DASHBOARD_WIDGET_FIELD_TYPE_MEDIA_TYPE:
				return ['type' => XML_ARRAY, 'rules' => [
					'name' => ['type' => XML_STRING | XML_REQUIRED]
				]];
			case CXmlConstantName::DASHBOARD_WIDGET_FIELD_TYPE_USER:
			case CXmlConstantValue::DASHBOARD_WIDGET_FIELD_TYPE_USER:
				return ['type' => XML_ARRAY, 'rules' => [
	...
	private function getHostValidationRules(): array
	{
		return [
			'host' => ['type' => XML_STRING | XML_REQUIRED],
			...
			'interfaces' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'interface', 'rules' => [
				'interface' => ['type' => XML_ARRAY, 'rules' => $this->getHostInterfaceValidationRules()]
			]],
			'items' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'item', 'rules' => [
				'item' => ['type' => XML_ARRAY, 'rules' => $this->getItemValidationRules()]
			]],
			'discovery_rules' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'discovery_rule', 'rules' => [
				'discovery_rule' => ['type' => XML_ARRAY, 'rules' => $this->getDiscoveryRuleValidationRules()]
			]],
			'httptests' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'httptest', 'rules' => [
				'httptest' => ['type' => XML_ARRAY, 'rules' => $this->getHttpTestValidationRules()]
			]],
			...
			'valuemaps' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'valuemap', 'rules' => [
				'valuemap' => ['type' => XML_ARRAY, 'rules' => $this->getValueMapValidationRules()]
			]]
		];

	}

	private function getHostGroupValidationRules(): array
	{
		return [
	...
	private function getTemplateValidationRules(): array
	{
		return [
			'uuid' => ['type' => XML_STRING | XML_REQUIRED, 'flags' => CImportDataNormalizer::LOWERCASE],
			...
			'items' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'item', 'rules' => [
				'item' => ['type' => XML_ARRAY, 'rules' => $this->getItemValidationRules(true)]
			]],
			'discovery_rules' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'discovery_rule', 'rules' => [
				'discovery_rule' => ['type' => XML_ARRAY, 'rules' => $this->getDiscoveryRuleValidationRules(true)]
			]],
			'httptests' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'httptest', 'rules' => [
				'httptest' => ['type' => XML_ARRAY, 'rules' => $this->getHttpTestValidationRules(true)]
			]],
			'tags' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'tag', 'rules' => [
				'tag' => ['type' => XML_ARRAY, 'rules' => [
					'tag' => ['type' => XML_STRING | XML_REQUIRED],
			...
			'dashboards' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'dashboard', 'rules' => [
				'dashboard' => ['type' => XML_ARRAY, 'rules' => $this->getDashboardValidationRules()]
			]],
			'valuemaps' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'valuemap', 'rules' => [
				'valuemap' => ['type' => XML_ARRAY, 'rules' => $this->getValueMapValidationRules(true)]
			]]
		];
	}

	private function getHostInterfaceValidationRules(): array
	{
		return [
	...
	private function getItemValidationRules(bool $is_template = false): array
	{
		if ($is_template) {
			return [
				'uuid' => ['type' => XML_STRING | XML_REQUIRED, 'flags' => CImportDataNormalizer::LOWERCASE],
				...
				'triggers' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'trigger', 'rules' => [
					'trigger' => ['type' => XML_ARRAY, 'rules' => $this->getTriggerValidationRules($is_template)]
				]]
			];
		}
		else {
	...
	private function getTriggerValidationRules(bool $is_template = false): array
	{
		$common_rules = [
			'expression' => ['type' => XML_STRING | XML_REQUIRED],
		...
		if ($is_template) {
			$template_rules = ['uuid' => ['type' => XML_STRING | XML_REQUIRED, 'flags' => CImportDataNormalizer::LOWERCASE]];

			return [$template_rules, ...$common_rules];
		}

		return $common_rules;
	}

	private function getDiscoveryRuleValidationRules(bool $is_template = false): array
	{
		if ($is_template) {
			return [
				'uuid' => ['type' => XML_STRING | XML_REQUIRED, 'flags' => CImportDataNormalizer::LOWERCASE],
					...
					'override' => ['type' => XML_ARRAY, 'rules' => [
						...
						'operations' => ['type' => XML_INDEXED_ARRAY, 'prefix' => 'operation', 'rules' => [
							'operation' => ['type' => XML_ARRAY, 'rules' => [
								'operationobject' => ['type' => XML_STRING, 'in' => $this->LLD_OVERRIDE_OPERATION_OBJECT, 'default' => CXmlConstantValue::LLD_OVERRIDE_OPERATION_OBJECT_ITEM_PROTOTYPE],
	...
	private function getDashboardValidationRules(): array
	{
		return [
			'uuid' => ['type' => XML_STRING | XML_REQUIRED, 'flags' => CImportDataNormalizer::LOWERCASE],
			'name' => ['type' => XML_STRING | XML_REQUIRED],
	...
	private function getValueMapValidationRules(bool $is_template = false): array
	{
		if ($is_template) {
			return [
				'uuid' => ['type' => XML_STRING | XML_REQUIRED, 'flags' => CImportDataNormalizer::LOWERCASE],
	...
	private function getHttpTestValidationRules(bool $is_template = false): array
	{
		$common_rules = [
			'name' => ['type' => XML_STRING | XML_REQUIRED],
	...
	private function getGraphValidationRules(): array
	{
		return [
	...
	private function getImageValidationRules(): array
	{
		return [
			'name' => ['type' => XML_STRING | XML_REQUIRED],
	...
	private function getMapValidationRules(): array
	{
		return [
			'name' => ['type' => XML_STRING | XML_REQUIRED],
			...
			'show_element_label' => ['type' => XML_STRING, 'default' => CXmlConstantValue::SHOW_LABEL_ALWAYS, 'in' => [CXmlConstantValue::SHOW_LABEL_AUTO_HIDE => CXmlConstantName::SHOW_LABEL_AUTO_HIDE, CXmlConstantValue::SHOW_LABEL_ALWAYS => CXmlConstantName::SHOW_LABEL_ALWAYS]],
			'show_link_label' => ['type' => XML_STRING, 'default' => CXmlConstantValue::SHOW_LABEL_ALWAYS, 'in' => [CXmlConstantValue::SHOW_LABEL_AUTO_HIDE => CXmlConstantName::SHOW_LABEL_AUTO_HIDE, CXmlConstantValue::SHOW_LABEL_ALWAYS => CXmlConstantName::SHOW_LABEL_ALWAYS]],
			...
			'selements' => ['type' => XML_INDEXED_ARRAY | XML_REQUIRED, 'prefix' => 'selement', 'rules' => [
				'selement' => ['type' => XML_ARRAY, 'rules' => $this->getMapSelementValidationRules()]
			]],
			'shapes' => ['type' => XML_INDEXED_ARRAY | XML_REQUIRED, 'prefix' => 'shape', 'rules' => [
				'shape' => ['type' => XML_ARRAY, 'rules' => $this->getMapShapeValidationRules()]
			]],
			'lines' => ['type' => XML_INDEXED_ARRAY | XML_REQUIRED, 'prefix' => 'line', 'rules' => [
				'line' => ['type' => XML_ARRAY, 'rules' => $this->getMapLineValidationRules()]
			]],
			'links' => ['type' => XML_INDEXED_ARRAY | XML_REQUIRED, 'prefix' => 'link', 'rules' => [
				'link' => ['type' => XML_ARRAY, 'rules' => $this->getMapLinkValidationRules()]
			]]
		];
	}
	private function getMapSelementValidationRules(): array
	{
		return [
	...
	private function getMapShapeValidationRules(): array
	{
		return [
			'type' => ['type' => XML_STRING | XML_REQUIRED],
			'x' => ['type' => XML_STRING | XML_REQUIRED],
	...
	private function getMapLineValidationRules(): array
	{
		return [
	...
	private function getMapLinkValidationRules(): array
	{
		return [
	...
	private function getMediaTypeValidationRules(): array
	{
		return [
			'name' => ['type' => XML_STRING | XML_REQUIRED],
