...
if ($data['ldap_config']['provision_status'] == JIT_PROVISIONING_ENABLED) {
	$formgrid
		->addItem([
			...
			(new CFormField((new CSpan(_('No match')))->addClass(ZBX_STYLE_DISABLED)))->setId('provisioning_role')
		])
		->addItem([
			new CLabel(_('User groups')),
			(new CFormField((new CSpan(_('No match')))->addClass(ZBX_STYLE_DISABLED)))->setId('provisioning_groups')
		])
		->addItem([
			new CLabel(_('Media type')),
			(new CFormField((new CSpan(_('No match')))->addClass(ZBX_STYLE_DISABLED)))->setId('provisioning_medias')
		]);
}
