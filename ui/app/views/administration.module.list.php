...
$html_page = (new CHtmlPage())
	...
	->setControls(
		(new CTag('nav', true,
			(new CForm())
				->addItem((new CVar(CCsrfTokenHelper::CSRF_TOKEN_NAME, CCsrfTokenHelper::getCsrfToken('module.scan')))
					->removeId()
				)
...
foreach ($data['modules'] as $moduleid => $module) {
	...
	if ($module['status'] == MODULE_STATUS_ENABLED) {
		$status = (new CLink(_('Enabled'), (new CUrl($status_url))->getUrl()))
			->addCsrfToken(CCsrfTokenHelper::getCsrfToken('module.disable'))
			->addClass(ZBX_STYLE_LINK_ACTION)
			->addClass(ZBX_STYLE_GREEN);
	}
	else {
		$status = (new CLink(_('Disabled'), (new CUrl($status_url))->getUrl()))
			->addCsrfToken(CCsrfTokenHelper::getCsrfToken('module.enable'))
			->addClass(ZBX_STYLE_LINK_ACTION)
...
$form->addItem([
	...
	new CActionButtonList('action', 'moduleids', [
		'module.enable' => ['name' => _('Enable'), 'confirm' => _('Enable selected modules?'),
			'csrf_token' => CCsrfTokenHelper::getCsrfToken('module.enable')
		],
		'module.disable' => ['name' => _('Disable'), 'confirm' => _('Disable selected modules?'),
			'csrf_token' => CCsrfTokenHelper::getCsrfToken('module.disable')
		]
	], 'modules')
]);
