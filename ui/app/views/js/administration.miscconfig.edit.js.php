...
const view = new class {
	init({rules, default_values}) {
		this.form_element = document.getElementById('miscconfig-form');
	...
	#initEvents() {
		...
		document.getElementById('validate_uri_schemes').addEventListener('change', (e) => {
			document.getElementById('uri_valid_schemes').disabled = !e.target.checked;
		});

		document.getElementById('x_frame_header_enabled').addEventListener('change', (e) => {
			document.getElementById('x_frame_options').disabled = !e.target.checked;
		});

		document.getElementById('iframe_sandboxing_enabled').addEventListener('change', (e) => {
			document.getElementById('iframe_sandboxing_exceptions').disabled = !e.target.checked;
		});
						...
						Object.entries(this.default_values).forEach(([key, value]) => {
							...
							if (input) {
								if (input.classList.contains('multiselect')) {
									$(input).multiSelect('clean');
								}
								else if (input.classList.contains('radio-list-control')) {
									input.querySelector(`input[value="${value}"]`).checked = true;
								}
	...
	#submit(e) {
		...
		this.form.validateSubmit(fields)
			.then((result) => {
				...
				curl.setArgument('action', 'miscconfig.update');
