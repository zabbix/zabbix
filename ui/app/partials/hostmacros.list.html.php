...
foreach ($data['macros'] as $i => $macro) {
	$macro_cell = [
		...
		new CVar('macros['.$i.'][discovery_state]', $macro['discovery_state'])
	];
	...
	$macro_value = (new CMacroValue($macro['type'], 'macros['.$i.']', null, false))
		->setReadonly(!($macro['discovery_state'] & CControllerHostMacrosList::DISCOVERY_STATE_CONVERTING));
	...
	$table->addRow([
		...
		new CCol(new CHorList($action_column))
	], 'form_row');
}
