...
class CControllerLldRuleEdit extends CController
{
	/**
	 * @var array
	 */
	...
	protected function checkPermissions(): bool {
		...
		if ($this->hasInput('itemid')) {
			$lldrules = API::DiscoveryRule()->get([
	...
	public function doAction(): void {
		...
		if (!$lldrule['discovered_lld'] &&
				$this->getInput('context') === 'host' && !($host['flags'] & ZBX_FLAG_DISCOVERY_CREATED)) {
		...
		$data = [
			...
			'preprocessing_types' => CDiscoveryRule::SUPPORTED_PREPROCESSING_TYPES,
			'preprocessing_test_type' => CControllerPopupItemTestEdit::ZBX_TEST_TYPE_LLD,
			'lldrule' => $lldrule,
		...
		$data['js_validation_rules'] = $lldrule['itemid']
			? (new CFormValidator(CControllerLldRuleUpdate::getValidationRules()))->getRules()
			: (new CFormValidator(CControllerLldRuleCreate::getValidationRules()))->getRules();
