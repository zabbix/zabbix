...
class CControllerItemUpdate extends CController {
	...
	protected function checkPermissions(): bool {
		return CWebUser::getType() >= USER_TYPE_ZABBIX_ADMIN
			&& API::Host()->get(['hostids' => $this->getInput('hostid')])
			&& API::Item()->get(['itemids' => $this->getInput('itemid')]);
	}
	...
	protected function getFormData(): array {
		...
		if ($this->getInput('type') == ITEM_TYPE_HTTPAGENT) {
			$item['authtype'] = $item['http_authtype'];
			$item['username'] = $item['http_username'];
			$item['password'] = $item['http_password'];
		}
		...
		return getSanitizedItemFields($item + $db_item);
	}
}
