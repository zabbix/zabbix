...
class CControllerHostList extends CController {
	...
	protected function checkInput(): bool {
		$fields = [
			...
			'filter_dns'          => 'string',
			'filter_port'         => 'string',
			'filter_status'       => 'in '.HOST_STATUS_ANY.','.HOST_STATUS_MONITORED.','.HOST_STATUS_NOT_MONITORED,
	...
	protected function doAction(): void {
		if ($this->hasInput('filter_set')) {
			...
			CProfile::update('web.hosts.filter_status', $this->getInput('filter_status', HOST_STATUS_ANY),
				PROFILE_TYPE_INT
			);
		...
		$filter = [
			...
			'status' => CProfile::get('web.hosts.filter_status', HOST_STATUS_ANY),
		...
		$hosts = API::Host()->get([
			...
			'filter' => [
				...
				'status' => ($filter['status'] == HOST_STATUS_ANY) ? null : $filter['status']
			],
			'proxyids' => $proxyids
		]);
