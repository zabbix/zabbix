...
class CControllerHostPrototypeUpdate extends CController {

	private $host_prototype = [];
	...
	protected function checkInput(): bool {
		...
		if (!$ret) {
			...
			$response = $form_errors
				...
				: ['error' => [
					'title' => _('Cannot update host prototype'),
	...
	protected function checkPermissions(): bool {
		...
		$host_prototype = API::HostPrototype()->get([
			'output' => ['hostid', 'host', 'templateid', 'status', 'discover'],
		...
		$this->host_prototype = $host_prototype[0];
	...
	protected function doAction(): void {
		...
		$data = [
			'host' => $this->getInput('host', $this->host_prototype['host']),
			...
			'status' => $this->getInput('status', $this->host_prototype['status']),
			'discover' => $this->getInput('discover', $this->host_prototype['discover']),
		...
		$host_prototype = ['hostid' => $this->getInput('hostid')] + getSanitizedHostPrototypeFields(
			['templateid' => $this->host_prototype['templateid']] + $data
		);

		$result = API::HostPrototype()->update($host_prototype);

		if ($result) {
			$output['success']['title'] = _('Host prototype updated');
