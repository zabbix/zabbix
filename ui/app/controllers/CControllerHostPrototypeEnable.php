...
class CControllerHostPrototypeEnable extends CController {
	...
	protected function checkInput(): bool {
		$fields = [
			...
			'discover' =>	'db hosts.discover|in '.ZBX_PROTOTYPE_DISCOVER,
			'status' =>		'db hosts.status|in '.HOST_STATUS_MONITORED
		];
	...
	protected function doAction(): void {
		...
		if ($this->hasInput('status')) {
			foreach ($host_prototypeids as $host_prototypeid) {
				$host_prototypes[] = ['hostid' => $host_prototypeid, 'status' => HOST_STATUS_MONITORED];
			}
		}
		else {
			foreach ($host_prototypeids as $host_prototypeid) {
				$host_prototypes[] = ['hostid' => $host_prototypeid, 'discover' => ZBX_PROTOTYPE_DISCOVER];
			}
		}
