<script type="text/x-jquery-tmpl" id="dcheckRowTPL">
	<tr id="dcheckRow_#{dcheckid}">
		...
		<td>
			<input type="button" class="input link_menu" name="remove" value=<?php echo CJs::encodeJson(('Remove')); ?> onclick="javascript: removeDCheckRow(#{dcheckid});" />
		</td>
	</tr>
</script>
...
<script type="text/x-jquery-tmpl" id="newDCheckTPL">
	<div id="new_check_form">
		<div class="objectgroup inlineblock border_dotted ui-corner-all">
			...
			<input type="button" id="add_new_dcheck" name="add_new_dcheck" value=<?php echo CJs::encodeJson(('Add')); ?>
				class="input button link_menu">
			&nbsp;&nbsp;
			<input type="button" id="cancel_new_dcheck" name="cancel_new_dcheck" value=<?php echo CJs::encodeJson(('Cancel')); ?>
				class="input button link_menu">
		</div>
	</div>
</script>
<script type="text/javascript">
	...
	function discoveryCheckTypeToString(svcPort) {
		var defPorts = {};
		defPorts[ZBX_SVC.ftp] = <?php echo CJs::encodeJson(('FTP')); ?>;
		defPorts[ZBX_SVC.http] = <?php echo CJs::encodeJson(('HTTP')); ?>;
		defPorts[ZBX_SVC.https] = <?php echo CJs::encodeJson(('HTTPS')); ?>;
		defPorts[ZBX_SVC.icmp] = <?php echo CJs::encodeJson(('ICMP ping')); ?>;
		defPorts[ZBX_SVC.imap] = <?php echo CJs::encodeJson(('IMAP')); ?>;
		defPorts[ZBX_SVC.tcp] = <?php echo CJs::encodeJson(('TCP')); ?>;
		defPorts[ZBX_SVC.ldap] = <?php echo CJs::encodeJson(('LDAP')); ?>;
		defPorts[ZBX_SVC.nntp] = <?php echo CJs::encodeJson(('NNTP')); ?>;
		defPorts[ZBX_SVC.pop] = <?php echo CJs::encodeJson(('POP')); ?>;
		defPorts[ZBX_SVC.snmpv1] = <?php echo CJs::encodeJson(('SNMPv1 agent')); ?>;
		defPorts[ZBX_SVC.snmpv2] = <?php echo CJs::encodeJson(('SNMPv2 agent')); ?>;
		defPorts[ZBX_SVC.snmpv3] = <?php echo CJs::encodeJson(('SNMPv3 agent')); ?>;
		defPorts[ZBX_SVC.smtp] = <?php echo CJs::encodeJson(('SMTP')); ?>;
		defPorts[ZBX_SVC.ssh] = <?php echo CJs::encodeJson(('SSH')); ?>;
		defPorts[ZBX_SVC.telnet] = <?php echo CJs::encodeJson(('Telnet')); ?>;
		defPorts[ZBX_SVC.agent] = <?php echo CJs::encodeJson(('Zabbix agent')); ?>;

		if (typeof(svcPort) == 'undefined') {
			return defPorts;
		}
		svcPort = parseInt(svcPort, 10);
		return isset(svcPort, defPorts) ? defPorts[svcPort] : <?php echo CJs::encodeJson(('Unknown')); ?>;
	}
	...
	function updateNewDCheckType(e) {
		...
		if (isset(dcheckType, keyRowTypes)) {
			var caption = (dcheckType == ZBX_SVC.agent) ? <?php echo CJs::encodeJson(('Key')); ?> : <?php echo CJs::encodeJson(('SNMP OID')); ?>;
	...
	function saveNewDCheckForm(e) {
		...
		for (var dcheckid in ZBX_CHECKLIST) {
			if ((typeof dCheck['key_'] == 'undefined' || ZBX_CHECKLIST[dcheckid]['key_'] === dCheck['key_'])
				...
				alert(<?php echo CJs::encodeJson(('Check already exists.')); ?>);
				return null;
			}
		}
		...
		switch (parseInt(dCheck.type, 10)) {
			...
			case ZBX_SVC.snmpv2:
				if (dCheck.snmp_community == '') {
					validationErrors.push(<?php echo CJs::encodeJson(('Incorrect SNMP community.')); ?>);
				}
			case ZBX_SVC.snmpv3:
				if (dCheck.key_ == '') {
					validationErrors.push(<?php echo CJs::encodeJson(('Incorrect SNMP OID.')); ?>);
				}
				break;
		}
