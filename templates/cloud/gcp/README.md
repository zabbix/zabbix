...
|User execution request latency|<p>Distribution of request latencies in milliseconds measured from when the request reaches a running instance to when it exits. Only includes time spent in user container.</p>|Dependent item|gcp.run.user_execution.request.latency<p>**Preprocessing**</p><ul><li><p>JSON Path: `$.user_execution_percentile`</p><p>⛔️Custom on fail: Discard value</p></li><li><p>Custom multiplier: `0.001`</p></li></ul>|
