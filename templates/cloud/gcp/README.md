...
2. Create the service account in Google Cloud Console for the project you have to monitor.
...
5. Copy the `project_id`, `private_key_id`, `private_key`, `client_email` from the JSON key file and add them  to the corresponding macros `{$GCP.PROJECT.ID}`, `{$GCP.PRIVATE.KEY.ID}`, `{$GCP.PRIVATE.KEY}`, `{$GCP.CLIENT.EMAIL}` on the template/host.
