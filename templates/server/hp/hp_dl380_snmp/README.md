...
|HPE ProLiant DL380: Host has been restarted|<p>The uptime counter has decreased, which indicates that the host has been restarted.<br>A decrease is ignored if the previous value was close enough to the 32-bit SNMP counter limit (2^32 hundredths of a second, about 497 days) for the counter to have wrapped between the two readings.<br>The problem is resolved automatically once uptime exceeds {$SNMP.UPTIME.WARN}.</p>|`(last(/HPE ProLiant DL380 by SNMP/hp.server.hw.uptime[hrSystemUptime])>0 and change(/HPE ProLiant DL380 by SNMP/hp.server.hw.uptime[hrSystemUptime])<0 and last(/HPE ProLiant DL380 by SNMP/hp.server.hw.uptime[hrSystemUptime],#2)<42949672.96-(lastclock(/HPE ProLiant DL380 by SNMP/hp.server.hw.uptime[hrSystemUptime])-lastclock(/HPE ProLiant DL380 by SNMP/hp.server.hw.uptime[hrSystemUptime],#2))) or (last(/HPE ProLiant DL380 by SNMP/hp.server.hw.uptime[hrSystemUptime])=0 and change(/HPE ProLiant DL380 by SNMP/hp.server.net.uptime[sysUpTime])<0 and last(/HPE ProLiant DL380 by SNMP/hp.server.net.uptime[sysUpTime],#2)<42949672.96-(lastclock(/HPE ProLiant DL380 by SNMP/hp.server.net.uptime[sysUpTime])-lastclock(/HPE ProLiant DL380 by SNMP/hp.server.net.uptime[sysUpTime],#2)))`|Warning|**Manual close**: Yes|
