...
All metrics are collected at once, thanks to Zabbix's bulk data collection. The template is available in Zabbix version 5.0.  
...
|----|-----------|----|----|
|Discrete sensors discovery|<p>Discovery the discrete IPMI sensors.</p>|DEPENDENT|ipmi.discrete.discovery<p>**Filter**:</p>AND <p>- A: {#SENSOR_READING_TYPE} NOT_MATCHES_REGEX `threshold`</p><p>- B: {#SENSOR_TYPE} MATCHES_REGEX `{$IPMI.SENSOR_TYPE.MATCHES}`</p><p>- C: {#SENSOR_TYPE} NOT_MATCHES_REGEX `{$IPMI.SENSOR_TYPE.NOT_MATCHES}`</p>|
|Threshold sensors discovery|<p>Discovery the threshold IPMI sensors.</p>|DEPENDENT|ipmi.sensors.discovery<p>**Filter**:</p>AND <p>- A: {#SENSOR_READING_TYPE} MATCHES_REGEX `threshold`</p><p>- B: {#SENSOR_TYPE} MATCHES_REGEX `{$IPMI.SENSOR_TYPE.MATCHES}`</p><p>- C: {#SENSOR_TYPE} NOT_MATCHES_REGEX `{$IPMI.SENSOR_TYPE.NOT_MATCHES}`</p>|
...
|----|-----------|----|----|----|
|IPMI: {#SENSOR_ID} value has changed|<p>The trigger is informing about changing a state of the discrete IPMI sensor. A problem of this trigger can be manually closed.</p>|`{TEMPLATE_NAME:ipmi.state_text[{#SENSOR_ID}].diff()}=1`|INFO|<p>Manual close: YES</p>|
|IPMI: {#SENSOR_ID} value is non-critical low (under {#SENSOR_LO_WARN} for 5m)|<p>The trigger is informing that a value less than the lower non-critical threshold has been reached.</p>|`{TEMPLATE_NAME:ipmi.value[{#SENSOR_ID}].min(5m)}<{#SENSOR_LO_WARN}`|WARNING|<p>**Depends on**:</p><p>- IPMI: {#SENSOR_ID} value is critical low (under {#SENSOR_LO_CRIT} for 5m)</p><p>- IPMI: {#SENSOR_ID} value is non-recoverable low (under {#SENSOR_LO_DISAST} for 5m)</p>|
|IPMI: {#SENSOR_ID} value is critical low (under {#SENSOR_LO_CRIT} for 5m)|<p>The trigger is informing that a value less than the lower critical threshold has been reached.</p>|`{TEMPLATE_NAME:ipmi.value[{#SENSOR_ID}].min(5m)}<{#SENSOR_LO_CRIT}`|HIGH|<p>**Depends on**:</p><p>- IPMI: {#SENSOR_ID} value is non-recoverable low (under {#SENSOR_LO_DISAST} for 5m)</p>|
|IPMI: {#SENSOR_ID} value is non-recoverable low (under {#SENSOR_LO_DISAST} for 5m)|<p>The trigger is informing that a value less than the lower non-recoverable threshold has been reached.</p>|`{TEMPLATE_NAME:ipmi.value[{#SENSOR_ID}].min(5m)}<{#SENSOR_LO_DISAST}`|DISASTER||
|IPMI: {#SENSOR_ID} value is non-critical high (over {#SENSOR_HI_WARN} for 5m)|<p>The trigger is informing that a value more than the upper non-critical threshold has been reached.</p>|`{TEMPLATE_NAME:ipmi.value[{#SENSOR_ID}].min(5m)}>{#SENSOR_HI_WARN}`|WARNING|<p>**Depends on**:</p><p>- IPMI: {#SENSOR_ID} value is critical high (over {#SENSOR_HI_CRIT} for 5m)</p><p>- IPMI: {#SENSOR_ID} value is non-recoverable high (over {#SENSOR_HI_DISAST} for 5m)</p>|
|IPMI: {#SENSOR_ID} value is critical high (over {#SENSOR_HI_CRIT} for 5m)|<p>The trigger is informing that a value more than the upper critical threshold has been reached.</p>|`{TEMPLATE_NAME:ipmi.value[{#SENSOR_ID}].min(5m)}>{#SENSOR_HI_CRIT}`|HIGH|<p>**Depends on**:</p><p>- IPMI: {#SENSOR_ID} value is non-recoverable high (over {#SENSOR_HI_DISAST} for 5m)</p>|
|IPMI: {#SENSOR_ID} value is non-recoverable high (over {#SENSOR_HI_DISAST} for 5m)|<p>The trigger is informing that a value more than the upper non-recoverable threshold has been reached.</p>|`{TEMPLATE_NAME:ipmi.value[{#SENSOR_ID}].min(5m)}>{#SENSOR_HI_DISAST}`|DISASTER||
