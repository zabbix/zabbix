...
|HAProxy {#PXNAME} {#SVNAME}: Health check error |<p>Please check the server for faults.</p> |`{TEMPLATE_NAME:haproxy.server.check_status[{#PXNAME},{#SVNAME}].regexp("(?:L[4-7]OK|^$)",#3)} = 0` |WARNING |<p>**Depends on**:</p><p>- HAProxy {#PXNAME} {#SVNAME}: Server is DOWN</p> |
