...
|TrueNAS: System name has changed |<p>The name of the system has changed. `Ack` to close the problem manually.</p> |`change(/TrueNAS by SNMP/system.name) and length(last(/TrueNAS by SNMP/system.name))>0` |INFO |<p>Manual close: YES</p> |
