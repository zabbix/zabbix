...
|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Device [{#NAME}]:[{#ID}]: Raw data resource|<p>Gets resource information for the device.</p>|Dependent item|acronis.cpc.device.res.raw[{#NAME}]<p>**Preprocessing**</p><ul><li><p>JSON Path: `$.[?(@.name == "{#NAME}")].first()`</p><p>⛔️Custom on fail: Set error to: `Could not parse resource data`</p></li></ul>|
|Device [{#NAME}]:[{#ID}]: Raw data resources status|<p>Gets statuses for device resources.</p>|HTTP agent|acronis.cpc.device.res.status.raw[{#NAME}]<p>**Preprocessing**</p><ul><li><p>JSON Path: `$.items`</p><p>⛔️Custom on fail: Set error to: `Could not parse resource status data`</p></li></ul>|
...
|Device [{#NAME}]:[{#ID}]: Protection status|<p>Protection status for device.</p>|Dependent item|acronis.cpc.device.protection.status[{#NAME}]<p>**Preprocessing**</p><ul><li><p>JSON Path: `$..aggregate.status.first()`</p><p>⛔️Custom on fail: Set error to: `Could not parse protection status`</p></li><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|
