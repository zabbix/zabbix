...
|{$AZURE.STORAGE.LOCATION.MATCHES} |<p>This macro is used in storage accounts discovery rule.</p> |`.*` |
|{$AZURE.STORAGE.LOCATION.NOT_MATCHES} |<p>This macro is used in storage accounts discovery rule.</p> |`CHANGE_IF_NEEDED` |
|{$AZURE.STORAGE.NAME.MATCHES} |<p>This macro is used in storage accounts discovery rule.</p> |`.*` |
|{$AZURE.STORAGE.NAME.NOT_MATCHES} |<p>This macro is used in storage accounts discovery rule.</p> |`CHANGE_IF_NEEDED` |
...
|Azure |Azure: Outbound flows |<p>Outbound Flows are a number of the current flows in the outbound direction (traffic going out of the VM).</p> |DEPENDENT |azure.vm.flows.outbound<p>**Preprocessing**:</p><p>- JSONPATH: `$.metrics.OutboundFlows.average`</p><p>⛔️ON_FAIL: `DISCARD_VALUE -> `</p> |
