...
The template to monitor Etcd cluster by Zabbix that work without any external scripts.
Most of the metrics are collected in one go, thanks to Zabbix bulk data collection.

`Template App Etcd` — collects metrics by HTTP agent from /metric endpoint.
https://etcd.io/docs/v3.4.0/op-guide/monitoring/#metrics-endpoint
...
You can set {$ETCD.USERNAME} and {$ETCD.PASSWORD} macros in the template for using on the host level.
...
|{$ETCD.SCHEME} |<p>-</p> |`http` |
|{$ETCD.USER} |<p>-</p> |`` |
|{$GRPC.CODE.MATCHES} |<p>Filter of discoverable grpc codes</p> |`.*` |
|{$GRPC.CODE.NOT_MATCHES} |<p>Filter to exclude discovered grpc codes</p> |`CHANGE_IF_NEEDED` |
...
|-----|----|-----------|----|---------------------|
|Etcd |Etcd: Service ping |<p>-</p> |SIMPLE |net.tcp.service["{$ETCD.SCHEME}","{HOST.CONN}","{$ETCD.PORT}"]<p>**Preprocessing**:</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `10m`</p> |
|Etcd |Etcd: Node health |<p>-</p> |HTTP_AGENT |etcd.node.health<p>**Preprocessing**:</p><p>- JSONPATH: `$.health`</p><p>- BOOL_TO_DECIMAL<p>- DISCARD_UNCHANGED_HEARTBEAT: `10m`</p> |
|Etcd |Etcd: Server is leader |<p>Whether or not this member is a leader. 1 if is, 0 otherwise.</p> |DEPENDENT |etcd.is.leader<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_server_is_leader `</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> 0`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `10m`</p> |
|Etcd |Etcd: Server has leader |<p>Whether or not a leader exists. 1 is existence, 0 is not.</p> |DEPENDENT |etcd.has.leader<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_server_has_leader `</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `10m`</p> |
...
|Etcd |Etcd: Maximum open file descriptors |<p>Maximum number of open file descriptors.</p> |DEPENDENT |etcd.max.fds<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `process_max_fds `</p> |
...
|Etcd |Etcd: Transaction per second |<p>The number of transaction seen by this member per second.</p> |DEPENDENT |etcd.txn.rate<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_debugging_mvcc_range_total `</p><p>- CHANGE_PER_SECOND |
...
|Etcd |Etcd: Bytes sent to {#GRPC.PEER} |<p>The number of bytes sent to peer with ID {#GRPC.PEER}</p> |DEPENDENT |etcd.bytes.sent.rate[{#GRPC.PEER}]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_network_peer_sent_bytes_total{To="{#GRPC.PEER}"} `</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> 0`</p><p>- CHANGE_PER_SECOND |
|Etcd |Etcd: Bytes received from {#GRPC.PEER} |<p>The number of bytes received from peer with ID {#GRPC.PEER}</p> |DEPENDENT |etcd.bytes.received.rate[{#GRPC.PEER}]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_network_peer_received_bytes_total{From="{#GRPC.PEER}"} `</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> 0`</p><p>- CHANGE_PER_SECOND |
|Etcd |Etcd: Send failures from {#GRPC.PEER} |<p>The number of send failures from peer with ID {#GRPC.PEER}</p> |DEPENDENT |etcd.sent.fail.rate[{#GRPC.PEER}]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_network_peer_sent_failures_total{To="{#GRPC.PEER}"} `</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> 0`</p><p>- CHANGE_PER_SECOND |
|Etcd |Etcd: Receive failures failures to {#GRPC.PEER} |<p>The number of receive failures from the peer with ID {#GRPC.PEER}</p> |DEPENDENT |etcd.received.fail.rate[{#GRPC.PEER}]<p>**Preprocessing**:</p><p>- PROMETHEUS_PATTERN: `etcd_network_peer_received_failures_total{To="{#GRPC.PEER}"} `</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> 0`</p><p>- CHANGE_PER_SECOND |
|Zabbix_raw_items |Etcd: Get node metrics |<p>-</p> |HTTP_AGENT |etcd.metrics |
|Zabbix_raw_items |Etcd: Get version |<p>-</p> |HTTP_AGENT |etcd.version |

## Triggers

|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----|----|----|
|Etcd: Service is down |<p>-</p> |`{TEMPLATE_NAME:net.tcp.service["{$ETCD.SCHEME}","{HOST.CONN}","{$ETCD.PORT}"].last()}=0` |AVERAGE |<p>Manual close: YES</p> |
|Etcd: Node healthcheck failed |<p>https://etcd.io/docs/v3.4.0/op-guide/monitoring/#health-check</p> |`{TEMPLATE_NAME:etcd.node.health.last()}=0` |AVERAGE |<p>**Depends on**:</p><p>- Etcd: Service is down</p> |
...
|Etcd: Current number of open file is too high (over {$ETCD.OPEN.FDS.MAX.WARN}% in 5m) |<p>"Heavy file descriptor usage (i.e., near the process’s file descriptor limit) indicates a potential file descriptor exhaustion issue. </p><p>If the file descriptors are exhausted, etcd may panic because it cannot create new WAL files."</p> |`{TEMPLATE_NAME:etcd.open.fds.min(5m)}/{Template App Etcd by HTTP:etcd.max.fds.last()}*100>{$ETCD.OPEN.FDS.MAX.WARN}` |WARNING | |
|Etcd: Failed to fetch info data (or no data for 30m) |<p>Zabbix has not received data for items for the last 30 minutes</p> |`{TEMPLATE_NAME:etcd.metrics.nodata(30m)}=1` |WARNING |<p>Manual close: YES</p><p>**Depends on**:</p><p>- Etcd: Service is down</p> |
