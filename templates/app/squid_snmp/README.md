...
### Setup Zabbix
1\. [Import](https://www.zabbix.com/documentation/5.2/manual/xml_export_import/templates) the template [template_app_squid_snmp.yaml[template_app_squid_snmp.yaml] into Zabbix.

2\. Set values for {$SQUID.SNMP.COMMUNITY}, {$SQUID.SNMP.PORT} and {$SQUID.HTTP.PORT} as configured in squid.conf.

3\. [Link](https://www.zabbix.com/documentation/5.2/manual/config/templates/linking) the imported template to a host with Squid.
