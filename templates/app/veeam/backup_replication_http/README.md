...
|----|-----------|-------|
|{$CRAEATED.AFTER} |<p>Returns sessions that are created after choosed days.</p> |`7` |
...
|{$VEEAM.API.URL} |<p>Veem API endpoint URL in the format <scheme>://<host>:<port>.</p> |`https://localhost:9419/api` |
...
|Proxies discovery |<p>Discovery Porxies.</p> |DEPENDENT |veeam.proxies.discovery<p>**Preprocessing**:</p><p>- JSONPATH: `$.proxies.data`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `6h`</p><p>**Filter**:</p>AND <p>- {#TYPE} MATCHES_REGEX `{$PROXIES.TYPE.MATCHES}`</p><p>- {#TYPE} NOT_MATCHES_REGEX `{$PROXIES.TYPE.NOT_MATCHES}`</p><p>- {#NAME} MATCHES_REGEX `{$PROXIES.NAME.MATCHES}`</p><p>- {#NAME} NOT_MATCHES_REGEX `{$PROXIES.NAME.NOT_MATCHES}`</p> |
...
|Veeam |Veeam: Get errors |<p>A list of errors from API requests.</p> |DEPENDENT |veeam.get.errors<p>**Preprocessing**:</p><p>- JSONPATH: `$.errors`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `1h`</p> |
|Veeam |Veeam proxies: Get server data by [{#NAME}] |<p>Get proxie server raw data.</p> |DEPENDENT |veeam.proxy.server.raw[{#NAME}]<p>**Preprocessing**:</p><p>- JSONPATH: `$.managedServers.data.[?(@.id=='{#HOSTID}')].first()`</p> |
|Veeam |Veeam proxies: Get data [{#NAME}] [{#TYPE}] |<p>Get proxie [{#NAME}] [{#TYPE}] raw data.</p> |DEPENDENT |veeam.proxy.raw[{#NAME}]<p>**Preprocessing**:</p><p>- JSONPATH: `$.proxies.data.[?(@.id=='{#ID}')].first()`</p> |
...
|Veeam |Veeam repositories: Used space [{#NAME}] [{#HOSTNAME}] [{#PATH}] |<p>Repository used space in GB.</p> |DEPENDENT |veeam.repository.capasity[{#NAME}]<p>**Preprocessing**:</p><p>- JSONPATH: `$.usedSpaceGB`</p> |
