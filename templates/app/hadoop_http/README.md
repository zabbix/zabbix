...
|ResourceManager: Cluster has the lost NodeManagers |<p>Cluster lost some NodeManagers.</p> |`{TEMPLATE_NAME:hadoop.resourcemanager.num_lost_nm.min(5m)}>0` |WARNING |<p>**Depends on**:</p><p>- ResourceManager: Cluster has no active NodeManagers</p> |
...
|{#HOSTNAME}: Failed to fetch NodeManager API page (or no data for 30m) |<p>Zabbix has not received data for items for the last 30 minutes.</p> |`{TEMPLATE_NAME:hadoop.nodemanager.state[{#HOSTNAME}].nodata(30m)}=1` |WARNING |<p>Manual close: YES</p><p>**Depends on**:</p><p>- {#HOSTNAME}: NodeManager has state {ITEM.VALUE}.</p> |
...
|{#HOSTNAME}: Failed to fetch DataNode API page (or no data for 30m) |<p>Zabbix has not received data for items for the last 30 minutes.</p> |`{TEMPLATE_NAME:hadoop.datanode.oper_state[{#HOSTNAME}].nodata(30m)}=1` |WARNING |<p>Manual close: YES</p><p>**Depends on**:</p><p>- {#HOSTNAME}: DataNode has state {ITEM.VALUE}.</p> |
