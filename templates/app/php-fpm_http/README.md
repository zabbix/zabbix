...
`Template App PHP-FPM by HTTP` collects metrics by polling PHP-FPM status-page with HTTP agent remotely.
...
|{$PHP_FPM.STATUS.PAGE} |<p>The path of PHP-FPM ping page.</p> |`status` |
...
|PHP-FPM |PHP-FPM: Version |<p>Current version PHP. Get from HTTP-Header "X-Powered-By" and may not work if you change default HTTP-headers.</p> |DEPENDENT |php-fpm.version<p>**Preprocessing**:</p><p>- REGEX: `^[.\s\S]*X-Powered-By: PHP/([.\d]{1,}) \1`</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> Unknown`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `3h`</p> |
