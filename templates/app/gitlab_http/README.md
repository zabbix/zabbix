...
|----|-----------|----|----|
|Unicorn metrics discovery |<p>DiscoveryUnicorn specific metrics, when Unicorn is used. </p> |DEPENDENT |gitlab.unicorn.discovery |
|Puma metrics discovery |<p>Discovery Puma specific metrics, when Puma is used.</p> |DEPENDENT |gitlab.puma.discovery |
|Readiness probes discovery |<p>Discovery of all services readiness probe checks. (Database, Redis, Gitaly etc.)</p> |DEPENDENT |gitlab.readiness.discovery |
...
|GitLab |GitLab: Redis: Client requests rate, queues |<p>Number of Redis client requests per second. (Instance: queues)</p> |DEPENDENT |gitlab.redis.client_requests.queues.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="gitlab_redis_client_requests_total" && @.labels.storage == "queues")].value.first()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: Redis: Client requests rate, cache |<p>Number of Redis client requests per second. (Instance: cache)</p> |DEPENDENT |gitlab.redis.client_requests.cache.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="gitlab_redis_client_requests_total" && @.labels.storage == "cache")].value.first()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: Redis: Client requests rate, shared_state |<p>Number of Redis client requests per second. (Instance: shared_state)</p> |DEPENDENT |gitlab.redis.client_requests.shared_state.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="gitlab_redis_client_requests_total" && @.labels.storage == "shared_state")].value.first()`</p><p>- CHANGE_PER_SECOND |
...
|GitLab |GitLab: Cache: Misses, total  |<p>The cache read miss count.</p> |DEPENDENT |gitlab.cache.misses_total.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="gitlab_cache_misses_total")].value.sum()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: Cache: Operations, total  |<p>The count of cache operations.</p> |DEPENDENT |gitlab.cache.operations_total.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="gitlab_cache_operations_total")].value.sum()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: Ruby: CPU |<p>Average CPU time util in seconds.</p> |DEPENDENT |gitlab.ruby.process_cpu_seconds.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="ruby_process_cpu_seconds_total")].value.avg()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: Ruby: Running_threads |<p>Number of running Ruby threads.</p> |DEPENDENT |gitlab.ruby.threads_running<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="gitlab_ruby_threads_running_threads")].value.sum()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: Ruby: File descriptors openned, avg |<p>Average number of openned file descriptors.</p> |DEPENDENT |gitlab.ruby.file_descriptors.avg<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="ruby_file_descriptors")].value.avg()`</p> |
|GitLab |GitLab: Ruby: File descriptors openned, max |<p>Maximum number of openned file descriptors.</p> |DEPENDENT |gitlab.ruby.file_descriptors.max<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="ruby_file_descriptors")].value.max()`</p> |
|GitLab |GitLab: Ruby: File descriptors openned, min |<p>Minimum number of openned file descriptors.</p> |DEPENDENT |gitlab.ruby.file_descriptors.min<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="ruby_file_descriptors")].value.min()`</p> |
...
|GitLab |GitLab: HTTP requests, total |<p>Number of requests received into the system.</p> |DEPENDENT |gitlab.http.requests.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="http_requests_total")].value.sum()`</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: HTTP requests, 5xx |<p>Number of handle failures of requests with HTTP-code 5xx.</p> |DEPENDENT |gitlab.http.requests.5xx.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="http_requests_total" && @.labels.status =~ '5..' )].value.sum()`</p><p>⛔️ON_FAIL: `DISCARD_VALUE -> `</p><p>- CHANGE_PER_SECOND |
|GitLab |GitLab: HTTP requests, 4xx |<p>Number of handle failures of requests with code 4XX.</p> |DEPENDENT |gitlab.http.requests.4xx.rate<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="http_requests_total" && @.labels.status =~ '4..' )].value.sum()`</p><p>⛔️ON_FAIL: `DISCARD_VALUE -> `</p><p>- CHANGE_PER_SECOND |
...
|GitLab Healthchecks |GitLab: Application server status |<p>Checks whether the application server is running. This probe is used to know if Rails Controllers are not deadlocked due to a multi-threading.</p> |HTTP_AGENT |gitlab.liveness<p>**Preprocessing**:</p><p>- JSONPATH: `$.status`</p><p>- BOOL_TO_DECIMAL<p>- DISCARD_UNCHANGED_HEARTBEAT: `30m`</p> |
|GitLab Healthchecks |GitLab: Readiness check status |<p>Status of readiness probe check.</p> |DEPENDENT |gitlab.readiness.status<p>**Preprocessing**:</p><p>- JSONPATH: `$.status`</p><p>- BOOL_TO_DECIMAL<p>- DISCARD_UNCHANGED_HEARTBEAT: `30m`</p> |
|GitLab Healthchecks |GitLab: Healthcheck component [{#GITLAB.COMPONENT}] |<p>Status of GitLab instance service.</p> |DEPENDENT |gitlab.healthcheck[{#GITLAB.COMPONENT}]<p>**Preprocessing**:</p><p>- JSONPATH: `$.{#GITLAB.COMPONENT}..status.first()`</p><p>- BOOL_TO_DECIMAL<p>- DISCARD_UNCHANGED_HEARTBEAT: `10m`</p> |
...
|Zabbix_raw_items |GitLab: Instance status check |<p>The readiness probe checks whether the GitLab instance is ready to accept traffic via Rails Controllers. Also, the check will validate the dependent services (Database, Redis, Gitaly etc.) and gives a status for each.</p> |HTTP_AGENT |gitlab.readiness |
|Zabbix_raw_items |GitLab: Check Unicorn | |DEPENDENT |gitlab.unicorn.get<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="unicorn_workers")].value.sum()`</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> []`</p><p>- JAVASCRIPT: `return JSON.stringify(value != "[]" ? [{'{#SINGLETON}': ''}] : []);`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `3h`</p> |
|Zabbix_raw_items |GitLab: Check Puma | |DEPENDENT |gitlab.puma.get<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.name=="puma_workers")].value.sum()`</p><p>⛔️ON_FAIL: `CUSTOM_VALUE -> []`</p><p>- JAVASCRIPT: `return JSON.stringify(value != "[]" ? [{'{#SINGLETON}': ''}] : []);`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `3h`</p> |
|Zabbix_raw_items |GitLab: Check Readness | |DEPENDENT |gitlab.readiness.get<p>**Preprocessing**:</p><p>- JAVASCRIPT: `Text is too long. Please see the template.`</p><p>- DISCARD_UNCHANGED_HEARTBEAT: `3h`</p> |
...
|GitLab: Gitlab instance is not able to accept traffic |<p>-</p> |`{TEMPLATE_NAME:gitlab.readiness.status.last()}=0` |HIGH |<p>**Depends on**:</p><p>- GitLab: Liveness check was failed</p> |
|GitLab: {#GITLAB.COMPONENT} healthcheck check failed |<p>The readiness probe checks was failed.</p> |`{TEMPLATE_NAME:gitlab.healthcheck[{#GITLAB.COMPONENT}].last()}=0` |HIGH | |
