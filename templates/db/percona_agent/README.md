...
# Percona by Zabbix agent

## Overview

This template is designed for the effortless deployment of Percona monitoring by Zabbix via Zabbix agent and doesn't require any external scripts.
...
|----|-----------|----|-----------------------|
|Get status variables|<p>Gets server global status information.</p>|Zabbix agent|percona.get_status_variables["{$PERCONA.HOST}","{$PERCONA.PORT}"]|
|Get global variables|<p>Gets server global variables for thread pool configuration.</p>|Zabbix agent|percona.get_global_variables["{$PERCONA.HOST}","{$PERCONA.PORT}"]|
|Status|<p>Percona server status.</p>|Zabbix agent|percona.ping["{$PERCONA.HOST}","{$PERCONA.PORT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `return value.indexOf('is alive') !== -1 ? 1 : 0;`</p></li><li><p>Discard unchanged with heartbeat: `10m`</p></li></ul>|
|Version|<p>Percona server version.</p>|Zabbix agent|percona.version["{$PERCONA.HOST}","{$PERCONA.PORT}"]<p>**Preprocessing**</p><ul><li><p>Regular expression: `(Server version)\s+(.+) \2`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|
...
|Buffer pool efficiency|<p>Shows how effectively the buffer pool is serving reads.</p>|Calculated|percona.buffer_pool_efficiency|
...
|InnoDB row lock time|<p>The total time spent in acquiring row locks for InnoDB tables.</p>|Dependent item|percona.innodb_row_lock_time<p>**Preprocessing**</p><ul><li><p>XML XPath: `The text is too long. Please see the template.`</p></li><li><p>Custom multiplier: `0.001`</p></li><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|
|InnoDB row lock wait time avg|<p>Average time spent waiting for row locks in InnoDB.</p>|Dependent item|percona.innodb_row_lock_time_avg<p>**Preprocessing**</p><ul><li><p>XML XPath: `The text is too long. Please see the template.`</p></li><li><p>Custom multiplier: `0.001`</p></li><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|
|InnoDB row lock time max|<p>The maximum time to acquire a row lock for InnoDB tables.</p>|Dependent item|percona.innodb_row_lock_time_max<p>**Preprocessing**</p><ul><li><p>XML XPath: `The text is too long. Please see the template.`</p></li><li><p>Custom multiplier: `0.001`</p></li><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|
...
|InnoDB buffer pool wait free|<p>Number of times InnoDB waited for a free page before reading or creating a page.</p><p>Normally, writes to the InnoDB buffer pool happen in the background.</p><p>When no clean pages are available, dirty pages are flushed first</p><p>in order to free some up. This counts the number of waits for this operation</p><p>to finish. If this value is not small, consider increasing `innodb_buffer_pool_size`.</p>|Dependent item|percona.innodb_buffer_pool_wait_free<p>**Preprocessing**</p><ul><li><p>XML XPath: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
...
|----|-----------|----------|--------|--------------------------------|
|Percona: Service is unreachable|<p>Percona is unreachable.</p>|`last(/Percona by Zabbix agent/percona.ping["{$PERCONA.HOST}","{$PERCONA.PORT}"])=0`|High||
|Percona: Version has changed|<p>The Percona version has changed. Acknowledge to close the problem manually.</p>|`last(/Percona by Zabbix agent/percona.version["{$PERCONA.HOST}","{$PERCONA.PORT}"],#1)<>last(/Percona by Zabbix agent/percona.version["{$PERCONA.HOST}","{$PERCONA.PORT}"],#2) and length(last(/Percona by Zabbix agent/percona.version["{$PERCONA.HOST}","{$PERCONA.PORT}"]))>0`|Info|**Manual close**: Yes|
|Percona: Service has been restarted|<p>Percona uptime is less than 10 minutes.</p>|`last(/Percona by Zabbix agent/percona.uptime)<10m`|Info||
|Percona: Failed to fetch data|<p>Zabbix has not received any data for the last 30 minutes.</p>|`nodata(/Percona by Zabbix agent/percona.uptime,30m)=1`|Info|**Depends on**:<br><ul><li>Percona: Service is unreachable</li></ul>|
|Percona: Server has aborted connections|<p>The number of failed attempts to connect to the Percona server has been more than `{$PERCONA.ABORTED_CONN.MAX.WARN}` in the last 5 minutes.</p>|`min(/Percona by Zabbix agent/percona.aborted_connects.rate,5m)>{$PERCONA.ABORTED_CONN.MAX.WARN}`|Average|**Depends on**:<br><ul><li>Percona: Refused connections</li></ul>|
|Percona: Refused connections|<p>Number of refused connections due to the `max_connections` limit being reached.</p>|`last(/Percona by Zabbix agent/percona.connection_errors_max_connections.rate)>0`|Average||
|Percona: Buffer pool utilization is too low|<p>The buffer pool utilization has been less than `{$PERCONA.BUFF_UTIL.MIN.WARN}`% in the last 5 minutes with server uptime over 1 hour. This means that there is a lot of unused RAM allocated for the buffer pool, which you can easily reallocate at the moment.</p>|`max(/Percona by Zabbix agent/percona.buffer_pool_utilization,5m)<{$PERCONA.BUFF_UTIL.MIN.WARN} and last(/Percona by Zabbix agent/percona.uptime)>3600`|Warning||
|Percona: Number of temporary files created per second is high|<p>The application using the database may be in need of query optimization.</p>|`min(/Percona by Zabbix agent/percona.created_tmp_files.rate,5m)>{$PERCONA.CREATED_TMP_FILES.MAX.WARN}`|Warning||
|Percona: Number of on-disk temporary tables created per second is high|<p>The application using the database may be in need of query optimization.</p>|`min(/Percona by Zabbix agent/percona.created_tmp_disk_tables.rate,5m)>{$PERCONA.CREATED_TMP_DISK_TABLES.MAX.WARN}`|Warning||
|Percona: Number of internal temporary tables created per second is high|<p>The application using the database may be in need of query optimization.</p>|`min(/Percona by Zabbix agent/percona.created_tmp_tables.rate,5m)>{$PERCONA.CREATED_TMP_TABLES.MAX.WARN}`|Warning||
|Percona: InnoDB efficiency is too low|<p>The InnoDB efficiency has been less than `{$PERCONA.INNODB_EFFICIENCY.MIN.CRIT}`% in the last 5 minutes.</p>|`max(/Percona by Zabbix agent/percona.innodb_scan.efficiency,5m)<{$PERCONA.INNODB_EFFICIENCY.MIN.CRIT} and last(/Percona by Zabbix agent/percona.innodb_total_scans.rate)>{$PERCONA.INNODB_SCANS.MIN.CRIT}`|High||
|Percona: InnoDB efficiency is low|<p>The InnoDB efficiency has been less than `{$PERCONA.INNODB_EFFICIENCY.MIN.WARN}`% in the last 5 minutes.</p>|`max(/Percona by Zabbix agent/percona.innodb_scan.efficiency,5m)<{$PERCONA.INNODB_EFFICIENCY.MIN.WARN} and last(/Percona by Zabbix agent/percona.innodb_total_scans.rate)>{$PERCONA.INNODB_SCANS.MIN.WARN}`|Warning|**Depends on**:<br><ul><li>Percona: InnoDB efficiency is too low</li></ul>|
|Percona: Server has slow queries|<p>The number of slow queries has been more than `{$PERCONA.SLOW_QUERIES.MAX.WARN}` in the last 5 minutes.</p>|`min(/Percona by Zabbix agent/percona.slow_queries.rate,5m)>{$PERCONA.SLOW_QUERIES.MAX.WARN}`|Warning||
|Percona: High rate of full table scans|<p>The rate of `SELECT` statements exceeds `{$PERCONA.FULLSCAN.SELECT_RATE.MIN.WARN}` per second and the full table scan ratio has been above `{$PERCONA.FULLSCAN.RATIO.WARN}` for the last 5 minutes.</p>|`avg(/Percona by Zabbix agent/percona.full_scans.ratio,5m)>{$PERCONA.FULLSCAN.RATIO.WARN} and avg(/Percona by Zabbix agent/percona.com_select.rate,5m)>{$PERCONA.FULLSCAN.SELECT_RATE.MIN.WARN}`|Warning||

### LLD rule Database discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database discovery|<p>Used for the discovery of databases.</p>|Zabbix agent|percona.db.discovery["{$PERCONA.HOST}","{$PERCONA.PORT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Database discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DBNAME}]: Size|<p>Database size.</p>|Zabbix agent|percona.dbsize["{$PERCONA.HOST}","{$PERCONA.PORT}","{#DBNAME}"]<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|

### LLD rule Database [{#DBNAME}]: Table discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DBNAME}]: Table discovery|<p>Used for the discovery of tables of the databases.</p>|Zabbix agent|percona.table.discovery["{$PERCONA.HOST}","{$PERCONA.PORT}","{#DBNAME}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Database [{#DBNAME}]: Table discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Table [{#DBNAME}/{#TABLE_NAME}]: Get data|<p>Database table data.</p>|Zabbix agent|percona.tabledata["{$PERCONA.HOST}","{$PERCONA.PORT}","{#DBNAME}","{#TABLE_NAME}"]|
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|Percona: Table [{#DBNAME}/{#TABLE_NAME}]: Total size growth is too high|<p>Total table size growth is too high.</p>|`(last(/Percona by Zabbix agent/percona.table.total_size["{#DBNAME}","{#TABLE_NAME}"])-min(/Percona by Zabbix agent/percona.table.total_size["{#DBNAME}","{#TABLE_NAME}"],5m))>{$PERCONA.TOTALSIZE.GROWTH.MAX.CRIT}`|High||
|Percona: Table [{#DBNAME}/{#TABLE_NAME}]: Total size growth is high|<p>Total table size growth is high.</p>|`(last(/Percona by Zabbix agent/percona.table.total_size["{#DBNAME}","{#TABLE_NAME}"])-min(/Percona by Zabbix agent/percona.table.total_size["{#DBNAME}","{#TABLE_NAME}"],5m))>{$PERCONA.TOTALSIZE.GROWTH.MAX.WARN}`|Warning|**Depends on**:<br><ul><li>Percona: Table [{#DBNAME}/{#TABLE_NAME}]: Total size growth is too high</li></ul>|
|Percona: Table [{#DBNAME}/{#TABLE_NAME}]: Free space very high|<p>Free table space is very high (fragmentation critical).</p>|`last(/Percona by Zabbix agent/percona.table.free_ratio["{#DBNAME}","{#TABLE_NAME}"]) > {$PERCONA.FRAGMENTATION.CRIT} and last(/Percona by Zabbix agent/percona.table.data_free["{#DBNAME}","{#TABLE_NAME}"]) > {$PERCONA.DATA_FREE.MIN.CRIT} and last(/Percona by Zabbix agent/percona.table.total_size["{#DBNAME}","{#TABLE_NAME}"]) > {$PERCONA.TABLE_SIZE.MIN.CRIT}`|High||
|Percona: Table [{#DBNAME}/{#TABLE_NAME}]: Free space high|<p>Free table space is high (fragmentation warning).</p>|`last(/Percona by Zabbix agent/percona.table.free_ratio["{#DBNAME}","{#TABLE_NAME}"]) > {$PERCONA.FRAGMENTATION.WARN} and last(/Percona by Zabbix agent/percona.table.data_free["{#DBNAME}","{#TABLE_NAME}"]) > {$PERCONA.DATA_FREE.MIN.WARN} and last(/Percona by Zabbix agent/percona.table.total_size["{#DBNAME}","{#TABLE_NAME}"]) > {$PERCONA.TABLE_SIZE.MIN.WARN}`|Warning|**Depends on**:<br><ul><li>Percona: Table [{#DBNAME}/{#TABLE_NAME}]: Free space very high</li></ul>|

### LLD rule Replication discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Replication discovery|<p>Discovery of the replication.</p>|Zabbix agent|percona.replication.discovery["{$PERCONA.HOST}","{$PERCONA.PORT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Replication discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Replication: Replica status {#MASTER_HOST}|<p>Gets status information on the essential parameters of the replica threads.</p>|Zabbix agent|percona.slave_status["{$PERCONA.HOST}","{$PERCONA.PORT}","{#SQL_STATEMENT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li></ul>|
|Replication: Replica SQL running state {#MASTER_HOST}|<p>Shows the state of the SQL thread.</p>|Dependent item|percona.slave_sql_running_state["{#MASTER_HOST}"]<p>**Preprocessing**</p><ul><li><p>XML XPath: `/resultset/row/field[@name='Slave_SQL_Running_State']/text()`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|Percona: Replication lag is too high|<p>Replication delay is too long.</p>|`min(/Percona by Zabbix agent/percona.seconds_behind_master["{#MASTER_HOST}"],5m)>{$PERCONA.REPL_LAG.MAX.WARN}`|Warning||
|Percona: The replica I/O thread is not running|<p>Indicates whether the I/O thread for reading the source's binary log is running.</p>|`last(/Percona by Zabbix agent/percona.slave_io_running["{#MASTER_HOST}"])=2`|Average||
|Percona: The replica I/O thread is not connected to a replication source|<p>Indicates whether the replica I/O thread is connected to the source.</p>|`last(/Percona by Zabbix agent/percona.slave_io_running["{#MASTER_HOST}"])<>1`|Warning|**Depends on**:<br><ul><li>Percona: The replica I/O thread is not running</li></ul>|
|Percona: The SQL thread is not running|<p>Indicates whether the SQL thread for executing events in the relay log is running.</p>|`last(/Percona by Zabbix agent/percona.slave_sql_running["{#MASTER_HOST}"])=2`|Warning|**Depends on**:<br><ul><li>Percona: The replica I/O thread is not running</li></ul>|
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|Percona: Thread pool max threads reached|<p>Thread pool has reached the configured max thread limit.</p>|`last(/Percona by Zabbix agent/percona.threadpool_threads[{#SINGLETON}])>=last(/Percona by Zabbix agent/percona.thread_pool_max_threads[{#SINGLETON}]) and last(/Percona by Zabbix agent/percona.thread_pool_max_threads[{#SINGLETON}])>0`|High||
|Percona: Thread pool utilization is too high|<p>Thread pool utilization has been above `{$PERCONA.THREADPOOL.UTILIZATION.MAX.WARN}`% in the last 5 minutes.</p>|`min(/Percona by Zabbix agent/percona.thread_pool_utilization[{#SINGLETON}],5m)>{$PERCONA.THREADPOOL.UTILIZATION.MAX.WARN} and last(/Percona by Zabbix agent/percona.thread_pool_max_threads[{#SINGLETON}])>0`|Warning||
|Percona: Thread pool idle ratio is too low|<p>Thread pool idle ratio has been less than `{$PERCONA.THREADPOOL.IDLE_RATIO.MIN.WARN}`% in the last 5 minutes.</p>|`max(/Percona by Zabbix agent/percona.thread_pool_idle_ratio[{#SINGLETON}],5m)<{$PERCONA.THREADPOOL.IDLE_RATIO.MIN.WARN} and last(/Percona by Zabbix agent/percona.threadpool_threads[{#SINGLETON}])>0`|Warning||
