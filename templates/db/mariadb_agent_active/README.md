...
|Version|<p>MariaDB server version.</p>|Zabbix agent (active)|mariadb.version["{$MARIADB.HOST}","{$MARIADB.PORT}"]<p>**Preprocessing**</p><ul><li><p>Regular expression: `(Server version)\s+(.+) \2`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
