...
|MySQL |MySQL: Binlog commits |<p>Total number of transactions committed to the binary log.</p> |DEPENDENT |mysql.binlog_commits.rate[{#SINGLETON}]<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.Variable_name=='Binlog_commits')].Value.first()`</p><p>- CHANGE_PER_SECOND |
|MySQL |MySQL: Binlog group commits |<p>Total number of group commits done to the binary log.</p> |DEPENDENT |mysql.binlog_group_commits.rate[{#SINGLETON}]<p>**Preprocessing**:</p><p>- JSONPATH: `$[?(@.Variable_name=='Binlog_group_commits')].Value.first()`</p><p>- CHANGE_PER_SECOND |
