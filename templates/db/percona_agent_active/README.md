...
|Version|<p>Percona server version.</p>|Zabbix agent (active)|percona.version["{$PERCONA.HOST}","{$PERCONA.PORT}"]<p>**Preprocessing**</p><ul><li><p>Regular expression: `(Server version)\s+(.+) \2`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
