...
|Version|<p>MySQL server version.</p>|Zabbix agent|mysql.version["{$MYSQL.HOST}","{$MYSQL.PORT}"]<p>**Preprocessing**</p><ul><li><p>Regular expression: `(Server version)\s+(.+) \2`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
...
|----|-----------|----------|--------|--------------------------------|
|MySQL: Service is unreachable|<p>MySQL is unreachable.</p>|`last(/MySQL by Zabbix agent/mysql.ping["{$MYSQL.HOST}","{$MYSQL.PORT}"])=0`|High||
...
|MySQL: Failed to fetch info data|<p>Zabbix has not received any data for items for the last 30 minutes.</p>|`nodata(/MySQL by Zabbix agent/mysql.uptime,30m)=1`|Info|**Depends on**:<br><ul><li>MySQL: Service is unreachable</li></ul>|
|MySQL: Server has aborted connections|<p>The number of failed attempts to connect to the MySQL server has been more than `{$MYSQL.ABORTED_CONN.MAX.WARN}` in the last 5 minutes.</p>|`min(/MySQL by Zabbix agent/mysql.aborted_connects.rate,5m)>{$MYSQL.ABORTED_CONN.MAX.WARN}`|Average|**Depends on**:<br><ul><li>MySQL: Refused connections</li></ul>|
...
|MySQL: Server has slow queries|<p>The number of slow queries has been more than `{$MYSQL.SLOW_QUERIES.MAX.WARN}` in the last 5 minutes.</p>|`min(/MySQL by Zabbix agent/mysql.slow_queries.rate,5m)>{$MYSQL.SLOW_QUERIES.MAX.WARN}`|Warning||
|MySQL: High rate of full table scans|<p>The rate of `SELECT` statements exceeds `{$MYSQL.FULLSCAN.SELECT_RATE.MIN.WARN}` per second and the full table scan ratio has been above `{$MYSQL.FULLSCAN.RATIO.WARN}` for the last 5 minutes.</p>|`avg(/MySQL by Zabbix agent/mysql.full_scans.ratio,5m)>{$MYSQL.FULLSCAN.RATIO.WARN} and avg(/MySQL by Zabbix agent/mysql.com_select.rate,5m)>{$MYSQL.FULLSCAN.SELECT_RATE.MIN.WARN}`|Warning||

### LLD rule Database discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database discovery|<p>Used for the discovery of databases.</p>|Zabbix agent|mysql.db.discovery["{$MYSQL.HOST}","{$MYSQL.PORT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Database discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DBNAME}]: Size|<p>Database size.</p>|Zabbix agent|mysql.dbsize["{$MYSQL.HOST}","{$MYSQL.PORT}","{#DBNAME}"]<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|

### LLD rule Database [{#DBNAME}]: Table discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DBNAME}]: Table discovery|<p>Used for the discovery of tables of the databases.</p>|Zabbix agent|mysql.table.discovery["{$MYSQL.HOST}","{$MYSQL.PORT}","{#DBNAME}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Database [{#DBNAME}]: Table discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Table [{#DBNAME}/{#TABLE_NAME}]: Get data|<p>Database table data.</p>|Zabbix agent|mysql.tabledata["{$MYSQL.HOST}","{$MYSQL.PORT}","{#DBNAME}","{#TABLE_NAME}"]|
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|MySQL: Table [{#DBNAME}/{#TABLE_NAME}]: Total size growth is too high|<p>Total table size growth is too high.</p>|`(last(/MySQL by Zabbix agent/mysql.table.total_size["{#DBNAME}","{#TABLE_NAME}"])-min(/MySQL by Zabbix agent/mysql.table.total_size["{#DBNAME}","{#TABLE_NAME}"],5m))>{$MYSQL.TOTALSIZE.GROWTH.MAX.CRIT}`|High||
|MySQL: Table [{#DBNAME}/{#TABLE_NAME}]: Total size growth is high|<p>Total table size growth is high.</p>|`(last(/MySQL by Zabbix agent/mysql.table.total_size["{#DBNAME}","{#TABLE_NAME}"])-min(/MySQL by Zabbix agent/mysql.table.total_size["{#DBNAME}","{#TABLE_NAME}"],5m))>{$MYSQL.TOTALSIZE.GROWTH.MAX.WARN}`|Warning|**Depends on**:<br><ul><li>MySQL: Table [{#DBNAME}/{#TABLE_NAME}]: Total size growth is too high</li></ul>|
|MySQL: Table [{#DBNAME}/{#TABLE_NAME}]: Free space very high|<p>Free table space is very high (fragmentation critical).</p>|`last(/MySQL by Zabbix agent/mysql.table.free_ratio["{#DBNAME}","{#TABLE_NAME}"]) > {$MYSQL.FRAGMENTATION.CRIT} and last(/MySQL by Zabbix agent/mysql.table.data_free["{#DBNAME}","{#TABLE_NAME}"]) > {$MYSQL.DATA_FREE.MIN.CRIT} and last(/MySQL by Zabbix agent/mysql.table.total_size["{#DBNAME}","{#TABLE_NAME}"]) > {$MYSQL.TABLE_SIZE.MIN.CRIT}`|High||
|MySQL: Table [{#DBNAME}/{#TABLE_NAME}]: Free space high|<p>Free table space is high (fragmentation warning).</p>|`last(/MySQL by Zabbix agent/mysql.table.free_ratio["{#DBNAME}","{#TABLE_NAME}"]) > {$MYSQL.FRAGMENTATION.WARN} and last(/MySQL by Zabbix agent/mysql.table.data_free["{#DBNAME}","{#TABLE_NAME}"]) > {$MYSQL.DATA_FREE.MIN.WARN} and last(/MySQL by Zabbix agent/mysql.table.total_size["{#DBNAME}","{#TABLE_NAME}"]) > {$MYSQL.TABLE_SIZE.MIN.WARN}`|Warning|**Depends on**:<br><ul><li>MySQL: Table [{#DBNAME}/{#TABLE_NAME}]: Free space very high</li></ul>|

### LLD rule Replication discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Replication discovery|<p>Discovery of the replication.</p>|Zabbix agent|mysql.replication.discovery["{$MYSQL.HOST}","{$MYSQL.PORT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Replication discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Replication: Replica status {#MASTER_HOST}|<p>Gets status information on the essential parameters of the replica threads.</p>|Zabbix agent|mysql.slave_status["{$MYSQL.HOST}","{$MYSQL.PORT}","{#SQL_STATEMENT}"]<p>**Preprocessing**</p><ul><li><p>JavaScript: `The text is too long. Please see the template.`</p></li></ul>|
...
|MySQL: The replica I/O thread is not running|<p>Indicates whether the I/O thread for reading the source's binary log is running.</p>|`last(/MySQL by Zabbix agent/mysql.slave_io_running["{#MASTER_HOST}"])=2`|Average||
|MySQL: The replica I/O thread is not connected to a replication source|<p>Indicates whether the replica I/O thread is connected to the source.</p>|`last(/MySQL by Zabbix agent/mysql.slave_io_running["{#MASTER_HOST}"])<>1`|Warning|**Depends on**:<br><ul><li>MySQL: The replica I/O thread is not running</li></ul>|
|MySQL: The SQL thread is not running|<p>Indicates whether the SQL thread for executing events in the relay log is running.</p>|`last(/MySQL by Zabbix agent/mysql.slave_sql_running["{#MASTER_HOST}"])=2`|Warning|**Depends on**:<br><ul><li>MySQL: The replica I/O thread is not running</li></ul>|
