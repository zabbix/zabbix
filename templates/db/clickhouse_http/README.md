...
|ClickHouse: Too many operations in queue (over {$CLICKHOUSE.QUEUE.SIZE.MAX.WARN} for 5m) |<p>-</p> |`{TEMPLATE_NAME:clickhouse.replica.queue_size["{#DB}.{#TABLE}"].min(5m)}>{$CLICKHOUSE.QUEUE.SIZE.MAX.WARN:"{#TABLE}"}` |WARNING | |
|ClickHouse: Differense between log_max_index and log_pointer is too hight (More than {$CLICKHOUSE.LOG_POSITION.DIFF.MAX.WARN})  |<p>-</p> |`{Template DB ClickHouse by HTTP:clickhouse.replica.log_max_index["{#DB}.{#TABLE}"].last()} - {TEMPLATE_NAME:clickhouse.replica.log_pointer["{#DB}.{#TABLE}"].last()} > {$CLICKHOUSE.LOG_POSITION.DIFF.MAX.WARN}` |WARNING | |
|ClickHouse: Number of active replicas less than number of total replicas |<p>-</p> |`{TEMPLATE_NAME:clickhouse.replica.active_replicas["{#DB}.{#TABLE}"].max(5m)} < {Template DB ClickHouse by HTTP:clickhouse.replica.total_replicas["{#DB}.{#TABLE}"].last()}` |WARNING | |
