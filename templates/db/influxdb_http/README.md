...
|InfluxDB: Too many tasks runs failures (over {$INFLUXDB.TASK.RUN.FAIL.MAX.WARN} for 5m) |<p>"Too many tasks failed runs across all tasks."</p> |`min(/InfluxDB by HTTP/influxdb.task_executor_complete.failed.rate,5m)>{$INFLUXDB.TASK.RUN.FAIL.MAX.WARN}` |WARNING | |
