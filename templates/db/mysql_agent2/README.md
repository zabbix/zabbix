...
|MySQL|MySQL: Created tmp files on disk|<p>How many temporary files mysqld has created.</p>|DEPENDENT|mysql.created_tmp_files<p>**Preprocessing**:</p><p>- JSONPATH: `$.Created_tmp_files`</p>|
|MySQL|MySQL: Created tmp tables on disk|<p>The number of internal on-disk temporary tables created by the server while executing statements.</p>|DEPENDENT|mysql.created_tmp_disk_tables<p>**Preprocessing**:</p><p>- JSONPATH: `$.Created_tmp_disk_tables`</p>|
|MySQL|MySQL: Created tmp tables on memory|<p>The number of internal temporary tables created by the server while executing statements.</p>|DEPENDENT|mysql.created_tmp_tables<p>**Preprocessing**:</p><p>- JSONPATH: `$.Created_tmp_tables`</p>|
