...
|MySQL |MySQL: Binlog commits diff |<p>Difference between binlog_commits and binlog_group_commits.</p> |CALCULATED |mysql.binlog_commits_diff[{#SINGLETON}]<p>**Expression**:</p>`last(mysql.binlog_commits[{#SINGLETON}])-last(mysql.binlog_group_commits[{#SINGLETON}])` |
...
|MySQL: SQL thread has read far ahead in the binlog |<p>SQL thread has read as far ahead in the binlog as the value of slave-parallel-max-queued allows. The worker threads are lagging behind, check Slave_SQL_Running_State for the error.</p> |`{TEMPLATE_NAME:mysql.binlog_commits_diff[{#SINGLETON}].last()} > 0` |WARNING | |
