...
|Slowlog entries per second|<p>Tracks the rate of slowlog entries from the Redis server.</p>|Zabbix agent|redis.slowlog.count["{$REDIS.CONN.URI}","{$REDIS.PASSWORD}","{$REDIS.USERNAME}"]<p>**Preprocessing**</p><ul><li>Change per second</li></ul>|
...
|----|-----------|----------|--------|--------------------------------|
|Redis: Failed to fetch INFO|<p>Zabbix has not received any data from the Redis INFO item for the last 30 minutes.</p>|`nodata(/Redis by Zabbix agent 2/redis.info["{$REDIS.CONN.URI}","{$REDIS.PASSWORD}","{$REDIS.SECTION}","{$REDIS.USERNAME}"],30m)=1`|Warning|**Manual close**: Yes<br>**Depends on**:<br><ul><li>Redis: Service is down</li></ul>|
