...
- Discovery of tables is disabled by default. To enable table discovery, change the value of macro `{$MARIADB.TABLE.NAMES.MATCHES}` to the name of a single table, or use a regex string to include multiple tables.
...
To exclude certain tables from discovery, change the value of macro `{$MARIADB.TABLE.NAMES.NOT_MATCHES}` to the name of a single table, or use a regex string to exclude multiple tables.
...
|{$MARIADB.FULLSCAN.RATIO.WARN}|<p>Warning threshold of full table scan ratio.</p>|`0.1`|
|{$MARIADB.FULLSCAN.SELECT_RATE.MIN.WARN}|<p>The minimum SELECT statements rate (per second) for trigger expressions.</p>|`5`|
|{$MARIADB.BUFF_UTIL.MIN.WARN}|<p>The minimum buffer pool utilization in percentage for trigger expressions.</p>|`50`|
...
|{$MARIADB.TOTALSIZE.GROWTH.MAX.WARN}|<p>The warning threshold of the table total size growth for trigger expressions.</p>|`300M`|
|{$MARIADB.TOTALSIZE.GROWTH.MAX.CRIT}|<p>The critical threshold of the table total size growth for trigger expressions.</p>|`1G`|
...
|{$MARIADB.DATA_FREE.MIN.WARN}|<p>The minimum data free value for warning fragmentation trigger expressions.</p>|`1G`|
|{$MARIADB.DATA_FREE.MIN.CRIT}|<p>The minimum data free value for critical fragmentation trigger expressions.</p>|`5G`|
...
|Created tmp files on disk per second|<p>How many temporary files `mariadbd` has created.</p>|Dependent item|mariadb.created_tmp_files.rate<p>**Preprocessing**</p><ul><li><p>JSON Path: `$[?(@.Variable_name=='Created_tmp_files')].Value.first()`</p></li><li>Change per second</li></ul>|
...
|InnoDB buffer pool wait free|<p>Number of times InnoDB waited for a free page before reading or creating a page. Normally, writes to the InnoDB buffer pool happen in the background. When no clean pages are available, dirty pages are flushed first in order to free some up. This counts the numbers of wait for this operation to finish. If this value is not small, look at the increasing `innodb_buffer_pool_size`.</p>|Dependent item|mariadb.innodb_buffer_pool_wait_free<p>**Preprocessing**</p><ul><li><p>JSON Path: `The text is too long. Please see the template.`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
...
|Calculated value of innodb_log_file_size|<p>`Innodb_log_file_size` is calculated as: (`innodb_os_log_written`-`innodb_os_log_written`(time shift -1h))/`{$MARIADB.INNODB_LOG_FILES}`. `Innodb_log_file_size` is the size in bytes of the each InnoDB redo log file in the log group. The combined size can be no more than 512 GB. Larger values mean less disk I/O due to less flushing checkpoint activity, but also slower recovery from a crash.</p>|Calculated|mariadb.innodb_log_file_size<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
...
|Binlog group commits|<p>Total number of group commits done to the binary log.</p>|Dependent item|mariadb.binlog_group_commits<p>**Preprocessing**</p><ul><li><p>JSON Path: `$[?(@.Variable_name=='Binlog_group_commits')].Value.first()`</p></li><li><p>Discard unchanged with heartbeat: `1h`</p></li></ul>|
...
|MariaDB: Server has slow queries|<p>The number of slow queries is more than `{$MARIADB.SLOW_QUERIES.MAX.WARN}` in the last 5 minutes.</p>|`min(/MariaDB by ODBC/mariadb.slow_queries.rate,5m)>{$MARIADB.SLOW_QUERIES.MAX.WARN}`|Warning||
|MariaDB: High rate of full table scans|<p>The rate of SELECT statements exceeds `{$MARIADB.FULLSCAN.SELECT_RATE.MIN.WARN}` per second and the full table scan ratio is above `{$MARIADB.FULLSCAN.RATIO.WARN}` in the last 5 minutes.</p>|`avg(/MariaDB by ODBC/mariadb.full_scans.ratio,5m)>{$MARIADB.FULLSCAN.RATIO.WARN} and avg(/MariaDB by ODBC/mariadb.com_select.rate,5m)>{$MARIADB.FULLSCAN.SELECT_RATE.MIN.WARN}`|Warning||

### LLD rule Database discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database discovery|<p>Used for the discovery of the databases.</p>|Dependent item|mariadb.database.discovery<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `1d`</p></li><li><p>JavaScript: `The text is too long. Please see the template.`</p></li></ul>|
...
### LLD rule Database [{#DATABASE}]: Tables enable discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DATABASE}]: Tables enable discovery|<p>Used to enable table discovery.</p>|Script|mariadb.table.enable.discovery[{#DATABASE}]<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Database [{#DATABASE}]: Tables enable discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DATABASE}]: Get tables|<p>Database tables discovery.</p>|Database monitor|db.odbc.get[{#SINGLETON}{#DATABASE}_tables,"{$MARIADB.DSN}"]|

### LLD rule Database [{#DATABASE}]: Tables discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Database [{#DATABASE}]: Tables discovery|<p>Used for the discovery tables of the databases.</p>|Dependent item|mariadb.tables.discovery[{#SINGLETON}{#DATABASE}]<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|

### Item prototypes for Database [{#DATABASE}]: Tables discovery

|Name|Description|Type|Key and additional info|
|----|-----------|----|-----------------------|
|Table [{#DATABASE}/{#TABLE_NAME}]: Get data|<p>Database tables data.</p>|Database monitor|db.odbc.get[{#SINGLETON}{#DATABASE}/{#TABLE_NAME}.data,"{$MARIADB.DSN}"]<p>**Preprocessing**</p><ul><li><p>JSON Path: `$.first()`</p><p>⛔️Custom on fail: Discard value</p></li></ul>|
...
|Replication: {#SOURCE.NAME} Host|<p>Describe the hostname or IP address of the MariaDB primary (source/master) server that the replica uses.</p>|Dependent item|mariadb.master_host["{#REPLICA.KEY}/{#REPLICA.NAME}"]<p>**Preprocessing**</p><ul><li><p>JSON Path: `$.{#SOURCE.NAME}_Host`</p></li><li><p>Discard unchanged with heartbeat: `6h`</p></li></ul>|
...
|MariaDB: The {#REPLICA.KEY} I/O thread is not running|<p>Whether the I/O thread for reading the {#SOURCE.KEY}'s binary log is running.</p>|`last(/MariaDB by ODBC/mariadb.slave_io_running["{#REPLICA.KEY}/{#REPLICA.NAME}"])=0`|Average||
...
|MariaDB: The SQL thread is not running|<p>Whether the SQL thread for executing events in the relay log is running.</p>|`last(/MariaDB by ODBC/mariadb.slave_sql_running["{#REPLICA.KEY}/{#REPLICA.NAME}"])=0`|Warning|**Depends on**:<br><ul><li>MariaDB: The {#REPLICA.KEY} I/O thread is not running</li></ul>|
