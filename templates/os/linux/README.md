...
|Memory |Available memory in % |<p>Available memory as percentage of total. See also: https://www.zabbix.com/documentation/current/manual/appendix/items/vm.memory.size_params</p> |ZABBIX_PASSIVE |vm.memory.size[pavailable] |
...
|Zabbix_raw_items |{#DEVNAME}: Get /sys/block/{#DEVNAME}/stat |<p>Get contents of /sys/block/{#DEVNAME}/stat for disk stats.</p> |ZABBIX_PASSIVE |vfs.file.contents[/sys/block/{#DEVNAME}/stat]<p>**Preprocessing**:</p><p>- JAVASCRIPT: `return JSON.stringify(value.trim().split(/ +/));`</p> |
