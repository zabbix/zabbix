...
NOTE: Starting from template version '7.4-1', the API token is used in the request header. For older template versions (where the API token is passed in the URL query parameter), with using FortiGate v7.4.5+ you must enable the following global setting:
