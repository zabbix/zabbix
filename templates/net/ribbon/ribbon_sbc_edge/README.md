...
|SIP Server [{#SIP.SERVER.NAME}]:[{#SIP.SERVER.DESCR}]: Downtime|<p>Displays the the time for which server was down due to not responding to options request or dns failures.</p>|Dependent item|ribbon.sip.server.downtime[{#SIP.SERVER.NAME}]<p>**Preprocessing**</p><ul><li><p>JSON Path: `$.rt_DownTime`</p></li></ul>|
