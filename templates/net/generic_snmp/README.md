...
|Network Generic Device: High ICMP ping loss||`min(/Network Generic Device by SNMP/icmppingloss,5m)>{$ICMP_LOSS_WARN} and min(/Network Generic Device by SNMP/icmppingloss,5m)<100`|Warning|**Depends on**:<br><ul><li>Network Generic Device: Unavailable by ICMP ping</li></ul>|
