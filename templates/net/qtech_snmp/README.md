...
|----|-----------|----|-----------------------|
|CPU utilization|<p>MIB: QTECH-MIB</p><p>The CPU utilization expressed in %.</p>|SNMP agent|system.cpu.util[switchCpuUsage.0]|
|Used memory|<p>MIB: QTECH-MIB</p><p>Used memory in bytes.</p>|SNMP agent|vm.memory.used[switchMemoryBusy.0]|
|Total memory|<p>MIB: QTECH-MIB</p><p>The total memory expressed in bytes.</p>|SNMP agent|vm.memory.total[switchMemorySize.0]|
|Memory utilization|<p>Memory utilization in %.</p>|Calculated|vm.memory.util[vm.memory.util.0]|
...
|Operating system|<p>MIB: QTECH-MIB</p>|SNMP agent|system.sw.os[sysSoftwareVersion.0]<p>**Preprocessing**</p><ul><li><p>Discard unchanged with heartbeat: `1d`</p></li></ul>|
|Temperature|<p>MIB: QTECH-MIB</p><p>Temperature readings of testpoint.</p>|SNMP agent|sensor.temp.value[switchTemperature.0]|
...
|QTech QSW: Host has been restarted|<p>The uptime counter has decreased, which indicates that the host has been restarted.<br>A decrease is ignored if the previous value was close enough to the 32-bit SNMP counter limit (2^32 hundredths of a second, about 497 days) for the counter to have wrapped between the two readings.<br>The problem is resolved automatically once uptime exceeds {$SNMP.UPTIME.WARN}.</p>|`(last(/QTech QSW by SNMP/system.hw.uptime[hrSystemUptime.0])>0 and change(/QTech QSW by SNMP/system.hw.uptime[hrSystemUptime.0])<0 and last(/QTech QSW by SNMP/system.hw.uptime[hrSystemUptime.0],#2)<42949672.96-(lastclock(/QTech QSW by SNMP/system.hw.uptime[hrSystemUptime.0])-lastclock(/QTech QSW by SNMP/system.hw.uptime[hrSystemUptime.0],#2))) or (last(/QTech QSW by SNMP/system.hw.uptime[hrSystemUptime.0])=0 and change(/QTech QSW by SNMP/system.net.uptime[sysUpTime.0])<0 and last(/QTech QSW by SNMP/system.net.uptime[sysUpTime.0],#2)<42949672.96-(lastclock(/QTech QSW by SNMP/system.net.uptime[sysUpTime.0])-lastclock(/QTech QSW by SNMP/system.net.uptime[sysUpTime.0],#2)))`|Warning|**Manual close**: Yes<br>**Depends on**:<br><ul><li>QTech QSW: No SNMP data collection</li></ul>|
...
|QTech QSW: High ICMP ping loss|<p>ICMP packets loss detected.</p>|`min(/QTech QSW by SNMP/icmppingloss,5m)>{$ICMP_LOSS_WARN} and min(/QTech QSW by SNMP/icmppingloss,5m)<100`|Warning|**Depends on**:<br><ul><li>QTech QSW: Unavailable by ICMP ping</li></ul>|
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|QTech QSW: {#SNMPINDEX}: Power supply is in critical state|<p>Please check the power supply unit for errors.</p>|`count(/QTech QSW by SNMP/sensor.psu.status[sysPowerStatus.{#SNMPINDEX}],#1,"eq","{$PSU_CRIT_STATUS}")=1`|Average||
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|QTech QSW: {#SNMPINDEX}: Fan is in critical state|<p>Please check the fan unit.</p>|`count(/QTech QSW by SNMP/sensor.fan.status[sysFanStatus.{#SNMPINDEX}],#1,"eq","{$FAN_CRIT_STATUS}")=1`|Average||
...
|Name|Description|Expression|Severity|Dependencies and additional info|
|----|-----------|----------|--------|--------------------------------|
|QTech QSW: Interface {#IFNAME}({#IFALIAS}): In half-duplex mode|<p>Please check autonegotiation settings and cabling.</p>|`last(/QTech QSW by SNMP/net.if.duplex[dot3StatsDuplexStatus.{#SNMPINDEX}])=2`|Warning|**Manual close**: Yes|
