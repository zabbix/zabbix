...
|System name has changed |<p>The name of the system has changed. `Ack` to close the problem manually.</p> |`last(/Linux generic by Zabbix agent/system.hostname,#1)<>last(/Linux generic by Zabbix agent/system.hostname,#2) and length(last(/Linux generic by Zabbix agent/system.hostname))>0` |INFO |<p>Manual close: YES</p> |
