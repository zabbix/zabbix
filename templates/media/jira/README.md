...
Jira webhook supports string, URL, number, datetime, single-select, multi-select, checkbox, and radio button fields. Note, that you can pass only one value for each custom field for now. Types of fields used in [media_jira_with_CF_example.xml](media_jira_with_CF_example.xml) are defined at the beginning of parameter placeholders.<br>
