...
func TestPlugin_execute(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+validBasicDevices", args: args{
			jsonRunner: false,
		}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
		...
		{name: "+validByIDDevices", args: args{
			byID:       true,
			jsonRunner: false,
		}, expectations: []expectation{
			{
		...
		{name: "+basicJSONRunner", args: args{
			jsonRunner: true,
		}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
		...
		{name: "+singleRAIDDeviceFromEnv1", args: args{
			jsonRunner: true,
		}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
		...
		{name: "+nonZeroStatusCode", args: args{
			jsonRunner: true,
		}, expectations: []expectation{
			{
				args: []string{
		...
		{name: "-basicDeviceScanError", args: args{jsonRunner: false}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
				err:  errs.New("test error"),
				out:  []byte(""),
			},
		}, wantRunner: nil, wantErr: true},
		{name: "-basicSmartScanError", args: args{jsonRunner: false}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
				err:  nil,
		...
		{name: "-basicDeviceNoSmart", args: args{jsonRunner: false}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
				err:  nil,
				out:  readControllerEnvironment(t, "manually_created_2_basic_devices").AllDevicesScan,
		...
		{name: "-megaraidSmartScanError", args: args{jsonRunner: false}, expectations: []expectation{
			{
				args: []string{"--scan", "-j"},
				err:  nil,
				out:  []byte("{}"),
...
func Test_getBasicDeviceInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+valid", deviceName: "/dev/sda", expectations: expectation{
			args: []string{"-a", "/dev/sda", "-j"},
			err:  nil,
			out:  readControllerFixture(t, "device/all_info_sda.json"),
		}, args: args{
		...
		{name: "-invalidJSON", deviceName: "/dev/sda", expectations: expectation{
			args: []string{"-a", "/dev/sda", "-j"},
			err:  nil,
			...
			jsonRunner: false,
		}, expectedResult: nil, wantErr: true},
		{name: "-noSmartStatus", deviceName: "/dev/sda", expectations: expectation{
			args: []string{"-a", "/dev/sda", "-j"},
			err:  nil,
		...
		{name: "-smartError", deviceName: "/dev/sda", expectations: expectation{
			args: []string{"-a", "/dev/sda", "-j"},
...
func Test_evaluateVersion(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+correctVersion", args: args{versionDigits: []int{7, 1}}, wantErr: false},
		{name: "+correctVersionOneDigit", args: args{versionDigits: []int{8}}, wantErr: false},
		{name: "+correctVersionMultipleDigits", args: args{versionDigits: []int{7, 1, 2}}, wantErr: false},
		{name: "-incorrectVersion", args: args{versionDigits: []int{7, 0}}, wantErr: true},
		{name: "-malformedVersion", args: args{versionDigits: []int{-7, 0}}, wantErr: true},
...
func Test_cutPrefix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+hasPrefix", args: args{in: "/dev/sda"}, want: "sda"},
		{name: "-noPrefix", args: args{in: "sda"}, want: "sda"},
		{name: "-empty", args: args{in: ""}, want: ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
...
func Test_deviceParser_checkErr(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+noErr", fields: fields{Smartctl: smartctlField{
			Messages:   nil,
			ExitStatus: 0,
		}}, wantErr: false, wantMsg: ""},
		{name: "+rawRespErr", fields: fields{
			rawResp: readControllerEnvironment(t, "env_1").
				AllSmartInfoScans.get(t, "-a /dev/sda -d 3ware,0 -j"),
		}, wantErr: true, wantMsg: "/dev/sda: Unknown device type '3ware,0', =======> VALID " +
			...
			"auto, test <=======."},
		{name: "+rawRespNoErr", fields: fields{
			rawResp: readControllerEnvironment(t, "env_1").
				AllSmartInfoScans.get(t, "-a /dev/sda -d sat -j"),
		}, wantErr: false, wantMsg: ""},
		{name: "+noErr", fields: fields{Smartctl: smartctlField{
		...
		{name: "+warning", fields: fields{
			Smartctl: smartctlField{
				Messages: []message{{Str: "barfoo"}}, ExitStatus: 3,
			},
		}, wantErr: true, wantMsg: "Barfoo."},
		{name: "-errorStatusOne", fields: fields{
			Smartctl: smartctlField{
				Messages: []message{{Str: "barfoo"}}, ExitStatus: 1,
			},
		}, wantErr: true, wantMsg: "Barfoo."},
		{name: "-errorStatusTwo", fields: fields{
			Smartctl: smartctlField{
				Messages: []message{{Str: "foobar"}}, ExitStatus: 2,
			},
		}, wantErr: true, wantMsg: "Foobar."},
		{name: "-twoErr", fields: fields{
			Smartctl: smartctlField{
				Messages: []message{{Str: "foobar"}, {Str: "barfoo"}}, ExitStatus: 2,
			},
		}, wantErr: true, wantMsg: "Foobar, barfoo."},
		{name: "-unknownErr/noMessage", fields: fields{
			Smartctl: smartctlField{
				Messages: []message{}, ExitStatus: 2,
			},
		}, wantErr: true, wantMsg: "Unknown error from smartctl."},
	}
...
func TestPlugin_checkVersion(t *testing.T) { //nolint:paralleltest
	...
	tests := []struct {
		...
		{name: "+valid", expect: expect{exec: true}, fields: fields{execOut: readControllerFixture(t, "version/valid.json")}, wantErr: false},
		{name: "-noCheck", expect: expect{exec: false}, fields: fields{
			lastVerCheck: time.Now(),
		}, wantErr: false},
		{name: "-executeErr", expect: expect{exec: true}, fields: fields{
			execOut: readControllerFixture(t, "version/valid.json"),
			execErr: errors.New("fail"),
		}, wantErr: true},
		{name: "-unmarshalErr", expect: expect{exec: true}, fields: fields{execOut: []byte("{")}, wantErr: true},
		{name: "-evaluateVersionErr", expect: expect{exec: true}, fields: fields{execOut: readControllerFixture(t, "version/invalid.json")}, wantErr: true},
	}
	//nolint:paralleltest
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_getAllDeviceInfoByType(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "-unmarshalErr", fields: fields{out: []byte("{")}, args: args{
			deviceName: "/dev/sda",
			deviceType: "raid,1,2,3",
		}, want: nil, wantErr: true},
		{name: "-smartctlErr", fields: fields{out: sampleFailedSmartInfoScan}, args: args{
			deviceName: "/dev/sda",
			deviceType: "raid,1,2,3",
		}, want: nil, wantErr: true},
		{name: "-missingSmartStatus", fields: fields{out: sampleMissingSmartStatusSmartInfoScan}, args: args{
			deviceName: "/dev/sda",
			deviceType: "raid,1,2,3",
		}, want: nil, wantErr: true},
	}
...
func Test_getRaidDevices(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+sat", expectations: []expectation{
			{
		...
		{name: "+scsi", expectations: []expectation{
			{
		...
		{name: "-invalidType3ware", expectations: []expectation{
			{
...
func Test_setDeviceData(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+validJsonRunner", args: args{jsonRunner: true, data: &SmartCtlDeviceData{
			Device: &deviceParser{
				ModelName:    "SAMSUNG MZVL21T0HCLR-00BH1",
				SerialNumber: "S641NX0T509005",
				Info: deviceInfo{
		...
		{name: "+validDeviceRunner", args: args{jsonRunner: false, data: &SmartCtlDeviceData{
			Device: &deviceParser{
				ModelName:    "SAMSUNG MZVL21T0HCLR-00BH1",
				SerialNumber: "S641NX0T509005",
				Info: deviceInfo{
...
func Test_runner_parseOutput(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "+validJSONRunner", args: args{jsonRunner: true}, fields: fields{
			jsonDevices: map[string]jsonDevice{
				"/dev/sda": {
		...
		{name: "+jsonRunnerWithDuplicateDevices", args: args{jsonRunner: true}, fields: fields{
			jsonDevices: map[string]jsonDevice{
				"/dev/sda": {
					serialNumber: "Serial123",
					jsonData:     "data1",
		...
		{name: "+JSONRunnerWithPrevData", args: args{jsonRunner: true}, fields: fields{
			devices: map[string]deviceParser{
		...
		{name: "+deviceRunnerWithPrevData", args: args{jsonRunner: false}, fields: fields{
			devices: map[string]deviceParser{
				"/dev/sda": {SerialNumber: "Serial123"},
				"/dev/sdb": {SerialNumber: "Serial123"},
			},
		...
		{name: "+nonJSONRunnerWithUniqueDevices", args: args{jsonRunner: false}, fields: fields{
			devices: map[string]deviceParser{
				"/dev/sda": {SerialNumber: "Serial123"},
		...
		{name: "+nonJSONRunnerWithDuplicateDevices", args: args{jsonRunner: false}, fields: fields{
			devices: map[string]deviceParser{
				"/dev/sda": {SerialNumber: "Serial123"},
				"/dev/sdb": {SerialNumber: "Serial123"},
			},
		...
		{name: "+jsonRunnerWithTwoDuplicateDevices", args: args{jsonRunner: false}, fields: fields{
			devices: map[string]deviceParser{
				"/dev/sda": {SerialNumber: "Serial123"},
				"/dev/sdb": {SerialNumber: "Serial123"},
		...
		{name: "-jsonRunnerWithoutDevices", args: args{jsonRunner: false}, fields: fields{
			devices: map[string]deviceParser{},
		}, wantRunner: runner{
			devices: map[string]deviceParser{},
		}},
		{name: "-dataDifferenceLoss", args: args{jsonRunner: true}, fields: fields{
			jsonDevices: map[string]jsonDevice{
				"/dev/sda": {
					serialNumber: "Serial123",
					jsonData:     "data1",
...
func TestPlugin_getDevices(t *testing.T) {
	...
	tests := []struct {
		...
		}, wantMegaraid: nil, wantErr: false},
		{name: "+envMac", args: args{
			scanCMD: defaultScan,
			raidCMD: defaultRaidScan,
		}, expect: expect{
			raidScanExec: true,
		...
		}, wantRaid: nil, wantMegaraid: nil, wantErr: false},
		{name: "+HBA_with_SAS_1", args: args{
			scanCMD: defaultScan,
			raidCMD: defaultRaidScan,
		}, expect: expect{
			raidScanExec: true,
		...
		}, wantBasic: nil, wantRaid: []deviceInfo{
			{
				Name:     "/dev/sda",
				InfoName: "/dev/sda",
				DevType:  "scsi",
		...
		}, wantBasic: nil, wantRaid: nil, wantMegaraid: nil, wantErr: true},
		{name: "-raidScanErr", args: args{
			scanCMD: defaultScan,
			raidCMD: defaultRaidScan,
		}, expect: expect{
			raidScanExec: true,
...
func Test_formatDeviceOutput(t *testing.T) {
	...
	tests := []struct {
		...
		}, wantBasicDev: []deviceInfo{sampleBasicDev1, sampleBasicDev2}, wantRaidDev: []deviceInfo{sampleRaidDev1, sampleRaidDev2}, wantMegaraidDev: []deviceInfo{sampleMegaraidDev1, sampleMegaraidDev2}},
		{name: "+megaraidDevices", args: args{
			basic: []deviceInfo{},
			raid: []deviceInfo{
				sampleRaidDev1, sampleRaidDev2,
				sampleMegaraidDev1, sampleMegaraidDev2,
			},
		}, wantBasicDev: nil, wantRaidDev: []deviceInfo{sampleRaidDev1, sampleRaidDev2}, wantMegaraidDev: []deviceInfo{sampleMegaraidDev1, sampleMegaraidDev2}},
		{name: "-duplicateDevInRaidAndBasic", args: args{
			basic: []deviceInfo{sampleBasicDev1, sampleRaidDev1},
			raid:  []deviceInfo{sampleRaidDev1, sampleRaidDev2},
		}, wantBasicDev: []deviceInfo{sampleBasicDev1}, wantRaidDev: []deviceInfo{sampleRaidDev1, sampleRaidDev2}, wantMegaraidDev: nil},
		{name: "-noDevices", args: args{
			basic: []deviceInfo{},
			raid:  []deviceInfo{},
		}, wantBasicDev: nil, wantRaidDev: nil, wantMegaraidDev: nil},
		{name: "-nilDevices", args: args{
			basic: nil,
			raid:  nil,
		}, wantBasicDev: nil, wantRaidDev: nil, wantMegaraidDev: nil},
	}
...
func TestPlugin_scanDevices(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "-execErr", fields: fields{
			execOut: readControllerFixture(t, "discovery/basic_scan.json"),
			execErr: errors.New("fail"),
		}, args: args{args: []string{"--scan", "-j"}}, want: nil, wantErr: true},
		{name: "-marshalErr", fields: fields{execOut: []byte("{")}, args: args{args: []string{"--scan", "-j"}}, want: nil, wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
