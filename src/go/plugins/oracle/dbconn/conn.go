...
func NewConnManager(logr log.Logger, opt *Options) *ConnManager {
	...
	connMgr := &ConnManager{
		...
		QueryStorage:  setCustomQuery(logr, opt.CustomQueriesPath),
...
//
//nolint:ireturn
func setCustomQuery(logr log.Logger, path string) yarn.Yarn {
	if path == "" {
	...
	if err != nil {
		...
		// createConn empty storage if an error occurred
		return yarn.NewFromMap(map[string]string{})
	}
