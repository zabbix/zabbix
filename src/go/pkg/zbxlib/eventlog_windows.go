...
func ProcessEventLogCheck(data unsafe.Pointer, item *EventLogItem, refresh int, cblob unsafe.Pointer) {
	log.Debugf("Calling C function \"metric_set_refresh\"")
	...
	log.Debugf("Calling C function \"get_eventlog_value\"")
	...
	log.Debugf("Calling C function \"free_eventlog_result\"")
	C.free_eventlog_result(result)

	item.LastTs = logTs

	if ret == C.FAIL {
		log.Debugf("Calling C function \"metric_set_unsupported\"")
		...
		log.Debugf("Calling C function \"metric_set_supported\"")
