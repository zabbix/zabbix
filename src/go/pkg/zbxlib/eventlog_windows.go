...
func ProcessEventLogCheck(data unsafe.Pointer, item *EventLogItem, refresh int, cblob unsafe.Pointer) {
	log.Critf("Calling C function \"metric_set_refresh()\"")
	...
	log.Critf("Calling C function \"get_eventlog_value()\"")
	...
	log.Critf("Calling C function \"free_eventlog_result()\"")
	C.free_eventlog_result(result)

	item.LastTs = logTs

	if ret == C.FAIL {
		log.Critf("Calling C function \"metric_set_unsupported()\"")
		...
		log.Critf("Calling C function \"metric_set_supported()\"")
