...
func init() {
	log.Debugf("Calling C function \"tls_init\"")
...
func describeCiphersuites(context unsafe.Pointer) (desc string) {
	var cDesc *C.char
	log.Debugf("Calling C function \"tls_describe_ciphersuites\"")
...
func (c *tlsConn) Error() (err error) {
	var cBuf *C.char
	var errmsg string
	log.Debugf("Calling C function \"tls_error\"")
...
func (c *tlsConn) ready() bool {
	log.Debugf("Calling C function \"tls_ready\"")
...
func (c *tlsConn) flushTLS() (err error) {
	for {
		log.Debugf("Calling C function \"tls_recv\"")
...
func (c *tlsConn) recvTLS() (err error) {
	...
	log.Debugf("Calling C function \"tls_send\"")
...
func (c *tlsConn) Close() (err error) {
	log.Debugf("Calling C function \"tls_close\"")
	cr := C.tls_close((*C.tls_t)(c.tls))
	c.conn.Close()

	log.Debugf("Calling C function \"tls_free\"")
...
func (c *tlsConn) verifyIssuerSubject(cfg *Config) (err error) {
	if cfg.Connect == ConnCert && (cfg.ServerCertIssuer != "" || cfg.ServerCertSubject != "") {
		...
		log.Debugf("Calling C function \"tls_validate_issuer_and_subject\"")
...
func (c *tlsConn) String() (desc string) {
	var cDesc *C.char
	log.Debugf("Calling C function \"tls_description\"")
...
func (c *Client) checkConnection() (err error) {
	log.Debugf("Calling C function \"tls_connected\"")
	if C.tls_connected((*C.tls_t)(c.tls)) == C.int(1) {
		return
	}
	log.Debugf("Calling C function \"tls_connected\"")
	for C.tls_connected((*C.tls_t)(c.tls)) != C.int(1) {
		log.Debugf("Calling C function \"tls_handshake\"")
...
func (c *Client) Write(b []byte) (n int, err error) {
	...
	log.Debugf("Calling C function \"tls_write\"")
...
func (c *Client) Read(b []byte) (n int, err error) {
	for {
		...
		log.Debugf("Calling C function \"tls_read\"")
...
func NewClient(nc net.Conn, cfg *Config, timeout time.Duration, shiftDeadline bool) (conn net.Conn, err error) {
	...
	log.Debugf("Calling C function \"tls_new_client\"")
	c := &Client{
		tlsConn: tlsConn{
			conn:          nc,
			buf:           make([]byte, 4096),
...
func (s *Server) checkConnection() (err error) {
	...
	for {
		log.Debugf("Calling C function \"tls_accept\"")
...
func NewServer(nc net.Conn, cfg *Config, b []byte, timeout time.Duration, shiftDeadline bool) (conn net.Conn, err error) {
	...
	// for TLS we overwrite the timeoutMode and force it to move on every read or write
	log.Debugf("Calling C function \"tls_new_server\"")
	s := &Server{
		tlsConn: tlsConn{
			conn:          nc,
			buf:           make([]byte, 4096),
...
func CopyrightMessage() (message string) {
	log.Debugf("Calling C function \"tls_version\"")
	version := C.tls_version()
	if version == nil {
		return ""
	}

	log.Debugf("Calling C function \"tls_version_static\"")
...
func Init(config *Config) (err error) {
	...
	if pskContext != nil {
		log.Debugf("Calling C function \"tls_free_context\"")
	...
	log.Debugf("Calling C function \"tls_new_context\"")
