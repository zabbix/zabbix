		...
		zbx_vector_uint64_t *triggerids_out)
{
	...
	if (0 > curr_case_idx)
		fail_msg("Error in the test execution sequence");
...
void test_try_task_closes_problem()
{
	...
	{
		if (0 == strcmp(cases[i].tested_function, "try_task_closes_problem"))
		{
			curr_case_idx = i;
			curr_case_name = cases[curr_case_idx].case_name;

			param1 = atoi(cases[curr_case_idx].in_params.values[0]);

			ret = tm_try_task_close_problem(param1);

			res = atoi(cases[curr_case_idx].out_params.values[0]);
