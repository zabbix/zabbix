		...
		const zbx_correlation_result_t *result)
{
	...
	task = cep_create_task_event_closed_by_correlation(cep_create_close_event(db_event), result->eventid,
			result->correlationid, db_event->eventid);

	/* 'close new' operations must skip actions for the problem and generated ok event */
	((zbx_cep_task_event_t *)task)->action_state = CEP_ACTION_DISABLED;
 ...
 *             c_eventid - [IN] eventid causing correlation                   *
 ...
 *                                                                            *
 ******************************************************************************/
static void	correlation_add_close_old_tasks(zbx_dbconn_t *db, zbx_uint64_t c_eventid,
		zbx_vector_mw_task_ptr_t *tasks, zbx_hashset_t *results)
{
	zbx_mw_task_t			*task;
	...
	while (NULL != (result = (zbx_correlation_result_t *)zbx_hashset_iter_next(&iter)))
	{
		...
		task = cep_create_task_event_closed_by_correlation(db_event, result->eventid, result->correlationid,
				c_eventid);
		zbx_vector_mw_task_ptr_append(tasks, task);
	}
		...
		zbx_vector_mw_task_ptr_t *tasks)
{
	...
	if (0 != results.num_data)
	{
		...
		correlation_add_close_old_tasks(db, db_event->eventid, tasks, &results);
	}
