 ...
 *                                                                            *
 ******************************************************************************/
static zbx_uint32_t	cep_window_event_pos(int index, int events_num)
{
	zbx_uint32_t	pos = CEP_POS_UNKNOWN;
...
static zbx_uint64_t	cep_window_execute_operations(const zbx_cep_rule_t *rule, zbx_cep_window_t *window,
		zbx_dbconn_pool_t *dbpool, int when, zbx_vector_mw_task_ptr_t *tasks)
{
	...
	while (NULL != (hevent = (zbx_cep_event_handle_t)zbx_queue_ptr_iter_next(&iter)))
	{
		...
		cep_event_context_init_with_handle(&ctx, zbx_cep_event_handle_addref(hevent),
			cep_window_event_pos(i++, events_num), dbpool);

		opmask |= cep_rule_event_context_execute_ops(rule, &ctx, when, NULL, tasks);
 ...
 *                                                                            *
 ******************************************************************************/
static void	cep_window_close(const zbx_cep_rule_t *rule, zbx_cep_window_t *window, zbx_dbconn_pool_t *dbpool,
		zbx_vector_mw_task_ptr_t *tasks)
{
	...
	for (int i = 0; i < events_num; i++)
	{
		...
		(void)cep_rule_event_context_execute_ops(rule, &ctx, ZBX_CEP_WHEN_WINDOW_CLOSED, NULL, tasks);
		cep_event_context_clear(&ctx);
	}
}
		...
		zbx_vector_mw_task_ptr_t *tasks)
{
	...
	{
		cep_window_unlock(window);
		opmask = cep_rule_event_context_execute_ops(rule, ctx, ZBX_CEP_WHEN_EVENT_EVICTED, NULL, tasks);
	}
	else
	{
	...
	{
		cep_window_lock(window);
		cep_window_close(rule, window, ctx->dbpool, tasks);
		cep_window_sync_entry_submit(window, tasks);
		cep_window_unlock(window);
	}
...
static zbx_uint64_t	cep_window_evict_expired(zbx_cep_window_t *window, zbx_cep_rule_t *rule,
		zbx_dbconn_pool_t *dbpool, time_t now, zbx_vector_mw_task_ptr_t *tasks)
{
	zbx_uint64_t	opmask = 0;
	...
	while (SUCCEED != zbx_queue_ptr_empty(&window->hevents))
	{
		...
		cep_event_context_init_with_handle(&ctx, zbx_cep_event_handle_addref(h), CEP_POS_FIRST, dbpool);
		...
		if (NULL != cep_event_context_get_event(&ctx))
		{
			...
			opmask |= cep_rule_event_context_execute_ops(rule, &ctx, ZBX_CEP_WHEN_EVENT_EVICTED, NULL,
					tasks);
		}
		...
		zbx_vector_mw_task_ptr_t *tasks)
{
	...
	opmask = cep_window_evict_expired(window, rule, dbpool, now, tasks);

	if (0 != (opmask & CEP_FLAG(ZBX_CEP_OP_CLOSE_WINDOW)))
		cep_window_close(rule, window, dbpool, tasks);
 ...
 *                                                                            *
 ******************************************************************************/
static void	cep_window_set_event_set_tag_value(zbx_cep_window_t *window, zbx_dbconn_pool_t *dbpool,
		zbx_cep_event_handle_t h, const char *tag, int value, zbx_vector_mw_task_ptr_t *tasks)
{
	int			index;
	...
	cep_event_context_init_with_handle(&ctx, zbx_cep_event_handle_addref(h), CEP_POS_UNKNOWN, dbpool);
 ...
 *                                                                            *
 * Parameters: ctx    - [IN] context of event to set cause on                 *
 *             hcause - [IN] handle of the causal event                       *
 *             ack    - [IN/OUT] acknowledge data                             *
 *                                                                            *
 ******************************************************************************/
static void	cep_window_set_event_cause(zbx_cep_event_context_t *ctx, zbx_cep_event_handle_t hcause,
		zbx_cep_acknowledge_t *ack)
{
	zbx_cep_t	*cep;
		...
		zbx_vector_mw_task_ptr_t *tasks)
{
	...
	else
	{
		...
		if (0 != zbx_queue_ptr_values_num(&window->hevents))
		{
			...
			{
				cep_window_set_event_set_tag_value(window, ctx->dbpool, h,
						rule->window->event_count_tag,
						zbx_queue_ptr_values_num(&window->hevents), tasks);
			}

			cep_window_set_event_cause(ctx, h, &ack);
		}
		else
			ctx->pos |= CEP_POS_FIRST;
		...
		cep_window_unlock(window);
		opmask = cep_rule_event_context_execute_ops(rule, ctx, ZBX_CEP_WHEN_EVENT_ADDED, &ack, tasks);
		ctx->pos &= ~CEP_POS_FIRST;
		...
		if (SUCCEED == cep_acknowledge_is_set(&ack))
		{
			...
			task = cep_create_task_acknowledge(&ack, rule->ruleid, cep_event_context_eventid(ctx));
			zbx_vector_mw_task_ptr_append(tasks, task);
			cep_acknowledge_clear(&ack);
		}
	}
		...
		zbx_vector_mw_task_ptr_t *tasks)
{
	...
	if (SUCCEED == cep_window_js_process_script(window, &es, &error))
		opmask |= cep_window_execute_operations(rule, window, dbpool, ZBX_CEP_WHEN_PATTERN_MATCH, tasks);
		...
		zbx_vector_mw_task_ptr_t *tasks)
{
	switch (window->type)
	{
		case ZBX_CEP_WINDOW_SIMPLE:
		case ZBX_CEP_WINDOW_CORRELATION:
			cep_window_sliding_process(window, dbpool, now, tasks);
			break;
		case ZBX_CEP_WINDOW_CAUSAL:
			cep_window_causal_process(window, dbpool, now, tasks);
			break;
		case ZBX_CEP_WINDOW_PATTERN:
			cep_window_js_process(window, dbpool, now, tasks);
			break;
	}
}
...
/******************************************************************************
 *                                                                            *
 * Purpose: queue cep window for processing                                   *
 *                                                                            *
 * Parameters: pool   - [IN/OUT] cep window pool                              *
 *             window - [IN/OUT] cep window to queue                          *
 *                                                                            *
 ******************************************************************************/
 ...
 *                                                                            *
 * Parameters: pool             - [IN/OUT] cep window pool to load windows    *
 *                                          into                              *
 *             db               - [IN] database connection to query           *
 *             windows          - [OUT] map from database group id to         *
 *                                       loaded window for resolving group    *
 *                                       events afterward                     *
...
/******************************************************************************
 *                                                                            *
 * Purpose: load events into cep window event queue                           *
 *                                                                            *
 * Parameters: window - [IN/OUT] cep window to load events into               *
 *             events - [IN/OUT] events to load, cleared on completion        *
 *                                                                            *
 ******************************************************************************/
