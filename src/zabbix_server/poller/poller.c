...
static int	parse_query_fields(const DC_ITEM *item, char **query_fields, unsigned char expand_macros)
{
	...
	do
	{
		...
		if (MACRO_EXPAND_YES == expand_macros)
		{
			substitute_simple_macros_secure(NULL, NULL, NULL,NULL, NULL, &item->host, item, NULL, NULL,
...
void	zbx_prepare_items(DC_ITEM *items, int *errcodes, int num, AGENT_RESULT *results, unsigned char expand_macros)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if (MACRO_EXPAND_YES == expand_macros && SUCCEED != substitute_key_macros_secure(&items[i].key, NULL,
		...
		switch (items[i].type)
		{
			case ITEM_TYPE_SNMPv3:
				...
				if (MACRO_EXPAND_YES == expand_macros)
				{
					substitute_simple_macros_secure(NULL, NULL, NULL, NULL, &items[i].host.hostid,
			...
			case ITEM_TYPE_SNMPv2c:
				...
				substitute_simple_macros_secure(NULL, NULL, NULL, NULL, &items[i].host.hostid, NULL,
			...
			case ITEM_TYPE_DB_MONITOR:
				if (MACRO_EXPAND_YES == expand_macros)
				{
					substitute_simple_macros_secure(NULL, NULL, NULL, NULL, NULL, NULL, &items[i],
			...
			case ITEM_TYPE_HTTPAGENT:
				...
				if (MACRO_EXPAND_YES == expand_macros)
				{
					substitute_simple_macros(NULL, NULL, NULL, NULL, &items[i].host.hostid, NULL,
							...
							0);
					substitute_simple_macros_secure(NULL, NULL, NULL,NULL, NULL, &items[i].host,
				...
				switch (items[i].post_type)
				{
					case ZBX_POSTTYPE_XML:
						if (SUCCEED != substitute_macros_xml_secure(&items[i].posts, &items[i],
					...
					case ZBX_POSTTYPE_JSON:
						substitute_simple_macros_secure(NULL, NULL, NULL,NULL, NULL,
				...
				substitute_simple_macros_secure(NULL, NULL, NULL,NULL, NULL, &items[i].host, &items[i],
						NULL, NULL, &items[i].headers, MACRO_TYPE_HTTP_RAW, NULL, 0);
				substitute_simple_macros(NULL, NULL, NULL, NULL, &items[i].host.hostid, NULL,
