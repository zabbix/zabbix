...
/*	struct linger ling;*/
	...
	if(hp==NULL)
	{
		...
		zbx_snprintf(error,MAX_STRING_LEN,"gethostbyname() failed [%s]", hstrerror(h_errno));
		zabbix_log(LOG_LEVEL_WARNING, error);
		result->msg=strdup(error);
#else
		zbx_snprintf(error,MAX_STRING_LEN,"gethostbyname() failed [%d]", h_errno);
		zabbix_log(LOG_LEVEL_WARNING, error);
		result->msg=strdup(error);
#endif
...
/*
	...
	if(s == -1)
	{
		zbx_snprintf(error,MAX_STRING_LEN,"Cannot create socket [%s]", strerror(errno));
		zabbix_log(LOG_LEVEL_WARNING, error);
		result->msg=strdup(error);
		return	FAIL;
	}
	...
	if( connect(s,(struct sockaddr *)&servaddr_in,sizeof(struct sockaddr_in)) == -1 )
	{
		...
		{
			case EINTR:
				zbx_snprintf(error,MAX_STRING_LEN,"Timeout while connecting to [%s]",item->host);
				zabbix_log(LOG_LEVEL_WARNING, error);
				result->msg=strdup(error);
				break;
			case EHOSTUNREACH:
				zbx_snprintf(error,MAX_STRING_LEN,"No route to host [%s]",item->host);
				zabbix_log(LOG_LEVEL_WARNING, error);
				result->msg=strdup(error);
				break;
			default:
				zbx_snprintf(error,MAX_STRING_LEN,"Cannot connect to [%s] [%s]",item->host, strerror(errno));
				zabbix_log(LOG_LEVEL_WARNING, error);
				result->msg=strdup(error);
		} 
		close(s);
	...
	zbx_snprintf(c, MAX_STRING_LEN, "%s\n",item->key);
	...
	if( write(s,c,strlen(c)) == -1 )
	{
		...
		{
			case EINTR:
				zbx_snprintf(error,MAX_STRING_LEN,"Timeout while sending data to [%s]",item->host);
				zabbix_log(LOG_LEVEL_WARNING, error);
				result->msg=strdup(error);
				break;
			default:
				zbx_snprintf(error,MAX_STRING_LEN,"Error while sending data to [%s] [%s]",item->host, strerror(errno));
				zabbix_log(LOG_LEVEL_WARNING, error);
				result->msg=strdup(error);
		} 
		close(s);
	...
	if(len == -1)
	{
		...
		{
			case 	EINTR:
					zbx_snprintf(error,MAX_STRING_LEN,"Timeout while receiving data from [%s]",item->host);
					zabbix_log(LOG_LEVEL_WARNING, error);
					result->msg=strdup(error);
					break;
			case	ECONNRESET:
					zbx_snprintf(error,MAX_STRING_LEN,"Connection reset by peer.");
					zabbix_log(LOG_LEVEL_WARNING, error);
					result->msg=strdup(error);
					close(s);
					return	NETWORK_ERROR;
			default:
				zbx_snprintf(error,MAX_STRING_LEN,"Error while receiving data from [%s] [%s]",item->host, strerror(errno));
				zabbix_log(LOG_LEVEL_WARNING, error);
				result->msg=strdup(error);
		} 
		close(s);
...
/*	if(item->itemid == 17828)
	...
	{
		zbx_snprintf(error,MAX_STRING_LEN,"Not supported by ZABBIX agent");
		result->msg=strdup(error);
		return NOTSUPPORTED;
	}
	else if( strcmp(c,"ZBX_ERROR") == 0)
	{
		zbx_snprintf(error,MAX_STRING_LEN,"ZABBIX agent non-critical error");
		result->msg=strdup(error);
		return AGENT_ERROR;
	}
	...
	{
		zbx_snprintf(error,MAX_STRING_LEN,"Got empty string from [%s] IP [%s] Parameter [%s]", item->host, item->ip, item->key);
		zabbix_log( LOG_LEVEL_WARNING, error);
	...
	{
		zbx_snprintf(error,MAX_STRING_LEN, "Type of received value [%s] is not sutable for [%s@%s] having type [%d]", c, item->key, item->host, item->value_type);
		zabbix_log( LOG_LEVEL_WARNING, error);
