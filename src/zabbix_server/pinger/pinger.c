		...
		int ping_result, char *error)
{
	...
	if (NOTSUPPORTED == ping_result)
	{
		...
		dc_add_history(item.itemid, item.value_type, item.flags, NULL, ts, item.state, error, NULL);
	}
	else
	{
		init_result(&value);
		...
		dc_add_history(item.itemid, item.value_type, item.flags, &value, ts, item.state, NULL, NULL);
