 ...
 *                                                                            *
 ******************************************************************************/
static int	lld_items_macro_paths_save(zbx_uint64_t hostid, zbx_vector_lld_item_full_ptr_t *items, int *host_locked)
{
		...
		int lastcheck)
{
	...
	zbx_db_begin();
	lld_items_get(&item_prototypes, &items, ZBX_FLAG_DISCOVERY_NORMAL);
	zbx_db_commit();
