...
static void	lld_queue_request(zbx_lld_manager_t *manager, const zbx_ipc_message_t *message)
{
	...
	{
		if ((((NULL != data->value && NULL != rule->tail->value) &&
				(NULL == data->error && NULL == rule->tail->error) &&
						0 == strcmp(data->value, rule->tail->value)) ||
				((NULL == data->value && NULL == rule->tail->value) &&
						(NULL != data->error && NULL != rule->tail->error &&
								0 == strcmp(data->error, rule->tail->error)))) &&
				data->meta == rule->tail->meta)
		{
