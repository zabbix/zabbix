		...
		const char *tls_psk, const zbx_vector_uint64_t *del_hostgroupids)
{
	...
	for (i = 0; i < hosts->values_num; i++)
	{
		...
		else
		{
			...
			{
				zbx_audit_host_prototype_create_entry(AUDIT_ACTION_UPDATE, host->hostid, host->name);
			}
...
static void	lld_templates_link(const zbx_vector_ptr_t *hosts, char **error)
{
	...
	for (i = 0; i < hosts->values_num; i++)
	{
		...
		{
			if (SUCCEED != DBdelete_template_elements_for_lld(host->hostid, &host->del_templateids, &err))
			{
