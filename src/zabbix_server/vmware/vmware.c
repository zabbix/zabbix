...
static zbx_vmware_cluster_t	*vmware_cluster_shared_dup(const zbx_vmware_cluster_t *src)
{
	...
	zbx_vector_str_reserve(&cluster->alarm_ids, src->alarm_ids.values_num);
...
static zbx_vmware_datastore_t	*vmware_datastore_shared_dup(const zbx_vmware_datastore_t *src)
{
	...
	zbx_vector_str_reserve(&datastore->alarm_ids, src->alarm_ids.values_num);
...
static zbx_vmware_datacenter_t	*vmware_datacenter_shared_dup(const zbx_vmware_datacenter_t *src)
{
	...
	zbx_vector_str_reserve(&datacenter->alarm_ids, src->alarm_ids.values_num);
...
static zbx_vmware_vm_t	*vmware_vm_shared_dup(const zbx_vmware_vm_t *src)
{
	...
	zbx_vector_str_reserve(&vm->alarm_ids, src->alarm_ids.values_num);
...
static	void	vmware_hv_shared_copy(zbx_vmware_hv_t *dst, const zbx_vmware_hv_t *src)
{
	...
	zbx_vector_str_reserve(&dst->alarm_ids, src->alarm_ids.values_num);
...
static zbx_vmware_data_t	*vmware_data_shared_dup(zbx_vmware_data_t *src)
{
	...
	zbx_vector_str_reserve(&data->alarm_ids, src->alarm_ids.values_num);
...
#	define ZBX_POST_VMWARE_GET_ALARMS						\
	...
	if (FAIL == zbx_soap_post(__func__, easyhandle, tmp, &doc_details, error))
		goto out;
	...
	ret = zbx_vector_vmware_alarm_details_bsearch(details, &cmp, ZBX_DEFAULT_STR_PTR_COMPARE_FUNC);
out:
		...
		zbx_vmware_alarms_data_t *alarms_data, char **error)
{
	...
	if (NULL == node && NULL == (node = zbx_xml_doc_get(xdoc, ZBX_XPATH_PROP_NAME_NODE("triggeredAlarmState"))))
		goto clean;
...
static zbx_vmware_vm_t	*vmware_service_create_vm(zbx_vmware_service_t *service, CURL *easyhandle,
		const char *id, zbx_vector_vmware_resourcepool_t *rpools, zbx_vmware_alarms_data_t *alarms_data,
		char **error)
{
	zbx_vmware_vm_t	*vm;
	char		*value;
...
static zbx_vmware_datastore_t	*vmware_service_create_datastore(const zbx_vmware_service_t *service, CURL *easyhandle,
		const char *id, zbx_vmware_alarms_data_t *alarms_data)
{
...
static int	vmware_service_init_hv(zbx_vmware_service_t *service, CURL *easyhandle, const char *id,
		...
		zbx_vmware_alarms_data_t *alarms_data, zbx_vmware_hv_t *hv, char **error)
{
	char				*value;
		...
		zbx_vmware_alarms_data_t *alarms_data, zbx_vector_vmware_datacenter_t *datacenters)
{
	...
	for (i = 0; i < nodeset->nodeNr; i++)
	{
		...
		if (FAIL == vmware_service_get_alarms_data(__func__, service, easyhandle, doc, nodeset->nodeTab[i],
...
#define ZBX_XPATH_GET_ROOT_ALARMS(object, idd)							\
		ZBX_XPATH_PROP_OBJECTS_ID(object, "[text()='" idd "']") "/"			\
...
static void	vmware_service_update(zbx_vmware_service_t *service)
{
	...
	if (ZBX_VMWARE_TYPE_VCENTER == service->type &&
			...
			&data->resourcepools, &alarms_data, &data->error))
	{
		goto clean;
	}
	...
	for (i = 0; i < hvs.values_num; i++)
	{
		...
		if (SUCCEED == vmware_service_init_hv(service, easyhandle, hvs.values[i], &data->datastores,
				&data->resourcepools, &alarms_data, &hv_local, &data->error))
		{
