...
static int	item_preproc_multiplier_variant(zbx_variant_t *value, const char *params, char **errmsg)
{
	...
	if (FAIL == zbx_item_preproc_convert_value_to_numeric(&value_num, value, errmsg))
		return FAIL;
		...
		zbx_item_history_value_t *history_value, char **errmsg)
{
	...
	if (SUCCEED == item_preproc_delta(item, value, ts, ZBX_PREPROC_DELTA_VALUE, history_value, &err))
		return SUCCEED;
		...
		zbx_item_history_value_t *history_value, char **errmsg)
{
	...
	if (SUCCEED == item_preproc_delta(item, value, ts, ZBX_PREPROC_DELTA_SPEED, history_value, &err))
		return SUCCEED;
		...
		const zbx_item_preproc_t *op, zbx_item_history_value_t *history_value, char **errmsg)
{
	switch (op->type)
	{
		...
		case ZBX_PREPROC_DELTA_VALUE:
			return item_preproc_delta_value(item, value, ts, history_value, errmsg);
		case ZBX_PREPROC_DELTA_SPEED:
			return item_preproc_delta_speed(item, value, ts, history_value, errmsg);
	}
