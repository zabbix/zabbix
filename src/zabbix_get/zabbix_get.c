...
static int	get_value(const char *source_ip, const char *host, unsigned short port, const char *key)
{
	...
	if (SUCCEED == (ret = zbx_tcp_connect(&s, source_ip, host, port, CONFIG_GET_TIMEOUT,
			config_tls->connect_mode, tls_arg1, tls_arg2)))
	{
...
int	main(int argc, char **argv)
{
...
#if defined(HAVE_GNUTLS) || defined(HAVE_OPENSSL)
			case '1':
				config_tls->connect = zbx_strdup(config_tls->connect,
						zbx_optarg);
				break;
			case '2':
				config_tls->ca_file = zbx_strdup(config_tls->ca_file,
						zbx_optarg);
				break;
			case '3':
				config_tls->crl_file = zbx_strdup(config_tls->crl_file,
						zbx_optarg);
				break;
			case '4':
				config_tls->server_cert_issuer =
						zbx_strdup(config_tls->server_cert_issuer, zbx_optarg);
				break;
			case '5':
				config_tls->server_cert_subject =
						zbx_strdup(config_tls->server_cert_subject, zbx_optarg);
				break;
			case '6':
				config_tls->cert_file = zbx_strdup(config_tls->cert_file,
						zbx_optarg);
				break;
			case '7':
				config_tls->key_file = zbx_strdup(config_tls->key_file,
						zbx_optarg);
				break;
			case '8':
				config_tls->psk_identity = zbx_strdup(config_tls->psk_identity,
						zbx_optarg);
				break;
			case '9':
				config_tls->psk_file = zbx_strdup(config_tls->psk_file,
						zbx_optarg);
				break;
			case 'A':
#if defined(HAVE_OPENSSL)
				config_tls->cipher_cmd13 =
						zbx_strdup(config_tls->cipher_cmd13, zbx_optarg);
				...
				break;
			case 'B':
				config_tls->cipher_cmd = zbx_strdup(config_tls->cipher_cmd,
						zbx_optarg);
				break;
#else
			case '1':
	...
	if (NULL != config_tls->connect || NULL != config_tls->ca_file || NULL != config_tls->crl_file ||
			NULL != config_tls->server_cert_issuer || NULL != config_tls->server_cert_subject ||
			NULL != config_tls->cert_file || NULL != config_tls->key_file ||
			NULL != config_tls->psk_identity || NULL != config_tls->psk_file ||
			NULL != config_tls->cipher_cmd13 || NULL != config_tls->cipher_cmd)
	{
#if defined(HAVE_GNUTLS) || defined(HAVE_OPENSSL)
