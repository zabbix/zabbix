...
#define PREPARE_UPDATE_JSON_SNMP_INTERFACE_OP(resource)			\
	...
	zbx_snprintf(audit_key_version,  AUDIT_DETAILS_KEY_LEN, #resource ".interfaces[%lu].details.version",		\
			interfaceid);										\
	zbx_snprintf(audit_key_bulk,  AUDIT_DETAILS_KEY_LEN, #resource".interfaces[%lu].details.bulk", interfaceid);	\
	...
	zbx_snprintf(audit_key_contextname,  AUDIT_DETAILS_KEY_LEN, #resource".interfaces[%lu].details.contextname",	\
			interfaceid);										\
