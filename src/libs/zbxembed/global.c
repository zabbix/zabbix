...
static duk_ret_t	es_btoa(duk_context *ctx)
{
	...
	es_duktape_push_result_string(ctx, b64str, strlen(b64str));
	zbx_free(str);
	zbx_free(b64str);
...
static duk_ret_t	es_md5(duk_context *ctx)
{
	...
	es_duktape_push_result_string(ctx, md5sum, ZBX_MD5_DIGEST_SIZE * 2);
	zbx_free(md5sum);
	zbx_free(str);
...
static duk_ret_t	es_sha256(duk_context *ctx)
{
	...
	es_duktape_push_result_string(ctx, hash_res_stringhexes, ZBX_SHA256_DIGEST_SIZE * 2);
...
static duk_ret_t	es_hmac(duk_context *ctx)
{
	...
	es_duktape_push_result_string(ctx, out, strlen(out));
	zbx_free(out);
