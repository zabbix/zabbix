...
static void	lld_items_get(zbx_uint64_t parent_itemid, zbx_vector_ptr_t *items,
		...
		const char *ipmi_sensor, const char *snmp_community, const char *port, const char *snmpv3_securityname,
...
static void	lld_item_make(zbx_vector_ptr_t *items, const char *name_proto, const char *key_proto,
		const char *units_proto, const char *params_proto, const char *snmp_oid_proto,
		const char *description_proto, struct zbx_json_parse *jp_row)
{
...
static void	lld_items_make(zbx_vector_ptr_t *items, const char *name_proto, const char *key_proto,
		const char *units_proto, const char *params_proto, const char *snmp_oid_proto,
		const char *description_proto, zbx_vector_ptr_t *lld_rows)
{
	int	i;
	...
	for (i = 0; i < lld_rows->values_num; i++)
	{
		...
		lld_item_make(items, name_proto, key_proto, units_proto, params_proto, snmp_oid_proto,
				description_proto, &lld_row->jp_row);
	}
		...
		const char *snmpv3_contextname)
{
	...
	char		*sql = NULL,
			...
			*name_esc, *key_esc, *units_esc, *params_esc, *snmp_oid_esc, *description_esc;
		...
		unsigned short lifetime, int lastcheck)
{
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		const char	*name_proto, *key_proto, *units_proto, *params_proto, *snmp_oid_proto, *delay_flex,
				*trapper_hosts, *formula, *logtimefmt, *ipmi_sensor, *snmp_community, *port,
		...
		lld_items_get(parent_itemid, &items, type, value_type, data_type, delay, delay_flex, history, trends,
				trapper_hosts, multiplier, delta, formula, logtimefmt, valuemapid, ipmi_sensor,
		...
		lld_items_make(&items, name_proto, key_proto, units_proto, params_proto, snmp_oid_proto,
				description_proto, lld_rows);
