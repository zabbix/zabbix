		...
		const zbx_lld_row_t *lld_row, const zbx_vector_ptr_t *lld_macros)
{
	...
	if (NULL != (graph = lld_graph_get(graphs, &lld_row->item_links)))
	{
		...
		substitute_lld_macros(&buffer, jp_row, lld_macros, ZBX_MACRO_SIMPLE, NULL, 0);
	...
	else
	{
		...
		graph->name_orig = NULL;
		substitute_lld_macros(&graph->name, jp_row, lld_macros, ZBX_MACRO_SIMPLE, NULL, 0);
...
static void	lld_graphs_make(const zbx_vector_ptr_t *gitems_proto, zbx_vector_ptr_t *graphs, zbx_vector_ptr_t *items,
		const char *name_proto, zbx_uint64_t ymin_itemid_proto, zbx_uint64_t ymax_itemid_proto,
		const zbx_vector_ptr_t *lld_rows, const zbx_vector_ptr_t *lld_macros)
{
	int	i;
