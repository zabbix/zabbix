...
int	zbx_db_init(zbx_dc_get_nextid_func_t cb_nextid, unsigned char program, const int log_slow_queries, char **error)
{
	zbx_cb_nextid = cb_nextid;

	if (ZBX_PROGRAM_TYPE_SERVER != program)
		return zbx_db_init_basic(zbx_cfg_dbhigh->config_dbname, db_schema, log_slow_queries, error);
