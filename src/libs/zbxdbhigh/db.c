...
/*	zbx_uint64_t	eventid_max=0;
	...
	zbx_snprintf(sql,sizeof(sql),"select eventid,value,clock from events where source=%d and object=%d and objectid=" ZBX_FS_UI64 " order by eventid desc",
...
int	DBupdate_item_status_to_notsupported(zbx_uint64_t itemid, char *error)
{
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		DBupdate_trigger_value(&trigger, TRIGGER_VALUE_UNKNOWN, now, error);
	}
