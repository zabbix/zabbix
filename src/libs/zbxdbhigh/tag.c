		...
		unsigned char tag_automatic)
{
	...
	tag = (zbx_db_host_tag_t *)zbx_malloc(NULL, sizeof(zbx_db_host_tag_t));
	tag->flags = ZBX_FLAG_DB_TAG_UNSET;
	tag->tag = zbx_strdup(NULL, tag_tag);
	tag->value = zbx_strdup(NULL, tag_value);
...
{
	const zbx_db_host_tag_t	* const tag1 = *(const zbx_db_host_tag_t * const *)d1;
	const zbx_db_host_tag_t	* const tag2 = *(const zbx_db_host_tag_t * const *)d2;
	int			ret;
...
ZBX_PTR_VECTOR_IMPL(db_host_tag_ptr, zbx_db_host_tag_t *)
