		...
		const char *opdata)
{
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		zbx_snprintf_alloc(&sql, &sql_alloc, &sql_offset,
				...
				(int)manual_close, opdata_esc, h_triggerid);
...
static int	DBcopy_template_triggers(zbx_uint64_t hostid, const zbx_vector_uint64_t *templateids)
{
	...
	zbx_strcpy_alloc(&sql, &sql_alloc, &sql_offset,
			"select distinct t.triggerid,t.description,t.expression,t.status,"
				...
				"t.correlation_mode,t.correlation_tag,t.manual_close,t.opdata"
			" from triggers t,functions f,items i"
			" where t.triggerid=f.triggerid"
				" and f.itemid=i.itemid"
				" and");
		...
		unsigned char flags)
{
	...
	if (0 != hst_graphid)
	{
		zbx_snprintf_alloc(&sql, &sql_alloc, &sql_offset,
				...
				" set name='%s',"
					"width=%d,"
					"height=%d,"
					"yaxismin=" ZBX_DB_FS_DBL64_SQL ","
					"yaxismax=" ZBX_DB_FS_DBL64_SQL ","
					...
					"show_legend=%d,"
					"show_3d=%d,"
					"percent_left=" ZBX_DB_FS_DBL64_SQL ","
					"percent_right=" ZBX_DB_FS_DBL64_SQL ","
					"ymin_type=%d,"
					"ymax_type=%d,"
					"ymin_itemid=%s,"
					"ymax_itemid=%s,"
	...
	else
	{
		...
		zbx_snprintf_alloc(&sql, &sql_alloc, &sql_offset,
				...
				" values (" ZBX_FS_UI64 ",'%s',%d,%d," ZBX_DB_FS_DBL64_SQL ","
				ZBX_DB_FS_DBL64_SQL "," ZBX_FS_UI64 ",%d,%d,%d,%d,%d," ZBX_DB_FS_DBL64_SQL ","
				ZBX_DB_FS_DBL64_SQL ",%d,%d,%s,%s,%d);\n",
