		...
		size_t *full_len, size_t *cutoff)
{
	...
	if (str_cutoff)	/* cut utf8 sequence if exists at the end */
	{
		size_t to_cut = 0;
		...
		*cutoff = (str_cutoff - string) - 1;
		...
		else if (0xe0 <= (unsigned char)*str_cutoff && 2 > to_cut)	/* cut in 3 byte sequence */
		{
			len_cutoff -= to_cut + 1;
			*cutoff -= to_cut + 1;
		}
		else if (0xc0 <= (unsigned char)*str_cutoff && 1 > to_cut)	/* cut in 2 byte sequence */
		{
			len_cutoff--;
			(*cutoff)--;
		}
...
{
	return __zbx_json_stringsize_limit(string, type, 0, NULL, NULL);
}
		...
		struct zbx_json *j, const char *name, const char *string, zbx_json_type_t type, size_t max_size)
{
	...
	len += __zbx_json_stringsize_limit(string, type, max_size, &orig_len, &str_len);
