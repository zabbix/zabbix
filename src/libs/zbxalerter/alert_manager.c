 ...
 *                                                                            *
 ******************************************************************************/
static void	am_queue_watchdog_alerts(zbx_am_t *manager, const zbx_config_dbhigh_t *config_dbhigh, int time_ping)
{
...
#define ZBX_AM_MEDIATYPE_CLEANUP_FREQUENCY	SEC_PER_HOUR
	...
	while (ZBX_IS_RUNNING())
	{
		...
		if (ZBX_DB_DOWN == manager.dbstatus)
		{
			...
			{
				am_queue_watchdog_alerts(&manager, alert_manager_args_in->config_dbhigh, time_ping);
				time_watchdog = now;
			}
		}
