...
static void	DBpatch_init_dashboard_template(zbx_db_dashboard_t *dashboard, char *name, uint64_t templateid)
{
...
static int	DBpatch_convert_screen_items(DB_RESULT result, uint64_t id, uint64_t templateid)
{
	DB_ROW			row;
	int			i, ret = SUCCEED;
...
int	DBpatch_convert_screen(uint64_t screenid, char *name, uint32_t templateid, uint64_t userid, int private)
{
	...
	if (0 != templateid)
	{
		DBpatch_init_dashboard_template(&dashboard, name, templateid);
	}
	else
	{
	...
	if (SUCCEED == ret)
		ret = DBpatch_convert_screen_items(result, id, templateid);
...
int	DBpatch_convert_slideshow(uint64_t slideshowid, char *name, int delay, uint64_t userid, int private)
{
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		if (NULL != result3)
			DBpatch_convert_screen_items(result3, dashboard_page.dashboard_pageid, 0);
