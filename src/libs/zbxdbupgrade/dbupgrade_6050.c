	...
	const zbx_db_table_t	table = {"proxy", "proxyid", 0,
			{
				...
				{"mode", "0", NULL, NULL, 0, ZBX_TYPE_INT, ZBX_NOTNULL | ZBX_PROXY, 0},
...
{
	return DBcreate_index("proxy", "proxy_1", "name", 0);
}
...
static int	DBpatch_6050044(void)
{
	const zbx_db_table_t	table = {"proxy_rtdata", "proxyid", 0,
			{
				{"proxyid", NULL, "proxy", "proxyid", 0, ZBX_TYPE_ID, ZBX_NOTNULL,
...
static int	DBpatch_6050046(void)
{
	...
	return zbx_db_insert_execute(&db_insert_rtdata);
}
