...
static int	dbsync_compare_host_inventory(const ZBX_DC_HOST_INVENTORY *hi, const DB_ROW dbrow)
{
	...
	if (dbsync_compare_str(dbrow[2], hi->type))
		return FAIL;

	if (dbsync_compare_str(dbrow[3], hi->type_full))
		return FAIL;

	if (dbsync_compare_str(dbrow[4], hi->name))
		return FAIL;

	if (dbsync_compare_str(dbrow[5], hi->alias))
		return FAIL;

	if (dbsync_compare_str(dbrow[6], hi->os))
		return FAIL;

	if (dbsync_compare_str(dbrow[7], hi->os_full))
		return FAIL;

	if (dbsync_compare_str(dbrow[8], hi->os_short))
		return FAIL;

	if (dbsync_compare_str(dbrow[9], hi->serialno_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[10], hi->serialno_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[11], hi->tag))
		return FAIL;

	if (dbsync_compare_str(dbrow[12], hi->asset_tag))
		return FAIL;

	if (dbsync_compare_str(dbrow[13], hi->macaddress_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[14], hi->macaddress_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[15], hi->hardware))
		return FAIL;

	if (dbsync_compare_str(dbrow[16], hi->hardware_full))
		return FAIL;

	if (dbsync_compare_str(dbrow[17], hi->software))
		return FAIL;

	if (dbsync_compare_str(dbrow[18], hi->software_full))
		return FAIL;

	if (dbsync_compare_str(dbrow[19], hi->software_app_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[20], hi->software_app_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[21], hi->software_app_c))
		return FAIL;

	if (dbsync_compare_str(dbrow[22], hi->software_app_d))
		return FAIL;

	if (dbsync_compare_str(dbrow[23], hi->software_app_e))
		return FAIL;

	if (dbsync_compare_str(dbrow[24], hi->contact))
		return FAIL;

	if (dbsync_compare_str(dbrow[25], hi->location))
		return FAIL;

	if (dbsync_compare_str(dbrow[26], hi->location_lat))
		return FAIL;

	if (dbsync_compare_str(dbrow[27], hi->location_lon))
		return FAIL;

	if (dbsync_compare_str(dbrow[28], hi->notes))
		return FAIL;

	if (dbsync_compare_str(dbrow[29], hi->chassis))
		return FAIL;

	if (dbsync_compare_str(dbrow[30], hi->model))
		return FAIL;

	if (dbsync_compare_str(dbrow[31], hi->hw_arch))
		return FAIL;

	if (dbsync_compare_str(dbrow[32], hi->vendor))
		return FAIL;

	if (dbsync_compare_str(dbrow[33], hi->contract_number))
		return FAIL;

	if (dbsync_compare_str(dbrow[34], hi->installer_name))
		return FAIL;

	if (dbsync_compare_str(dbrow[35], hi->deployment_status))
		return FAIL;

	if (dbsync_compare_str(dbrow[36], hi->url_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[37], hi->url_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[38], hi->url_c))
		return FAIL;

	if (dbsync_compare_str(dbrow[39], hi->host_networks))
		return FAIL;

	if (dbsync_compare_str(dbrow[40], hi->host_netmask))
		return FAIL;

	if (dbsync_compare_str(dbrow[41], hi->host_router))
		return FAIL;

	if (dbsync_compare_str(dbrow[42], hi->oob_ip))
		return FAIL;

	if (dbsync_compare_str(dbrow[43], hi->oob_netmask))
		return FAIL;

	if (dbsync_compare_str(dbrow[44], hi->oob_router))
		return FAIL;

	if (dbsync_compare_str(dbrow[45], hi->date_hw_purchase))
		return FAIL;

	if (dbsync_compare_str(dbrow[46], hi->date_hw_install))
		return FAIL;

	if (dbsync_compare_str(dbrow[47], hi->date_hw_expiry))
		return FAIL;

	if (dbsync_compare_str(dbrow[48], hi->date_hw_decomm))
		return FAIL;

	if (dbsync_compare_str(dbrow[49], hi->site_address_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[50], hi->site_address_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[51], hi->site_address_c))
		return FAIL;

	if (dbsync_compare_str(dbrow[52], hi->site_city))
		return FAIL;

	if (dbsync_compare_str(dbrow[53], hi->site_state))
		return FAIL;

	if (dbsync_compare_str(dbrow[54], hi->site_country))
		return FAIL;

	if (dbsync_compare_str(dbrow[55], hi->site_zip))
		return FAIL;

	if (dbsync_compare_str(dbrow[56], hi->site_rack))
		return FAIL;

	if (dbsync_compare_str(dbrow[57], hi->site_notes))
		return FAIL;

	if (dbsync_compare_str(dbrow[58], hi->poc_1_name))
		return FAIL;

	if (dbsync_compare_str(dbrow[59], hi->poc_1_email))
		return FAIL;

	if (dbsync_compare_str(dbrow[60], hi->poc_1_phone_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[61], hi->poc_1_phone_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[62], hi->poc_1_cell))
		return FAIL;

	if (dbsync_compare_str(dbrow[63], hi->poc_1_screen))
		return FAIL;

	if (dbsync_compare_str(dbrow[64], hi->poc_1_notes))
		return FAIL;

	if (dbsync_compare_str(dbrow[65], hi->poc_2_name))
		return FAIL;

	if (dbsync_compare_str(dbrow[66], hi->poc_2_email))
		return FAIL;

	if (dbsync_compare_str(dbrow[67], hi->poc_2_phone_a))
		return FAIL;

	if (dbsync_compare_str(dbrow[68], hi->poc_2_phone_b))
		return FAIL;

	if (dbsync_compare_str(dbrow[69], hi->poc_2_cell))
		return FAIL;

	if (dbsync_compare_str(dbrow[70], hi->poc_2_screen))
		return FAIL;

	if (dbsync_compare_str(dbrow[71], hi->poc_2_notes))
		return FAIL;
