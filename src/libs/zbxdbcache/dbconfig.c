...
static void	DCsync_host_inventory(zbx_dbsync_t *sync)
{
	...
	while (SUCCEED == (ret = zbx_dbsync_next(sync, &rowid, &row, &tag)))
	{
		...
		DCstrpool_replace(found, &host_inventory->type, row[2]);
		DCstrpool_replace(found, &host_inventory->type_full, row[3]);
		DCstrpool_replace(found, &host_inventory->name, row[4]);
		DCstrpool_replace(found, &host_inventory->alias, row[5]);
		DCstrpool_replace(found, &host_inventory->os, row[6]);
		DCstrpool_replace(found, &host_inventory->os_full, row[7]);
		DCstrpool_replace(found, &host_inventory->os_short, row[8]);
		DCstrpool_replace(found, &host_inventory->serialno_a, row[9]);
		DCstrpool_replace(found, &host_inventory->serialno_b, row[10]);
		DCstrpool_replace(found, &host_inventory->tag, row[11]);
		DCstrpool_replace(found, &host_inventory->asset_tag, row[12]);
		DCstrpool_replace(found, &host_inventory->macaddress_a, row[13]);
		DCstrpool_replace(found, &host_inventory->macaddress_b, row[14]);
		DCstrpool_replace(found, &host_inventory->hardware, row[15]);
		DCstrpool_replace(found, &host_inventory->hardware_full, row[16]);
		DCstrpool_replace(found, &host_inventory->software, row[17]);
		DCstrpool_replace(found, &host_inventory->software_full, row[18]);
		DCstrpool_replace(found, &host_inventory->software_app_a, row[19]);
		DCstrpool_replace(found, &host_inventory->software_app_b, row[20]);
		DCstrpool_replace(found, &host_inventory->software_app_c, row[21]);
		DCstrpool_replace(found, &host_inventory->software_app_d, row[22]);
		DCstrpool_replace(found, &host_inventory->software_app_e, row[23]);
		DCstrpool_replace(found, &host_inventory->contact, row[24]);
		DCstrpool_replace(found, &host_inventory->location, row[25]);
		DCstrpool_replace(found, &host_inventory->location_lat, row[26]);
		DCstrpool_replace(found, &host_inventory->location_lon, row[27]);
		DCstrpool_replace(found, &host_inventory->notes, row[28]);
		DCstrpool_replace(found, &host_inventory->chassis, row[29]);
		DCstrpool_replace(found, &host_inventory->model, row[30]);
		DCstrpool_replace(found, &host_inventory->hw_arch, row[31]);
		DCstrpool_replace(found, &host_inventory->vendor, row[32]);
		DCstrpool_replace(found, &host_inventory->contract_number, row[33]);
		DCstrpool_replace(found, &host_inventory->installer_name, row[34]);
		DCstrpool_replace(found, &host_inventory->deployment_status, row[35]);
		DCstrpool_replace(found, &host_inventory->url_a, row[36]);
		DCstrpool_replace(found, &host_inventory->url_b, row[37]);
		DCstrpool_replace(found, &host_inventory->url_c, row[38]);
		DCstrpool_replace(found, &host_inventory->host_networks, row[39]);
		DCstrpool_replace(found, &host_inventory->host_netmask, row[40]);
		DCstrpool_replace(found, &host_inventory->host_router, row[41]);
		DCstrpool_replace(found, &host_inventory->oob_ip, row[42]);
		DCstrpool_replace(found, &host_inventory->oob_netmask, row[43]);
		DCstrpool_replace(found, &host_inventory->oob_router, row[44]);
		DCstrpool_replace(found, &host_inventory->date_hw_purchase, row[45]);
		DCstrpool_replace(found, &host_inventory->date_hw_install, row[46]);
		DCstrpool_replace(found, &host_inventory->date_hw_expiry, row[47]);
		DCstrpool_replace(found, &host_inventory->date_hw_decomm, row[48]);
		DCstrpool_replace(found, &host_inventory->site_address_a, row[49]);
		DCstrpool_replace(found, &host_inventory->site_address_b, row[50]);
		DCstrpool_replace(found, &host_inventory->site_address_c, row[51]);
		DCstrpool_replace(found, &host_inventory->site_city, row[52]);
		DCstrpool_replace(found, &host_inventory->site_state, row[53]);
		DCstrpool_replace(found, &host_inventory->site_country, row[54]);
		DCstrpool_replace(found, &host_inventory->site_zip, row[55]);
		DCstrpool_replace(found, &host_inventory->site_rack, row[56]);
		DCstrpool_replace(found, &host_inventory->site_notes, row[57]);
		DCstrpool_replace(found, &host_inventory->poc_1_name, row[58]);
		DCstrpool_replace(found, &host_inventory->poc_1_email, row[59]);
		DCstrpool_replace(found, &host_inventory->poc_1_phone_a, row[60]);
		DCstrpool_replace(found, &host_inventory->poc_1_phone_b, row[61]);
		DCstrpool_replace(found, &host_inventory->poc_1_cell, row[62]);
		DCstrpool_replace(found, &host_inventory->poc_1_screen, row[63]);
		DCstrpool_replace(found, &host_inventory->poc_1_notes, row[64]);
		DCstrpool_replace(found, &host_inventory->poc_2_name, row[65]);
		DCstrpool_replace(found, &host_inventory->poc_2_email, row[66]);
		DCstrpool_replace(found, &host_inventory->poc_2_phone_a, row[67]);
		DCstrpool_replace(found, &host_inventory->poc_2_phone_b, row[68]);
		DCstrpool_replace(found, &host_inventory->poc_2_cell, row[69]);
		DCstrpool_replace(found, &host_inventory->poc_2_screen, row[70]);
		DCstrpool_replace(found, &host_inventory->poc_2_notes, row[71]);
	}
	...
	for (; SUCCEED == ret; ret = zbx_dbsync_next(sync, &rowid, &row, &tag))
	{
		...
		zbx_strpool_release(host_inventory->type);
		zbx_strpool_release(host_inventory->type_full);
		zbx_strpool_release(host_inventory->name);
		zbx_strpool_release(host_inventory->alias);
		zbx_strpool_release(host_inventory->os);
		zbx_strpool_release(host_inventory->os_full);
		zbx_strpool_release(host_inventory->os_short);
		zbx_strpool_release(host_inventory->serialno_a);
		zbx_strpool_release(host_inventory->serialno_b);
		zbx_strpool_release(host_inventory->tag);
		zbx_strpool_release(host_inventory->asset_tag);
		zbx_strpool_release(host_inventory->macaddress_a);
		zbx_strpool_release(host_inventory->macaddress_b);
		zbx_strpool_release(host_inventory->hardware);
		zbx_strpool_release(host_inventory->hardware_full);
		zbx_strpool_release(host_inventory->software);
		zbx_strpool_release(host_inventory->software_full);
		zbx_strpool_release(host_inventory->software_app_a);
		zbx_strpool_release(host_inventory->software_app_b);
		zbx_strpool_release(host_inventory->software_app_c);
		zbx_strpool_release(host_inventory->software_app_d);
		zbx_strpool_release(host_inventory->software_app_e);
		zbx_strpool_release(host_inventory->contact);
		zbx_strpool_release(host_inventory->location);
		zbx_strpool_release(host_inventory->location_lat);
		zbx_strpool_release(host_inventory->location_lon);
		zbx_strpool_release(host_inventory->notes);
		zbx_strpool_release(host_inventory->chassis);
		zbx_strpool_release(host_inventory->model);
		zbx_strpool_release(host_inventory->hw_arch);
		zbx_strpool_release(host_inventory->vendor);
		zbx_strpool_release(host_inventory->contract_number);
		zbx_strpool_release(host_inventory->installer_name);
		zbx_strpool_release(host_inventory->deployment_status);
		zbx_strpool_release(host_inventory->url_a);
		zbx_strpool_release(host_inventory->url_b);
		zbx_strpool_release(host_inventory->url_c);
		zbx_strpool_release(host_inventory->host_networks);
		zbx_strpool_release(host_inventory->host_netmask);
		zbx_strpool_release(host_inventory->host_router);
		zbx_strpool_release(host_inventory->oob_ip);
		zbx_strpool_release(host_inventory->oob_netmask);
		zbx_strpool_release(host_inventory->oob_router);
		zbx_strpool_release(host_inventory->date_hw_purchase);
		zbx_strpool_release(host_inventory->date_hw_install);
		zbx_strpool_release(host_inventory->date_hw_expiry);
		zbx_strpool_release(host_inventory->date_hw_decomm);
		zbx_strpool_release(host_inventory->site_address_a);
		zbx_strpool_release(host_inventory->site_address_b);
		zbx_strpool_release(host_inventory->site_address_c);
		zbx_strpool_release(host_inventory->site_city);
		zbx_strpool_release(host_inventory->site_state);
		zbx_strpool_release(host_inventory->site_country);
		zbx_strpool_release(host_inventory->site_zip);
		zbx_strpool_release(host_inventory->site_rack);
		zbx_strpool_release(host_inventory->site_notes);
		zbx_strpool_release(host_inventory->poc_1_name);
		zbx_strpool_release(host_inventory->poc_1_email);
		zbx_strpool_release(host_inventory->poc_1_phone_a);
		zbx_strpool_release(host_inventory->poc_1_phone_b);
		zbx_strpool_release(host_inventory->poc_1_cell);
		zbx_strpool_release(host_inventory->poc_1_screen);
		zbx_strpool_release(host_inventory->poc_1_notes);
		zbx_strpool_release(host_inventory->poc_2_name);
		zbx_strpool_release(host_inventory->poc_2_email);
		zbx_strpool_release(host_inventory->poc_2_phone_a);
		zbx_strpool_release(host_inventory->poc_2_phone_b);
		zbx_strpool_release(host_inventory->poc_2_cell);
		zbx_strpool_release(host_inventory->poc_2_screen);
		zbx_strpool_release(host_inventory->poc_2_notes);
	}
