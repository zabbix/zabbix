...
#	define	ZBX_BACKTRACE_SIZE	60
	...
	if (0 != (flags & ZBX_FATAL_LOG_PC_REG_SF))
	{
		...
		for (i = 16; i >= 2; i--)
		{
			zabbix_log(LOG_LEVEL_CRIT, "+0x%02x(%%ebp) = ebp + %2u = %08x = %10u = %11d%s",
					i * ZBX_PTR_SIZE, i * ZBX_PTR_SIZE,
					*(unsigned int *)((void *)ZBX_GET_REG(uctx, REG_EBP) + i * ZBX_PTR_SIZE),
					*(unsigned int *)((void *)ZBX_GET_REG(uctx, REG_EBP) + i * ZBX_PTR_SIZE),
					*(int *)((void *)ZBX_GET_REG(uctx, REG_EBP) + i * ZBX_PTR_SIZE),
					i == 2 ? " <--- call arguments" : "");
		}
		zabbix_log(LOG_LEVEL_CRIT, "+0x%02x(%%ebp) = ebp + %2u = %08x%28s<--- return address",
