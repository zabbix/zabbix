		...
		else
		{
			if (SQLT_IBDOUBLE != data_type && SQLT_BDOUBLE != data_type)
			{
				...
				if (OCI_SUCCESS == err)
				{
					...
					{
#define ZBX_MAX_OCI_NUMBER_WIDTH	22
						/* retrieve the column width in bytes */
						err = OCIAttrGet((void *)parmdp, (ub4)OCI_DTYPE_PARAM, (void *)&col_width,
								(ub4 *)NULL, (ub4)OCI_ATTR_DATA_SIZE, (OCIError *)oracle.errhp);

						if (ZBX_MAX_OCI_NUMBER_WIDTH > col_width && SQLT_NUM == data_type)
						{
							col_width = ZBX_MAX_OCI_NUMBER_WIDTH;
						}
#undef ZBX_MAX_OCI_NUMBER_WIDTH
					}
				}
