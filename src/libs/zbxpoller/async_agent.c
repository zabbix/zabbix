...
static int	async_task_process_task_agent_cb(short event, void *data, int *fd, const char *addr, char *dnserr,
...
out:
	...
	if (ZABBIX_AGENT_STEP_CONNECT_INIT == agent_context->step)
		return async_task_process_task_agent_cb(0, data, fd, addr, dnserr, NULL);
