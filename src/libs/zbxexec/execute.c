...
static int	zbx_popen(pid_t *pid, const char *command)
{
	...
	zabbix_log(LOG_LEVEL_WARNING, "execl() failed for [%s]: %s", command, strerror(errno));
	exit(0);
}
...
int	zbx_execute(const char *command, char **buffer, char *error, size_t max_error_len, int timeout)
{
	...
	*error = '\0';
	...
	if (0 == CreateProcess(NULL, cmd, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, &si, &pi))
	{
	...
	if (-1 != (fd = zbx_popen(&pid, command)))
	{
		...
		if (-1 == rc || -1 == zbx_waitpid(pid))
		{
			if (EINTR == errno)
				...
				zbx_snprintf(error, max_error_len, "zbx_waitpid() failed: %s", strerror(errno));

			/* kill the whole process group, pid must be the leader */
			if (-1 == kill(-pid, SIGTERM))
				zabbix_log(LOG_LEVEL_ERR, "failed to kill [%s]: %s", command, strerror(errno));
