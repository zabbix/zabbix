		...
		zbx_proxy_suppress_t *nodata_win, char **info, unsigned int mode)
{
	...
	{
		/* 'last_valueid' is a local, incrementing identifier generated by the agent */
		/* it uniquely identifies each value added to the internal agent buffer, allowing the server */
		/* to detect out-of-order or duplicate values per session (host/proxy) */
		/* this value is included in data sent to the server, which compares it with the last known */
		/* id from that proxy session 'session->last_id' to ensure proper ordering and detect anomalies. */
		if (session->last_id > last_valueid)
		{
			zabbix_log(LOG_LEVEL_WARNING, "received historical identifier" ZBX_FS_UI64 "from proxy "
					ZBX_FS_UI64" is lower than the last processed historical identifier "
