...
int	zbx_check_service_default_addr(AGENT_REQUEST *request, const char *default_addr, AGENT_RESULT *result, int perf)
{
	...
	if (0 == strncmp("net.tcp.service", get_rkey(request), 15))
	{
		if (0 == strcmp(service, "ssh"))
		{
			...
			ret = check_ssh(ip, port, get_config_timeout_cb(), &value_int);
		}
		else if (0 == strcmp(service, "ldap"))
		{
			...
			ret = check_ldap(ip, port, get_config_timeout_cb(), &value_int);
#else
		...
		else if (0 == strcmp(service, "smtp"))
		{
			...
			ret = tcp_expect(ip, port, get_config_timeout_cb(), NULL, validate_smtp, "QUIT\r\n",
					&value_int);
		}
		else if (0 == strcmp(service, "ftp"))
		{
			...
			ret = tcp_expect(ip, port, get_config_timeout_cb(), NULL, validate_ftp, "QUIT\r\n",
					&value_int);
		}
		else if (0 == strcmp(service, "http"))
		{
			...
			ret = tcp_expect(ip, port, get_config_timeout_cb(), NULL, NULL, NULL, &value_int);
		}
		else if (0 == strcmp(service, "pop"))
		{
			if (NULL == port_str || '\0' == *port_str)
				port = ZBX_DEFAULT_POP_PORT;
			ret = tcp_expect(ip, port, get_config_timeout_cb(), NULL, validate_pop, "QUIT\r\n",
					&value_int);
		}
		else if (0 == strcmp(service, "nntp"))
		{
			...
			ret = tcp_expect(ip, port, get_config_timeout_cb(), NULL, validate_nntp, "QUIT\r\n",
					&value_int);
		}
		else if (0 == strcmp(service, "imap"))
		{
			...
			ret = tcp_expect(ip, port, get_config_timeout_cb(), NULL, validate_imap, "a1 LOGOUT\r\n",
					&value_int);
		}
		...
		else if (0 == strcmp(service, "https"))
		{
			...
			ret = check_https(ip, port, get_config_timeout_cb(), &value_int);
#else
		...
		else if (0 == strcmp(service, "telnet"))
		{
			...
			ret = check_telnet(ip, port, get_config_timeout_cb(), &value_int);
		}
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid first parameter."));
	...
	else	/* net.udp.service */
	{
		if (0 == strcmp(service, "ntp"))
		{
			...
			ret = check_ntp(ip, port, get_config_timeout_cb(), &value_int);
		}
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid first parameter."));
