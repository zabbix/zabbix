...
int	set_result_type(AGENT_RESULT *result, int value_type, char *c)
{
	...
	switch (value_type)
	{
		...
		case ITEM_VALUE_TYPE_FLOAT:
			...
			if (SUCCEED == is_double(c, ZBX_FLAG_DOUBLE_PLAIN))
			{
...
static double	*get_result_dbl_value(AGENT_RESULT *result)
{
	...
	else if (0 != ISSET_STR(result))
	{
		...
		if (SUCCEED != is_double(result->str, ZBX_FLAG_DOUBLE_PLAIN))
			return NULL;
	...
	else if (0 != ISSET_TEXT(result))
	{
		...
		if (SUCCEED != is_double(result->text, ZBX_FLAG_DOUBLE_PLAIN))
			return NULL;
