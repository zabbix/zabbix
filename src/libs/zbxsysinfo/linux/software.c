...
int	system_sw_packages(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	*buffer = '\0';
	...
	for (i = 0; NULL != package_managers[i].name; i++)
	{
		...
		if (SUCCEED == zbx_execute(mng->test_cmd, &buf, tmp, sizeof(tmp), get_config_timeout_cb(),
				ZBX_EXIT_CODE_CHECKS_DISABLED, NULL) &&
				'\0' != *buf)	/* consider this manager if test_cmd outputs anything to stdout */
		{
			if (SUCCEED != zbx_execute(mng->list_cmd, &buf, tmp, sizeof(tmp), get_config_timeout_cb(),
					ZBX_EXIT_CODE_CHECKS_DISABLED, NULL))
			{
				continue;
			}
...
int	system_sw_packages_get(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	for (i = 0; NULL != package_managers[i].name; i++)
	{
		...
		if (SUCCEED == zbx_execute(mng->test_cmd, &buf, error, sizeof(error), get_config_timeout_cb(),
				ZBX_EXIT_CODE_CHECKS_DISABLED, NULL) &&
				'\0' != *buf)	/* consider this manager if test_cmd outputs anything to stdout */
		{
			if (SUCCEED != zbx_execute(mng->details_cmd, &buf, error, sizeof(error), get_config_timeout_cb(),
					ZBX_EXIT_CODE_CHECKS_DISABLED, NULL))
			{
				continue;
			}
