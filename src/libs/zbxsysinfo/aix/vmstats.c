...
#undef ZBX_MAX_WAIT_VMSTAT
	if (!VMSTAT_COLLECTOR_STARTED(get_collector()))
	{
	...
	if (0 == (get_collector())->vmstat.data_available)
	{
		(get_collector())->vmstat.enabled = 1;

		while (wait--)
		{
			zbx_sleep(1);
			if (1 == (get_collector())->vmstat.data_available)
				break;
		}
	...
	if (0 == strcmp(section, "ent"))
	{
		...
		SET_DBL_RESULT(result, (get_collector())->vmstat.ent);
	}
	else if (NULL == type)
	{
	...
	{
		if (0 == strcmp(type, "fi"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.fi);
		else if (0 == strcmp(type, "fo"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.fo);
		else if (0 == strcmp(type, "pi"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.pi);
		else if (0 == strcmp(type, "po"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.po);
		else if (0 == strcmp(type, "fr"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.fr);
		else if (0 == strcmp(type, "sr"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.sr);
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid second parameter."));
			return SYSINFO_RET_FAIL;
	...
	{
		if (0 == strcmp(type, "in"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.in);
		else if (0 == strcmp(type, "sy"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.sy);
		else if (0 == strcmp(type, "cs"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cs);
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid second parameter."));
			return SYSINFO_RET_FAIL;
	...
	{
		if (0 == strcmp(type, "us"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_us);
		else if (0 == strcmp(type, "sy"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_sy);
		else if (0 == strcmp(type, "id"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_id);
		else if (0 == strcmp(type, "wa"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_wa);
		else if (0 == strcmp(type, "pc"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_pc);
		else if (0 == strcmp(type, "ec"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_ec);
		else if (0 == strcmp(type, "lbusy"))
		{
			if (0 == (get_collector())->vmstat.shared_enabled)
			{
				SET_MSG_RESULT(result, zbx_strdup(NULL, "logical partition type is not \"shared\"."));
				return SYSINFO_RET_FAIL;
			}

			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_lbusy);
		}
		else if (0 == strcmp(type, "app"))
		{
			...
			if (0 == (get_collector())->vmstat.pool_util_authority)
			{
				SET_MSG_RESULT(result, zbx_strdup(NULL, "pool utilization authority not set."));
				return SYSINFO_RET_FAIL;
			}

			SET_DBL_RESULT(result, (get_collector())->vmstat.cpu_app);
		}
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid second parameter."));
	...
	else if (0 == strcmp(section, "disk"))
	{
		if (0 == strcmp(type, "bps"))
			SET_UI64_RESULT(result, (get_collector())->vmstat.disk_bps);
		else if (0 == strcmp(type, "tps"))
			SET_DBL_RESULT(result, (get_collector())->vmstat.disk_tps);
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid second parameter."));
			return SYSINFO_RET_FAIL;
	...
	else if (0 == strcmp(section, "memory"))
	{
		...
		{
			if (0 != (get_collector())->vmstat.aix52stats)
			{
				SET_UI64_RESULT(result, (get_collector())->vmstat.mem_avm);
			}
			else
			{
			...
			SET_UI64_RESULT(result, (get_collector())->vmstat.mem_fre);
		else
		{
			SET_MSG_RESULT(result, zbx_strdup(NULL, "Invalid second parameter."));
			return SYSINFO_RET_FAIL;
