...
{
	double	time_left = get_config_timeout_cb() - (*time_previous_query_finished - time_first_query_started);
...
int	system_uname(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	if (0 >= get_config_timeout_cb() - (time_previous_query_finished - start_time))
	{
