	...
	switch (errcode)
	{
		case SUCCEED:
			...
			if (NULL != value)
			{
				...
				value = NULL;
				zbx_preprocess_item_value(1, itemid, poller->request->hostid, ITEM_VALUE_TYPE_TEXT,
					flags, &result, &ts, state, NULL);
		...
		case CONFIG_ERROR:
			...
			zbx_preprocess_item_value(1, itemid, poller->request->hostid, ITEM_VALUE_TYPE_TEXT, flags, NULL,
					&ts, state, value);
			break;
		default:
