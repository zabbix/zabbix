		...
		const zbx_vector_ptr_t *lld_macros, char *error, size_t max_error_len)
{
	...
	*dot = '\0';
	...
	substitute_key_macros(&key, NULL, NULL, jp_row, lld_macros, MACRO_TYPE_ITEM_KEY, NULL, 0);
	*dot = '.';
	...
	if (SUCCEED != substitute_function_lld_param(params, *data + lld_end - params + 1, 0, &replace_to,
			&replace_to_alloc, &replace_to_offset, jp_row, lld_macros, error, max_error_len))
	{
		goto out;
	}
...
static int	process_lld_macro_token(char **data, zbx_token_t *token, int flags, const struct zbx_json_parse *jp_row,
		const zbx_vector_ptr_t *lld_macros, char *error, size_t error_len)
{
		...
		const zbx_vector_ptr_t *lld_macros)
{
	...
	substitute_lld_macros(&context, jp_row, lld_macros, ZBX_TOKEN_LLD_MACRO | ZBX_TOKEN_LLD_FUNC_MACRO, NULL, 0);
		...
		const zbx_vector_ptr_t *lld_macros, char *error, size_t max_error_len)
{
	...
	ret = substitute_function_lld_param(*data + par_l + 1, par_r - (par_l + 1), 0, &exp, &exp_alloc, &exp_offset,
			jp_row, lld_macros, error, max_error_len);
	...
	while (SUCCEED == ret && SUCCEED == zbx_token_find(*data, pos, &token, ZBX_TOKEN_SEARCH_BASIC))
	{
		if (0 != (token.type & flags))
		{
			switch (token.type)
			{
				case ZBX_TOKEN_LLD_MACRO:
				case ZBX_TOKEN_LLD_FUNC_MACRO:
					ret = process_lld_macro_token(data, &token, flags, jp_row, lld_macros, error,
							max_error_len);
					pos = token.loc.r;
					break;
				case ZBX_TOKEN_USER_MACRO:
					process_user_macro_token(data, &token, jp_row, lld_macros);
					pos = token.loc.r;
					break;
				case ZBX_TOKEN_SIMPLE_MACRO:
					process_simple_macro_token(data, &token, jp_row, lld_macros, error,
							max_error_len);
					pos = token.loc.r;
					break;
				case ZBX_TOKEN_FUNC_MACRO:
					...
					{
						ret = substitute_func_macro(data, &token, jp_row, lld_macros, error,
								max_error_len);
						pos = token.loc.r;
					}
					break;
			...
			char **param)
{
	...
	const zbx_vector_ptr_t		*lld_macros = replace_key_param_data->lld_macros;
...
int	substitute_key_macros(char **data, zbx_uint64_t *hostid, DC_ITEM *dc_item, const struct zbx_json_parse *jp_row,
		const zbx_vector_ptr_t *lld_macros, int macro_type, char *error, size_t maxerrlen)
{
	...
	replace_key_param_data.lld_macros = lld_macros;
	...
	for (p = e; p < len + e ; p += sep_pos + 1)
	{
		...
		if (1 == key_in_param && p == e)
		{
			...
			if (SUCCEED != parse_host_key(param, &host, &key) ||
					SUCCEED != substitute_key_macros(&key, NULL, NULL, jp_row, lld_macros,
		...
		const zbx_vector_ptr_t *lld_macros, char *error, int maxerrlen)
{
	...
	do
	{
		...
		substitute_lld_macros(&p_name, jp_row, lld_macros, ZBX_MACRO_ANY, NULL, 0);
		substitute_lld_macros(&p_value, jp_row, lld_macros, ZBX_MACRO_ANY, NULL, 0);
...
static void	substitute_macros_in_xml_elements(const DC_ITEM *item, const struct zbx_json_parse *jp_row,
		const zbx_vector_ptr_t *lld_macros, xmlNode *node)
{
	xmlChar	*value;
	xmlAttr	*attr;
	char	*value_tmp;
	...
	for (;NULL != node; node = node->next)
	{
		switch (node->type)
		{
			case XML_TEXT_NODE:
				...
				if (NULL != item)
				{
					...
					substitute_lld_macros(&value_tmp, jp_row, lld_macros, ZBX_MACRO_XML, NULL, 0);
			...
			case XML_CDATA_SECTION_NODE:
				...
				if (NULL != item)
				{
					...
					substitute_lld_macros(&value_tmp, jp_row, lld_macros, ZBX_MACRO_ANY, NULL, 0);
		...
		substitute_macros_in_xml_elements(item, jp_row, lld_macros, node->children);
	}
}
#endif
		...
		const zbx_vector_ptr_t *lld_macros, char *error, int maxerrlen)
{
	...
	substitute_macros_in_xml_elements(item, jp_row, lld_macros, root_element);
