...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_ABS                                                     *
 *                                                                            *
 * Purpose: evaluate function 'abs' for the item                              *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 *             ts         - [IN] the function evaluation time                 *
 *             error      - [OUT] the error message                           *
 *                                                                            *
 * Return value: SUCCEED - evaluated successfully, result is stored in 'value'*
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_ABS(zbx_variant_t *value, DC_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
			zbx_variant_set_dbl(value, fabs(record.value.dbl));
		else if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
			zbx_variant_set_ui64(value, fabs((double)record.value.ui64));
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_CBRT                                                    *
 *                                                                            *
 * Purpose: evaluate function 'cbrt' for the item                             *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_CBRT(zbx_variant_t *value, DC_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
			result_val = cbrt(record.value.dbl);
		else if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
			result_val = cbrt(record.value.ui64);

		zbx_variant_set_dbl(value, result_val);
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_CEIL                                                    *
 *                                                                            *
 * Purpose: evaluate function 'ceil' for the item                             *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_CEIL(zbx_variant_t *value, DC_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		double	ceil_val = ceil(record.value.dbl);

		if (0 > ceil_val)
			zbx_variant_set_dbl(value, ceil_val);
		else
			zbx_variant_set_ui64(value, (zbx_uint64_t)ceil_val);
 ...
 *                                                                            *
 * Purpose: evaluate function 'exp' for the item                              *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_EXP(zbx_variant_t *value, DC_ITEM *item, const char *function, const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		if (0 == strcmp(function, "m1"))
			func_ptr = expm1;
		else
			func_ptr = exp;

		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
			result_val = func_ptr((double)record.value.ui64);
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
			result_val = func_ptr(record.value.dbl);
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_FLOOR                                                   *
 *                                                                            *
 * Purpose: evaluate function 'floor' for the item                            *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_FLOOR(zbx_variant_t *value, DC_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		double	floor_val = floor(record.value.dbl);

		if (0 > floor_val)
			zbx_variant_set_dbl(value, floor_val);
		else
			zbx_variant_set_ui64(value, (zbx_uint64_t)floor_val);
 ...
 *                                                                            *
 * Purpose: evaluate function 'log' for the item                              *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_LOG(zbx_variant_t *value, DC_ITEM *item, const char *function, const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
 ...
 *                                                                            *
 * Purpose: evaluate function 'power' for the item                            *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, power value, time shift      *
 *             ts         - [IN] the function evaluation time                 *
 *             error      - [OUT] the error message                           *
 *                                                                            *
 * Return value: SUCCEED - evaluated successfully, result is stored in 'value'*
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_POWER(zbx_variant_t *value, DC_ITEM *item, const char *parameters, const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED != get_function_parameter_float(parameters, 1, ZBX_PARAM_MANDATORY, &arg))
	{
	...
	{
		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
			zbx_variant_set_ui64(value, pow(record.value.ui64, arg));
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
			zbx_variant_set_dbl(value, pow(record.value.dbl, arg));
 ...
 *                                                                            *
 * Purpose: evaluate function 'round' for the item                            *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_ROUND(zbx_variant_t *value, DC_ITEM *item, const char *parameters, const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL, arg;
	zbx_history_record_t	record;
	zbx_value_type_t	arg_type;
	...
	if (SUCCEED != get_function_parameter_int(parameters, 1, ZBX_PARAM_MANDATORY, &arg, &arg_type))
	{
		*error = zbx_strdup(*error, "invalid first parameter");
		goto out;
	}
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		multiplier = pow(10.0, arg);
		result_val = round(record.value.dbl * multiplier ) / multiplier;
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_RAND                                                    *
 *                                                                            *
 * Purpose: evaluate function 'rand' for the item                             *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_RAND(zbx_variant_t *value)
{
	zbx_uint64_t	rand_val;
	int		ret = SUCCEED;
	...
	zbx_variant_set_ui64(value, rand_val);
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_SIGNUM                                                  *
 *                                                                            *
 * Purpose: evaluate function 'signum' for the item                           *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_SIGNUM(zbx_variant_t *value, DC_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
		{
			if (record.value.ui64 == 0)
				ret_val = 0;
		}
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
		{
			if (record.value.dbl < 0)
				ret_val = -1;
			else if (record.value.dbl == 0)
				ret_val = 0;
		}

		zbx_variant_set_dbl(value, ret_val);
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_SQRT                                                    *
 *                                                                            *
 * Purpose: evaluate function 'sqrt' for the item                             *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_SQRT(zbx_variant_t *value, DC_ITEM *item, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
		{
			result_val = sqrt(record.value.ui64);
		}
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
		{
			if (0 <= record.value.dbl)
				result_val = sqrt(record.value.dbl);
			else
			{
				*error = zbx_strdup(*error, "Mathematical error, wrong value was passed");
				goto out;
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_TRUNCATE                                                *
 *                                                                            *
 * Purpose: evaluate function 'truncate' for the item                         *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_TRUNCATE(zbx_variant_t *value, DC_ITEM *item, const char *parameters, const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL, arg;
	zbx_history_record_t	record;
	...
	if (0 > arg)
	{
		*error = zbx_strdup(*error, "invalid first parameter's value");
		goto out;
	}
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
			...
			result_val = floor(multiplier * record.value.dbl) / multiplier;
...
/*****************************************************************************
*                                                                            *
* Function: evaluate_GEOM                                                    *
*                                                                            *
* Purpose: evaluate geometric function from math.h                           *
*                                                                            *
* Parameters: value      - [OUT] variant to store result                     *
*             item       - [IN] item (performance metric)                    *
*             func_ptr   - [IN] function to be called to calculate a value   *
*             parameters - [IN] Nth last value, time shift                   *
*             ts         - [IN] the function evaluation time                 *
*             error      - [OUT] the error message                           *
*                                                                            *
...
*                                                                            *
******************************************************************************/
static int	evaluate_GEOM(zbx_variant_t *value, DC_ITEM *item, double (*func_ptr)(double), const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
		{
			if ((acos == func_ptr || asin == func_ptr) && (record.value.ui64 > 1))
			{
				*error = zbx_strdup(*error, "Mathematical error, wrong value was passed");
				goto out;
			}
			else
				result_val = func_ptr(record.value.ui64);
		}
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
		{
			if ((acos == func_ptr || asin == func_ptr) && (record.value.dbl > 1 || record.value.dbl < -1))
			{
				*error = zbx_strdup(*error, "Mathematical error, wrong value was passed");
				goto out;
			}
...
/******************************************************************************
 *                                                                            *
 * Function: evaluate_ATAN2                                                   *
 *                                                                            *
 * Purpose: evaluate function 'atan2' for the item                            *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, atan2 value, time shift      *
 *             ts         - [IN] the function evaluation time                 *
 *             error      - [OUT] the error message                           *
 *                                                                            *
 * Return value: SUCCEED - evaluated successfully, result is stored in 'value'*
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_ATAN2(zbx_variant_t *value, DC_ITEM *item, const char *parameters, const zbx_timespec_t *ts, char **error)
{
	int			ret = FAIL;
	double			arg;
	zbx_history_record_t	record;
	...
	{
		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
			zbx_variant_set_dbl(value, atan2(record.value.ui64, arg));
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
			zbx_variant_set_dbl(value, atan2(record.value.dbl, arg));
 ...
 *                                                                            *
 * Purpose: evaluate function 'mod' for the item                              *
 *                                                                            *
 * Parameters: value      - [OUT] variant to store result                     *
 *             item       - [IN] item (performance metric)                    *
 *             parameters - [IN] Nth last value, time shift                   *
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_MOD(zbx_variant_t *value, DC_ITEM *item, const char *parameters, const zbx_timespec_t *ts,
		char **error)
{
	int			ret = FAIL, arg;
	zbx_history_record_t	record;
	...
	if (SUCCEED == zbx_vc_get_value(item->itemid, item->value_type, ts, &record))
	{
		...
		if (ITEM_VALUE_TYPE_UINT64 == item->value_type)
			result_val = fmod(record.value.ui64, arg);
		else if (ITEM_VALUE_TYPE_FLOAT == item->value_type)
			result_val = fmod(record.value.dbl, arg);
		...
		const zbx_timespec_t *ts, char **error)
{
	...
	else if (0 == strcmp(function, "abs"))
	{
		ret = evaluate_ABS(value, item, ts, error);
	}
	else if (0 == strcmp(function, "cbrt"))
	{
		ret = evaluate_CBRT(value, item, ts, error);
	}
	else if (0 == strcmp(function, "ceil"))
	{
		ret = evaluate_CEIL(value, item, ts, error);
	}
	else if (0 == strncmp(function, "exp", 3))
	{
		ret = evaluate_EXP(value, item, function + 3, ts, error);
	}
	else if (0 == strcmp(function, "floor"))
	{
		ret = evaluate_FLOOR(value, item, ts, error);
	}
	else if (0 == strncmp(function, "log", 3))
	{
		ret = evaluate_LOG(value, item, function + 3, ts, error);
	}
	else if (0 == strcmp(function, "power"))
	{
		ret = evaluate_POWER(value, item, parameter, ts, error);
	}
	else if (0 == strcmp(function, "round"))
	{
		ret = evaluate_ROUND(value, item, parameter, ts, error);
	}
	else if (0 == strcmp(function, "rand"))
	{
		ret = evaluate_RAND(value);
	}
	else if (0 == strcmp(function, "signum"))
	{
		ret = evaluate_SIGNUM(value, item, ts, error);
	}
	else if (0 == strcmp(function, "sqrt"))
	{
		ret = evaluate_SQRT(value, item, ts, error);
	}
	else if (0 == strcmp(function, "truncate"))
	{
		ret = evaluate_TRUNCATE(value, item, parameter, ts, error);
	}
	else if (0 == strcmp(function, "acos"))
	{
		ret = evaluate_GEOM(value, item, acos, ts, error);
	}
	else if (0 == strcmp(function, "asin"))
	{
		ret = evaluate_GEOM(value, item, asin, ts, error);
	}
	else if (0 == strcmp(function, "atan"))
	{
		ret = evaluate_GEOM(value, item, atan, ts, error);
	}
	else if (0 == strcmp(function, "atan2"))
	{
		ret = evaluate_ATAN2(value, item, parameter, ts, error);
	}
	else if (0 == strcmp(function, "cos"))
	{
		ret = evaluate_GEOM(value, item, cos, ts, error);
	}
	else if (0 == strcmp(function, "cosh"))
	{
		ret = evaluate_GEOM(value, item, cosh, ts, error);
	}
	else if (0 == strcmp(function, "cot"))
	{
		ret = evaluate_GEOM(value, item, cot, ts, error);
	}
	else if (0 == strcmp(function, "sin"))
	{
		ret = evaluate_GEOM(value, item, sin, ts, error);
	}
	else if (0 == strcmp(function, "sinh"))
	{
		ret = evaluate_GEOM(value, item, sinh, ts, error);
	}
	else if (0 == strcmp(function, "tan"))
	{
		ret = evaluate_GEOM(value, item, tan, ts, error);
	}
	else if (0 == strcmp(function, "degrees"))
	{
		ret = evaluate_GEOM(value, item, degrees, ts, error);
	}
	else if (0 == strcmp(function, "radians"))
	{
		ret = evaluate_GEOM(value, item, radians, ts, error);
	}
	else if (0 == strcmp(function, "mod"))
	{
		ret = evaluate_MOD(value, item, parameter, ts, error);
	}
	else if (0 == strcmp(function, "pi"))
	{
		zbx_variant_set_dbl(value, ZBX_MATH_CONST_PI);
		ret = SUCCEED;
	}
	else if (0 == strcmp(function, "e"))
	{
		zbx_variant_set_dbl(value, ZBX_MATH_CONST_E);
		ret = SUCCEED;
	}
	else
	{
