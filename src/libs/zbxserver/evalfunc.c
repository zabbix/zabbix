...
static int	get_function_parameter_uint31(zbx_uint64_t hostid, const char *parameters, int Nparam, int *value, int *flag)
{
	...
	if (SUCCEED == substitute_simple_macros(NULL, NULL, NULL, &hostid, NULL, NULL, NULL,
...
static int	get_function_parameter_str(zbx_uint64_t hostid, const char *parameters, int Nparam, char **value)
{
	...
	*value = zbx_malloc(*value, FUNCTION_PARAMETER_LEN_MAX);
	...
	res = substitute_simple_macros(NULL, NULL, NULL, &hostid, NULL, NULL, NULL, value, MACRO_TYPE_COMMON, NULL, 0);
clean:
	if (SUCCEED == res)
