...
static int	process_trap(zbx_socket_t *sock, char *s, zbx_timespec_t *ts,
		...
		const char *config_adapter_url, const char *config_adapter_connect_to, zbx_ipc_async_socket_t *rtc)
{
	int	ret = SUCCEED;
		...
		else if (0 == strcmp(value, ZBX_PROTO_VALUE_DEVICE_INIT))
		{
			if (0 != (zbx_get_program_type_cb() & ZBX_PROGRAM_TYPE_SERVER))
			{
				zbx_trapper_device_init(sock, &jp, config_comms, config_frontend_allowed_ip,
						config_adapter_url, config_adapter_connect_to);
			}
		}
		...
		const char *config_adapter_url, const char *config_adapter_connect_to, zbx_ipc_async_socket_t *rtc)
{
	...
	process_trap(sock, sock->buffer, ts, config_comms, config_vault, config_startup_time,
			...
			config_frontend_allowed_ip, config_adapter_url, config_adapter_connect_to, rtc);
}
