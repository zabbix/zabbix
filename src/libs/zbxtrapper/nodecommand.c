		...
		char **result, char **debug)
{
	...
	char			*timezone = NULL, *webhook_params_json = NULL, error[MAX_STRING_LEN];
	...
	*error = '\0';
	...
	timezone = zbx_db_get_user_timezone(user->userid);

	if (NULL == timezone || 0 == strcmp(timezone, ZBX_TIMEZONE_DEFAULT_VALUE))
	{
		zbx_config_t	cfg;

		zbx_config_get(&cfg, ZBX_CONFIG_FLAGS_DEFAULT_TIMEZONE);
		timezone = zbx_strdup(timezone, cfg.default_timezone);
	...
	if (ZBX_SCRIPT_TYPE_WEBHOOK != script.type)
	{
		if (SUCCEED != zbx_substitute_simple_macros_unmasked(NULL, problem_event, recovery_event, &user->userid,
				NULL, &host, NULL, NULL, NULL, NULL, NULL, timezone, &script.command, macro_type, error,
				sizeof(error)))
		{
			goto fail;
		}
		...
		if (SUCCEED != zbx_substitute_simple_macros(NULL, problem_event, recovery_event, &user->userid, NULL,
				&host, NULL, NULL, NULL, NULL, NULL, timezone, &script.command_orig, macro_type, error,
				sizeof(error)))
		{
			THIS_SHOULD_NEVER_HAPPEN;
			goto fail;
	...
	else
	{
		for (int i = 0; i < webhook_params.values_num; i++)
		{
			if (SUCCEED != zbx_substitute_simple_macros_unmasked(NULL, problem_event, recovery_event,
					&user->userid, NULL, &host, NULL, NULL, NULL, NULL, NULL, timezone,
