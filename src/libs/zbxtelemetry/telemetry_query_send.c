		...
		const char *config_ssl_key_location, char **out, char **error)
{
	...
	if (SUCCEED == ret)
	{
		zabbix_log(LOG_LEVEL_INFORMATION, "MYTEST: %s(): '%s'", __func__, ZBX_NULL2STR(http_out));
		*out = http_out;
		http_out = NULL;
	}
	else
	{
		zabbix_log(LOG_LEVEL_INFORMATION, "MYTEST2: %s(): '%s'", __func__, ZBX_NULL2STR(http_error));
		*error = http_error;
		http_error = NULL;
	}
		...
		const char *config_ssl_key_location, char **out, char **error)
{
	...
	if (SUCCEED != tq_send_query_clickhouse_raw(query, now, lasttimestamp, conn_params, config_source_ip,
			config_ssl_ca_location, config_ssl_cert_location, config_ssl_key_location, &result_raw, error))
		goto out;

	/* TODO: transform http_out to json */
	*out = zbx_strdup(NULL, result_raw);
