...
static int	regexp_compile(const char *pattern, int flags, zbx_regexp_t **regexp, const char **err_msg)
{
	...
	*err_msg = NULL;
	...
	if (NULL == (pcre2_regexp = pcre2_compile((PCRE2_SPTR8)pattern, PCRE2_ZERO_TERMINATED, PCRE2_UTF | flags,
...
#undef MATCHES_BUFF_SIZE
	...
	{
		if (0 <= (r = pcre2_match(regexp->pcre2_regexp, (PCRE2_SPTR8)string, PCRE2_ZERO_TERMINATED, 0, flags,
