		...
		zbx_vector_uint64_t *deleted_itemids, const zbx_config_vault_t *config_vault, int proxyconfig_frequency)
{
	...
	double		sec, csec, hsec, hisec, htsec, gmsec, hmsec, ifsec, idsec, isec, pisec, tsec, dsec, fsec,
	...
	zbx_dbsync_t	config_sync, hosts_sync, hi_sync, htmpl_sync, gmacro_sync, hmacro_sync, if_sync, items_sync,
			prototype_items_sync, item_discovery_sync, triggers_sync, tdep_sync,
	...
	if (SUCCEED == ZBX_CHECK_LOG_LEVEL(LOG_LEVEL_DEBUG))
	{
		total = csec + hsec + hisec + htsec + gmsec + hmsec + ifsec + idsec + isec  + pisec + tsec +
