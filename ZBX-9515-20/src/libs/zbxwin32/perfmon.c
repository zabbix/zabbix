...
PDH_STATUS	zbx_PdhCalculateCounterFromRawValue(const char *function, const char *counterpath, int *status,
		PDH_HCOUNTER handle, PDH_RAW_COUNTER rawValues[], int *olderRawValue, PDH_FMT_COUNTERVALUE *counterValue)
{
	PDH_STATUS	pdh_status;

	*olderRawValue = (*olderRawValue + 1) & 1;

	if (ERROR_SUCCESS == (pdh_status = PdhCalculateCounterFromRawValue(handle, PDH_FMT_DOUBLE | PDH_FMT_NOCAP100,
			&rawValues[(*olderRawValue + 1) & 1],
			(PERF_COUNTER_INITIALIZED < *status ? &rawValues[*olderRawValue] : NULL), counterValue)) &&
			PDH_CSTATUS_VALID_DATA != counterValue->CStatus && PDH_CSTATUS_NEW_DATA != counterValue->CStatus)
	{
		/* ERROR_SUCCESS does not guarantee that a valid counter value was calculated. These cases need to be */
		/* treated separately. Some (e.g., rate) counters require two raw values, MSDN lacks documentation    */
		/* about what happens but tests show that PDH_CSTATUS_INVALID_DATA is returned in this case.          */
	...
	else if (PDH_CALC_NEGATIVE_DENOMINATOR == pdh_status)
	{
		zabbix_log(LOG_LEVEL_DEBUG, "%s(): PDH_CALC_NEGATIVE_DENOMINATOR error occurred in counterpath '%s'."
				" Value ignored", function, counterpath);
	}
	else if (PDH_CALC_NEGATIVE_TIMEBASE == pdh_status)
	{
		zabbix_log(LOG_LEVEL_DEBUG, "%s(): PDH_CALC_NEGATIVE_TIMEBASE error occurred in counterpath '%s'."
				" Value ignored", function, counterpath);
	}
	else if (PDH_CALC_NEGATIVE_VALUE == pdh_status)
	{
		zabbix_log(LOG_LEVEL_DEBUG, "%s(): PDH_CALC_NEGATIVE_VALUE error occurred in counterpath '%s'."
				" Value ignored", function, counterpath);
	}
	else
	{
		zabbix_log(LOG_LEVEL_DEBUG, "%s(): cannot calculate performance counter value '%s': %s",
				function, counterpath, strerror_from_module(pdh_status, L"PDH.DLL"));
	}
...
PDH_STATUS	calculate_counter_value(const char *function, const char *counterpath, double *value)
{
	...
	int			olderRawValue = 0, status = PERF_COUNTER_INITIALIZED;
					...
					&rawValues[olderRawValue])))
	{
		...
		if (PDH_CSTATUS_INVALID_DATA != pdh_status && PDH_CALC_NEGATIVE_DENOMINATOR != pdh_status &&
		...
		{
			zabbix_log(LOG_LEVEL_DEBUG, "%s(): timeout while calculating counter value '%s': %s",
					function, counterpath, strerror_from_module(pdh_status, L"PDH.DLL"));
			break;
		}
