...
//			$user['user_groups'] = get_request('user_groups', array());
			...
			if(isset($_REQUEST['userid'])){
				...
				// if($result)	$result = API::UserGroup()->updateUsers(array('users' => $user, 'usrgrps' => $usrgrps));
				// if($result === false)
					// error(API::UserGroup()->resetErrors());
				...
				if(!$result)
					error(API::User()->resetErrors());
				// if($result) $result = API::UserGroup()->updateUsers(array('users' => $result, 'usrgrps' => $usrgrps));
				// if($result === false)
					// error(API::UserGroup()->resetErrors());
				$result = ($result === false) ? false : true;
				$result = DBend($result);
		...
		$users = API::User()->get(array('userids' => $_REQUEST['userid'],  'extendoutput' => 1));
		$user = reset($users);
...
// Add USER to GROUP
		...
		$user = API::User()->get(array('userids'=>$_REQUEST['userid'],'extendoutput'=>1));
		$user = reset($user);
...
// ----- GO -----
		...
		$group_userid = get_request('group_userid', array());
		$db_users = API::User()->get(array('userids' => $group_userid, 'extendoutput' => 1));
