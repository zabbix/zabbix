...
class CPieGraphDraw extends CGraphDraw {
	...
	protected function selectData() {
		...
		for ($i = 0; $i < $this->num; $i++) {
			...
			$isTrendsSwitchedOn = $config['hk_trends_global'] ? ($config['hk_trends'] > 0) : ($item['trends'] > 0);

			if (!$isTrendsSwitchedOn || (($item['history'] * SEC_PER_DAY) > (time() - ($from_time + $this->period / 2)))) {
