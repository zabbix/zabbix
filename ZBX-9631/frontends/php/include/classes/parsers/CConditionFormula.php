...
class CConditionFormula {
	...
	/**
	 * Array of constants positions [pos => constant]
	 *
	 * @var array
	 */
	private $constantsPositions = array();
	 ...
	 * @return string
	 */
	public function replaceLettersIds($formula, $ids) {
		$this->constants = array();
	...
	/**
	 * Find constant and its position, put values in constantsPositions array.
	 *
	 * @return boolean
	 */
	protected function findConstantPosition() {
		if ( $this->parseConstant() ) {
			$constant = end($this->constants);

			// parseConst() advances position to last character, let's get position of 1st character.
			$constantStartPosition = ( $this->pos - strlen($constant) + 1 );

			$this->constantsPositions[$constantStartPosition] = $constant;
	...
	/**
	 * Replace constants in formula with ids
	 *
	 * @return boolean
	 */
	protected function replaceConstants() {
		if ( ! empty( $this->constantsPositions ) ) {
			end($this->constantsPositions);
			while ( !is_null($position = key($this->constantsPositions)) ) {
				$constant = current($this->constantsPositions);

				if ( array_key_exists($constant, $this->ids) ) {
					$constantLength = strlen($constant);
					$constantId = '{' . $this->ids[$constant] . '}';
					$idLength = strlen($constantId);
					$this->formula = substr_replace($this->formula, $constantId, $position, $constantLength);
				}

				prev($this->constantsPositions);
			}
