...
$col = new CCol(new CButton('addAgentInterface', _('Add'), null, 'link_menu'));
...
$col = new CCol(new CButton('addSNMPInterface', _('Add'), null, 'link_menu'));
$col->setAttribute('colspan', 5);
...
foreach ($templates as $tid => $temp_name) {
	...
	$tmplList->addRow($temp_name, array(
		...
		isset($original_templates[$tid])
				? new CSubmit('unlink_and_clear['.$tid.']', _('Unlink and clear'), null, 'link_menu') : SPACE
	));
}
...
foreach ($hostInventoryFields as $inventoryNo => $inventoryInfo) {
	...
	$inventoryFormList->addRow($inventoryInfo['title'], array(
		$input,
		$populatingItem
	));
}
