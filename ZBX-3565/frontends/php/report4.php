...
$year = get_request('year', $CURRENT_YEAR);
...
if(($min_time = DBfetch(DBselect('select min(clock) as clock from alerts'))) && $min_time['clock']){
	$MIN_YEAR = date('Y', $min_time['clock']);
}
else{
...
$form->addItem(S_MEDIA_TYPE.SPACE);
...
switch($period){
	case 'yearly':
		$from = $MIN_YEAR;
		$to	= $CURRENT_YEAR;
		array_push($header, new CCol(S_YEAR, 'center'));
	...
	case 'monthly':
		$from = 1;
		$to	= 12;
		array_push($header, new CCol(S_MONTH,'center'));
	...
	case 'dayly':
		$from = 1;
		$to	= 365;
		array_push($header, new CCol(S_DAY,'center'));
	...
	default:
		$from = 0;
		$to	= 52;
		array_push($header,new CCol(S_FROM,'center'),new CCol(S_TILL,'center'));
		function get_time($w)	{
			global $year;
...
$db_users = DBselect('select * from users where '.DBin_node('userid'));
while($user_data = DBfetch($db_users)){
	array_push($header, new CImg('vtext.php?text='.$user_data['alias'].(($css == 'css_od.css')?'&color=white':'')));
...
order_result($media_types, 'description');
...
if(($media_type == 0) && !empty($media_types)){
