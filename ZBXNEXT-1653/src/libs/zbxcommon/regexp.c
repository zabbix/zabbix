 ...
 *          returned.                                                            *
 *                                                                               *
 * Parameters: text            - [IN] the parsed string                          *
 ...
 *                                                                               *
 *********************************************************************************/
static char	*zbx_regexp_sub_replace(const char *text, const char *output_template, regmatch_t *match, size_t nmatch)
{
	char		*ptr = NULL;
...
char	*zbx_regexp_sub(const char *string, const char *pattern, const char *output_template)
{
	...
	if (0 != regcomp(&re, pattern, REG_EXTENDED | REG_NEWLINE))
		return NULL;

	if (0 == regexec(&re, string, sizeof(match) / sizeof(match[0]), match, 0))
		ptr = zbx_regexp_sub_replace(string, output_template, match, sizeof(match) / sizeof(match[0]));
