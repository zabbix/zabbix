...
<tr id="slides_#{rowId}" class="sortable">
	...
	<td>#{name}</td>
	<td>
		<input class="input text" type="text" id="slides_#{rowId}_delay" name="slides[#{rowId}][delay]" placeholder=<?php echo CJs::encodeJson(_('default')); ?> value="" size="5" maxlength="5" onchange="validateNumericBox(this, true, false);" style="text-align: right;">
	</td>
	<td>
		<input type="button" class="input link_menu" id="remove_#{rowId}" remove_slide="#{rowId}" value=<?php echo CJs::encodeJson(_('Remove')); ?> onclick="removeSlide(this);" />
	</td>
</tr>
</script>
