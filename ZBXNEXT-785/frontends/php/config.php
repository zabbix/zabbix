...
elseif ($_REQUEST['config'] == 6) {
	...
	if (isset($_REQUEST['add_map'])) {
		...
		for ($i = 0; $i < $cnt; $i++) {
			if ($_REQUEST['valuemap'][$i]['value'] != $_REQUEST['add_value']) {
				continue;
			}
...
elseif ($_REQUEST['config'] == 10) {
	...
	elseif (inarr_isset(array('add_expression', 'new_expression'))) {
...
elseif ($_REQUEST['config'] == 10) {
	if (isset($_REQUEST['form'])) {
		...
		while ($exp = DBfetch($db_exps)) {
			...
			$expressions[$exp['regexpid']]->addRow(array($count[$exp['regexpid']], ' &raquo; ', $exp['expression'],
