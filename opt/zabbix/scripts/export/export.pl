...
dw_delete_csvs() unless(opt('dry-run'));
...
foreach (@$tlds_ref)
{
	...
	__get_test_data($tld, $from, $till);

	# dump each TLD in to not use all the memory
	dw_write_csv_files();
}
...
dw_write_csv_files();
dw_write_csv_catalogs() unless (opt('dry-run'));
...
sub __get_test_data
{
	...
	foreach my $service (keys(%{$services}))
	{
		...
		foreach my $row_ref (@$rows_ref)
		{
			...
			if ($inc_idx < $incidents_count && $incidents->[$inc_idx]->{'end'})
			{
				...
				dbg('failed_tests:', $incidents->[$inc_idx]->{'failed_tests'});
	...
	foreach (sort(keys(%$result)))
	{
		...
		foreach my $service (sort(keys(%{$result->{$tld}})))
		{
			...
			# incidents
			foreach (@{$result->{$tld}->{$service}->{'incidents'}})
			{
...
sub __format_rtt
{
	...
	return "UNDEF" unless (defined($rtt));
