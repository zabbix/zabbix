...
our @EXPORT = qw(ID_PROBE ID_TLD ID_NS_NAME ID_NS_IP ID_TRANSPORT_PROTOCOL ID_TEST_TYPE ID_SERVICE_CATEGORY
		...
		dw_write_csv_catalogs dw_delete_csvs dw_get_cycle_id dw_translate_cycle_id);
...
sub dw_delete_csvs
{
	foreach my $id_type (keys(%DATAFILES))
	{
		my $name = __csv_file_name($id_type);
		if (-f $name)
		{
	...
	foreach my $id_type (keys(%CATALOGS))
	{
		my $name = __csv_catalog_name($id_type);
		if (-f $name)
		{
			unlink($name) or fail("cannot delete file \"$name\": $!");
		}
...
sub __csv_catalog_name
{
	...
	my $file_name = $CATALOGS{$id_type};

	die("Catalog '$id_type' is unknown") unless $file_name;
