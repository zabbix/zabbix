...
notsupported:
	...
	if (ITEM_STATE_NOTSUPPORTED == h->state)
	{
		...
		if (0 != strcmp(item->db_error, h->value_orig.err))
		{
			...
			sql_start = sql_continue;
			zabbix_log(LOG_LEVEL_WARNING, "error reason for \"%s:%s\" changed: \"%s\"", item->host.host, item->key_origin, h->value_orig.err);
			zbx_free(value_esc);
	...
	else
	{
		if (ITEM_STATE_NOTSUPPORTED == item->db_state)
		{
			zabbix_log(LOG_LEVEL_WARNING, "item \"%s:%s\" became supported", item->host.host, item->key_orig);
