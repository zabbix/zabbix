...
// TODO Shall we use $_POST here? Otherwise google bot may remove all hosts
...
// TODO Replace with API calls
			...
			while ($dbHost = DBfetch($dbHosts)) {
				...
				if ($oldStatus == HOST_STATUS_MONITORED) {
					continue;
				}

// TODO Replace with API call
				$result &= updateHostStatus($dbHost['hostid'], HOST_STATUS_MONITORED);
...
// Why $hosts? Also $hosts may be set to empty array()
		...
		if ($result) {
			$response->setMessageOk(_n('Host enabled', 'Hosts enabled', $this->data['updatedHosts']));
		}
		else {
			$response->setMessageError(_n('Cannot enable host', 'Cannot enable hosts', $this->data['updatedHosts']));
		}
		return $response;
	}
}
