...
static void	config_hmacro_add_index(zbx_hashset_t *hmacro_index, ZBX_DC_HMACRO *hmacro)
{
	...
	{
		hmacro_hm = zbx_hashset_insert(hmacro_index, &hmacro_hm_local, sizeof(ZBX_DC_GMACRO_M));
...
static void	DCsync_gmacros(DB_RESULT result)
{
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		if (SUCCEED != zbx_macro_parse_dyn(row[1], &macro, &param))
		{
			zabbix_log(LOG_LEVEL_WARNING, "cannot parse macro \"%s\" parameter", row[1]);
			continue;
		}
		...
		if (0 == found || 0 != strcmp(gmacro->macro, macro) || 0 != zbx_strcmp_null(gmacro->param, param))
		{
			if (1 == found)
		...
		if (NULL == param)
			...
			DCstrpool_replace(found, &gmacro->param, param);
	...
	while (NULL != (gmacro = zbx_hashset_iter_next(&iter)))
	{
		...
		if (NULL != gmacro->param)
			zbx_strpool_release(gmacro->param);
...
static void	DCsync_hmacros(DB_RESULT result)
{
	...
	while (NULL != (row = DBfetch(result)))
	{
		...
		if (SUCCEED != zbx_macro_parse_dyn(row[2], &macro, &param))
		{
			zabbix_log(LOG_LEVEL_WARNING, "cannot parse host \"%s\" macro \"%s\" parameter", row[1],
					row[2]);
			continue;
		}
		...
		if (0 == found || 0 != strcmp(hmacro->macro, macro) || 0 != zbx_strcmp_null(hmacro->param, param))
		{
			if (1 == found)
		...
		if (NULL == param)
			...
			DCstrpool_replace(found, &hmacro->param, param);
	...
	while (NULL != (hmacro = zbx_hashset_iter_next(&iter)))
	{
		...
		if (NULL != hmacro->param)
			zbx_strpool_release(hmacro->param);
...
static int	DCget_host_macro(zbx_uint64_t *hostids, int host_num, const char *macro, const char *param,
		char **replace_to)
{
	...
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() macro:'%s' param:'%s'", __function_name, macro, NULL == param ? "none" :
			param);
	...
	for (i = 0; i < host_num; i++)
	{
		...
		if (NULL != (hmacro_hm = zbx_hashset_search(&config->hmacros_hm, &hmacro_hm_local)))
		{
			for (j = 0; j < hmacro_hm->hmacros.values_num; j++)
			{
				...
				{
					if (0 == zbx_strcmp_null(hmacro->param, param))
					{
					...
					if (NULL == *replace_to && NULL != param && NULL == hmacro->param)
						*replace_to = zbx_strdup(*replace_to, hmacro->value);
				}
			}
		}
	...
	if (FAIL == ret)
	{
		...
		if (0 != templateids.values_num)
		{
			...
			ret = DCget_host_macro(templateids.values, templateids.values_num, macro, param, replace_to);
		}
...
static void	DCget_global_macro(const char *macro, const char *param, char **replace_to)
{
	...
	if (NULL != (gmacro_m = zbx_hashset_search(&config->gmacros_m, &gmacro_m_local)))
	{
		for (i = 0; i < gmacro_m->gmacros.values_num; i++)
		{
			...
			{
				if (0 == zbx_strcmp_null(gmacro->param, param))
				{
				...
				/* check for the default (without parameters) macro value */
				if (NULL == *replace_to && NULL != param && NULL == gmacro->param)
					*replace_to = zbx_strdup(*replace_to, gmacro->value);
			}
		}
	}
...
void	DCget_user_macro(zbx_uint64_t *hostids, int host_num, const char *macro, char **replace_to)
{
	...
	if (SUCCEED != zbx_macro_parse_dyn(macro, &name, &param))
		goto out;
...
void	zbx_umc_resolve(zbx_hashset_t *cache)
{
	...
	while (NULL != (object = zbx_hashset_iter_next(&iter)))
	{
		for (i = 0; i < object->macros.values_num; i++)
		{
			...
			if (SUCCEED != zbx_macro_parse_dyn((char *)pair->first, &name, &param))
				continue;

			if (FAIL == DCget_host_macro(object->hostids.values, object->hostids.values_num, name, param,
					(char **)&pair->second))
			{
				DCget_global_macro(name, param, (char **)&pair->second);
			}
