 ...
 * Function: zbx_read2                                                        *
 *                                                                            *
 * Purpose: Read text lines from a file descriptor into buffer                *
 *                                                                            *
 * Parameters: fd          - [IN] file descriptor to read from                *
 *             buf         - [IN] buffer to read into                         *
 *             count       - [IN] buffer size in bytes                        *
 *             encoding    - [IN] pointer a to text string describing         *
 *                           encoding. The following encodings are            *
 *                           recognized:                                      *
 *                             "UNICODE"                                      *
 *                             "UNICODEBIG"                                   *
 *                             "UNICODEFFFE"                                  *
 ...
 *                           set.                                             *
 *             s_pos       - [IN/OUT] array of structures with start and end  *
 *                           positions of strings. The end position is a      *
 *                           position of the first byte of newline separator  *
 *                           at the end of the string.                        *
 *                           if 's_pos' is NULL, memory will be allocated.    *
 *             s_num       - [IN/OUT] number of used elements in 's_pos'      *
 *             s_num_alloc - [IN/OUT] number of allocated elements in 's_pos' *
 *                                                                            *
 * Return value: On success, the number of bytes read is returned (0 (zero)   *
 *               indicates end of file).                                      *
 *               On error, -1 is returned and errno is set appropriately.     *
 *                                                                            *
 * Comments: Reading continues until end-of-file of until buffer is full.     *
 *           Memory for 's_pos' may be allocated/reallocated if necessary.    *
 *           It is a responsibility of a caller to free 's_pos' memory.       *
 *           Newline separators are stored in the buffer, but the end         *
 *           positions are set to the beginning of the newline separators.    *
 *                                                                            *
 ******************************************************************************/
			...
			int *s_num_alloc)
{
	...
	if (NULL == *s_pos)
	{
		*s_num_alloc = 1000;	/* number of lines expected in 64 kB buffer */
		*s_pos = zbx_malloc(*s_pos, (size_t)*s_num_alloc * sizeof(zbx_start_end_pos_t));
	}

	*s_num = 0;

	if ('\0' == *encoding)	/* single-byte character set */
	{
		const char	*p1, *p, *p_end, *p_next;

		p1 = buf;			/* beginning of current line */
		p = buf;			/* current byte */
		p_end = buf + (size_t)nbytes;	/* end of data */
...
eol1:			/* end of line found */
			if (*s_num_alloc == *s_num)
			{
				*s_num_alloc += 1000;
				*s_pos = zbx_realloc(*s_pos, (size_t)*s_num_alloc * sizeof(zbx_start_end_pos_t));
			}

			(*s_pos)[*s_num].start = (size_t)(p1 - buf);
			(*s_pos)[*s_num].end = (size_t)(p - buf);
			(*s_num)++;
			p1 = p_next;
			p = p_next;
		}
	...
	else	/* 2- or 4-byte character set */
	{
		size_t	i1 = 0, i_next;

		i = 0;

		while (i <= (size_t)nbytes - szbyte)
		{
			if (0 == memcmp(&buf[i], lf, szbyte))	/* LF (Unix) */
			{
				i_next = i + szbyte;
