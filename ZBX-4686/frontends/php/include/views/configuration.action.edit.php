...
if (isset($data['actionid'])) {
	$frmAction->addVar('actionid', $data['actionid']);
}
...
$actionList->addRow(_('Default message'), new CTextArea('def_longdata', $data['def_longdata']));
...
if (EVENT_SOURCE_TRIGGERS == $data['eventsource']) {
	...
	if ($data['recovery_msg']) {
		...
		$actionList->addRow(_('Recovery message'), new CTextArea('r_longdata', $data['r_longdata']));
	}
	else {
...
foreach ($data['conditions'] as $id => $condition) {
	...
	if (!str_in_array($condition['conditiontype'], $allowedConditions)) {
		continue;
	}
...
foreach ($data['operations'] as $opid => $operation) {
	if (!str_in_array($operation['operationtype'], $allowedOperations)) {
		continue;
	}
...
if (isset($_REQUEST['new_operation'])) {
	...
	switch ($new_operation['operationtype']) {
		case OPERATION_TYPE_MESSAGE:
			...
			if (!$new_operation['opmessage']['default_msg']) {
				...
				$tblOper->addRow(array(_('Message'), new CTextArea('new_operation[opmessage][message]', $new_operation['opmessage']['message'])));
			}
			else {
		...
		case OPERATION_TYPE_COMMAND:
			...
			// add participations
			if (!isset($new_operation['opcommand_grp'])) {
				$new_operation['opcommand_grp'] = array();
			}
			if (!isset($new_operation['opcommand_hst'])) {
				$new_operation['opcommand_hst'] = array();
			}
			...
			// command
			$commandTextArea = new CTextArea('new_operation[opcommand][command]', $new_operation['opcommand']['command']);
	...
	$operationList->addRow(_('Operation details'), new CDiv(array($tblOper, $footer), 'objectgroup inlineblock border_dotted ui-corner-all'));
